...
|---|---|---|---|---|---|
| G1 | 退款（同店/跨店/跨日）| hot/cool | **hot** | 资金 Tier 1，收银台现场触发，需 < 100ms | ✅ 同意 |
| G2 | 修改订单 / 作废订单 | hot/cool | **hot** | 资金 Tier 1，操作连续性强 | ✅ 同意 |
| G3 | 储值卡充值 | hot/cool | **hot** | 资金 Tier 1，会员现场操作 | ✅ 同意 |
| G4 | 储值卡消费 / 余额查询 | hot/cool | **hot** | 收银关键路径 | ✅ 同意 |
| G5 | 优惠券核销 | hot/cool | **hot** | 收银关键路径，扫码后即扣 | ✅ 同意 |
| G6 | 礼品卡 | hot/cool | **hot** | 同 G3 | ✅ 同意 |
| G7 | 押金支付 / 退还（宴会 / 包间）| hot/cool | **hot** | 资金 Tier 1，CLAUDE.md §十七 已锁 Tier 1 | ✅ 同意 |
| G8 | 存酒查询 / 续存 / 提酒 | hot/cool | **hot** | 押金 Tier 1（§十七）| ✅ 同意 |
| G9 | 宴会订单创建（提前 1-7 天）| hot/cool | **cool** | 不在收银台高峰用，内勤前端用 | ✅ 同意 |
| G10 | 宴会当天结算 | hot/cool | **hot** | 结算时段高峰，Tier 1 | ✅ 同意 |
| G11 | 桌台预订（pre-booking）| hot/cool | **cool** | 不影响实时收银 | ✅ 同意 |
| G12 | 排队叫号 | hot/cool | **cool** | 独立 surface（前台/电视屏），不影响收银 | ✅ 同意 |
| G13 | 会员卡查询 / 绑定 | hot/cool | **cool** | 响应稍慢可接受，注册流程在前 | ✅ 同意 |
| G14 | 菜品 86 / 沽清 | hot/cool | **cool** | 厨师用，OrderScreen 接收 KDS 推送即可 | ✅ 同意 |
| G15 | 小费 / 服务费 | hot/cool | **hot** | 收银关键，包间最低消费一并 | ✅ 同意 |
| G16 | 打印小票补打 | hot/cool | **hot** | 收银台现场触发 | ✅ 同意 |
| G17 | 设备状态 / 重启 / 网络诊断 | hot/cool | **cool** | 维护用，IT/店长 | ✅ 同意 |
| G18 | 外卖单接单 / 推单 / 改单 | hot/cool | **hot** | 高频实时（美团/饿了么/抖音 webhook 推送）| ✅ 同意 |
| G19 | 全电发票申请（顾客现场扫码）| hot/cool | **hot** | 资金 Tier 1（§十七），现场触发 | ✅ 同意 |
| G20 | 全电发票管理（红冲/重打/批量）| hot/cool | **cool** | 财务后台用，非收银台 | ✅ 同意 |
...
### 灰色地带（§四 20 项）：
- G1 退款                         : [x] 同意 hot   [ ] 改 cool   [ ] 分屏
- G2 修改订单/作废                : [x] 同意 hot   [ ] 改 cool   [ ] 分屏
- G3 储值卡充值                   : [x] 同意 hot   [ ] 改 cool   [ ] 分屏
- G4 储值卡消费/余额查询          : [x] 同意 hot   [ ] 改 cool   [ ] 分屏
- G5 优惠券核销                   : [x] 同意 hot   [ ] 改 cool   [ ] 分屏
- G6 礼品卡                       : [x] 同意 hot   [ ] 改 cool   [ ] 分屏
- G7 押金支付/退还                : [x] 同意 hot   [ ] 改 cool   [ ] 分屏
- G8 存酒查询/续存/提酒           : [x] 同意 hot   [ ] 改 cool   [ ] 分屏
- G9 宴会订单创建（提前 1-7 天）  : [x] 同意 cool  [ ] 改 hot    [ ] 分屏
- G10 宴会当天结算                : [x] 同意 hot   [ ] 改 cool   [ ] 分屏
- G11 桌台预订                    : [x] 同意 cool  [ ] 改 hot    [ ] 分屏
- G12 排队叫号                    : [x] 同意 cool  [ ] 改 hot    [ ] 分屏
- G13 会员卡查询/绑定             : [x] 同意 cool  [ ] 改 hot    [ ] 分屏
- G14 菜品 86/沽清                : [x] 同意 cool  [ ] 改 hot    [ ] 分屏
- G15 小费/服务费                 : [x] 同意 hot   [ ] 改 cool   [ ] 分屏
- G16 打印小票补打                : [x] 同意 hot   [ ] 改 cool   [ ] 分屏
- G17 设备状态/重启/网络诊断      : [x] 同意 cool  [ ] 改 hot    [ ] 分屏
- G18 外卖单接单/推单/改单        : [x] 同意 hot   [ ] 改 cool   [ ] 分屏
- G19 全电发票申请（顾客现场扫码）: [x] 同意 hot   [ ] 改 cool   [ ] 分屏
- G20 全电发票管理（红冲/重打/批量）: [x] 同意 cool  [ ] 改 hot    [ ] 分屏

### 签字
日期：2026-05-07
创始人：lichun（创始人，G1-G20 全同意 Claude 建议）
```
