...
#### Phase 4a-2 阻塞项：`approval_instances` 三 schema 冲突（**必须 founder review 时决策**）

`tx-org/approval_flow.py` (v031 schema) / `tx-org/approval_flow_engine.py` (v059 schema) /
`tx-expense/approval_engine.py` (v235c schema) 三套 ORM model 期望同表名不同 schema。
路线 a 多 service 各有独立 alembic 后，**仍不能直接生成 baseline**：你无法为 tx-org +
tx-expense 同时拆出一份 `approval_instances` baseline SQL（schema 互斥）。

founder 必须在 4a-2 选择其一：

| 候选 | 描述 | 工作量 |
|---|---|---|
| 1 | 保留 1 个 schema（推荐 v235c — tx-expense），另 2 service rename 表（`tx_org_approval_flow_instances` / `tx_org_workflow_instances`）+ 改 ORM model | 中 |
| 2 | 三 service 各建独立表（`tx_expense_approval_instances` / `tx_org_*` / `tx_ops_*`）— 路线 a 多 alembic 自然支持，无需 cross-service 协调 | 大 |
| 3 | 如果三 schema 实际是同一逻辑实体的不同视角 — 选最完整的 schema 作 canonical，其他 ORM model 改字段名映射 | 大 + 需业务调研 |

类似 case 同样阻塞 4a-2：`banquet_leads` (4 schema) / `delivery_dispatches` (2) /
`pos_crash_reports` (2) / `banquet_contracts` (2) — 25 个 multi-creator 表全部需逐一决策。
