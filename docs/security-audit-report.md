...
`services/gateway/src/core/tenant_filter.py` 中 `TENANT_TABLES` 集合仅包含 18 张表，与 RLS 策略覆盖的表一致。但项目有 130+ 个模型含 store_id，意味着**绝大多数表既没有 RLS 保护，也没有 ORM 层自动过滤**。
...
在整个 `services/` 目录中未搜索到 `SET ROLE`、`SET LOCAL`、`SECURITY DEFINER`、`BYPASSRLS` 等危险模式。
...
**文件**: `infra/nginx/conf.d/snippets/security-headers.conf`
...
- 虽然 FastAPI 层有 `RateLimitMiddleware`，但 Nginx 层缺乏保护意味着：
  - DDoS 攻击请求会全部打到应用层
...
**文件**: `services/gateway/src/core/config.py`
...
2. 对于 API 域名，`$tenant_id` 来自 `map` 默认值 `platform_admin`，所有通过 API 域名的请求都会获得 `platform_admin` 权限
...
开发环境端口暴露可接受，但需确保这些端口不会在服务器防火墙上开放。
