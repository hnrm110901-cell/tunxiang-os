...
| 指标 | 当前 | M1（30天） | M2（90天） | M3（180天） |
|------|------|-----------|-----------|------------|
| Tier 1 测试通过率 | 部分 | 100% | 100% | 100% |
| P99 延迟（200桌并发） | 未实测 | < 300ms | < 200ms | < 200ms |
| 支付成功率 | 未实测 | > 99% | > 99.9% | > 99.9% |
| 断网恢复（数据完整） | 256s 同步 | 4h | 4h | 8h |
| **触控规范达标率（Store）** | ~70% | ≥ 95% | ≥ 99% | 100% |
| **品牌色硬编码数** | ~134 处 | 0 | 0 | 0 |
| **a11y 评分（axe-core）** | 未测 | 报告基线 | ≥ 80 | ≥ 90 |
| 占位 Agent 数（共 8 个） | 8 | 6 | 2 | 0 |
| A2UI 白名单组件数 | 6 | 6 | 18 | 24 |
| Core ML 真实模型数 | 0 / 4 | 0 / 4 | 1 / 4 | 4 / 4 |
...
| S8-05 | 4h 断网压测（CRDT 验证） | edge/sync-engine | 4 | T1 | 重连后 0 丢失、0 冲突 |
