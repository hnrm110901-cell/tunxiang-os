...
## Step 6: Seed emit_event (触发 tx_events_emit_total Counter)

`tx_events_emit_total` 由 `shared/events/src/emitter.py` 注册, 当任何服务调用
`emit_event()` 时递增。tx-event-relay 本身提供 relay 专属指标
(`tx_event_relay_outbox_pending_count` 等) 无需 seed 即可查询。

若要验证 `tx_events_emit_total`, 可通过 tx-trade 的任意写路径触发 emit_event。
推荐使用 gateway health 端点确认服务在线后通过 seed 脚本 (T3 follow-up):

```bash
# 方案 A: 查询 tx-event-relay 自身的 relay 指标 (无需 seed, 服务启动即有值)
curl -sf "http://localhost:9090/api/v1/query?query=tx_event_relay_outbox_pending_count" \
  | jq '.data.result'
# 预期: [{...,"value":[timestamp,"0"]}] (shadow 期间 outbox 表为空, count=0 是预期)

# 方案 B: 查询 tx_events_emit_total (需要通过 tx-trade 下单触发 emit_event)
# 如有业务测试环境可调用 tx-trade cashier endpoint 创建测试订单
# 无业务流量时 counter 不存在 / result=[] 是正常的 (Counter 懒初始化)
curl -sf "http://localhost:9090/api/v1/query?query=tx_events_emit_total" \
  | jq '.data.result'
```

注意: `tx_events_emit_total` 是懒初始化 Counter, 没有 emit_event 调用时不出现在
`/metrics` 输出。Counter 存在但 value=0 或 result=[] 均属正常 dev 环境状态。
**核心验收目标是 Step 5 三 job health=up**, 表示 Prometheus 可正确鉴权 + 抓取指标。
