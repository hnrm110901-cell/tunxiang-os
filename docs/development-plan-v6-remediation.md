...
**文件**: `services/gateway/src/core/celery_tasks.py` (151个broad except, 含1个静默吞没)
...
**新建文件**: `services/gateway/src/core/exceptions.py`
...
```python
# shared/adapters/pinzhi/tests/test_adapter.py
...
repos:
  ...
  - repo: https://github.com/PyCQA/bandit
    ...
    hooks:
      - id: bandit
        args: [-r, services/gateway/src/, -ll]
```
...
```bash
grep -rn "anthropic\|claude\|Anthropic" services/gateway/src/ --include="*.py"
```

**Step 2**: 建立ModelRouter (v1配置字典版)
- 文件: `services/gateway/src/core/model_router.py`
- 统一调用接口 + 成本计量 + 调用日志
...
**每客户一个快照文件**: `docs/ontology-snapshots/{customer_code}.md`
