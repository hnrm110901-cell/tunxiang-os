...
class TestCreateOverrideUpsert:
    ...
    def test_upsert_channel_whitelist_complete(self) -> None:
        ...
        assert required_channels.issubset(_VALID_CHANNELS), f"缺少渠道: {required_channels - _VALID_CHANNELS}"
...
class TestConflictDetection:
    ...
    def test_no_conflict_below_threshold(self) -> None:
        ...
        diff_rate = (delivery_price - dine_in_price) / dine_in_price
        assert diff_rate <= threshold_rate, f"五一广场店涨价 {diff_rate:.1%} 未超过阈值 {threshold_rate:.1%}，不应告警"
    ...
    def test_mock_override_ov004_should_conflict(self) -> None:
        ...
        assert diff_rate > 0.30, f"ov-004 价差率 {diff_rate:.1%} 应超过 30% 阈值"
    ...
    def test_conflict_endpoint_route_registered(self) -> None:
        ...
        assert any("conflicts" in r for r in routes), f"冲突检测路由未注册，已注册路由: {routes}"

    def test_stats_endpoint_route_registered(self) -> None:
        """统计端点应正确注册在路由上。"""
        routes = [r.path for r in router.routes]
        assert any("stats" in r for r in routes), f"统计路由未注册，已注册路由: {routes}"
