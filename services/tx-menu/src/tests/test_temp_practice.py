...
class TestBuildCustomizations:
    ...
    def test_practice_name_fallback(self):
        ...
        result = build_customizations([{"id": "p-001", "practice_name": "半糖", "additional_price_fen": 0}])
...
class TestCalcPracticeExtraFen:
    ...
    def test_with_extra_price(self):
        """正常提取加价"""
        cust = build_customizations([{"practice_id": "p1", "name": "加蛋", "additional_price_fen": 200, "quantity": 2}])
        assert calc_practice_extra_fen(cust) == 400

    def test_zero_extra_price(self):
        """纯免费做法，加价为0"""
        cust = build_customizations([{"practice_id": "p1", "name": "微辣", "additional_price_fen": 0}])
