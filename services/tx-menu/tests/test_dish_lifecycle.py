...
class TestDishHealthScoreEngine:
    ...
    def _inject(
        self, dish_id, store_id, tenant_id, price=5000, cost=2000, total_sales=100, return_count=0, total_orders=100
    ):
        inject_dish_score_data(
            dish_id,
            store_id,
    ...
    def test_total_score_is_sum_of_dimensions(self):
        """综合评分 == 三维子分之和"""
        self._inject(
            "dish-1", STORE_1, TENANT_A, price=5000, cost=2000, total_sales=50, return_count=5, total_orders=100
        )
        engine = DishHealthScoreEngine()
        score = run(engine.score_dish("dish-1", STORE_1, TENANT_A, db=None))
        assert score is not None
        assert abs(score.total_score - (score.margin_score + score.sales_rank_score + score.review_score)) < 0.01
    ...
    def test_healthy_status(self):
        """高分菜品状态为 healthy"""
        self._inject(
            "dish-1", STORE_1, TENANT_A, price=5000, cost=2000, total_sales=200, return_count=0, total_orders=100
        )
        engine = DishHealthScoreEngine()
        score = run(engine.score_dish("dish-1", STORE_1, TENANT_A, db=None))
        assert score is not None
    ...
    def test_critical_status(self):
        ...
        # 毛利率极低 + 退菜率高 + 销量最低
        self._inject(
            "dish-1", STORE_1, TENANT_A, price=5000, cost=4800, total_sales=1, return_count=30, total_orders=100
        )
        engine = DishHealthScoreEngine()
        score = run(engine.score_dish("dish-1", STORE_1, TENANT_A, db=None))
        assert score is not None
    ...
    def test_custom_weights(self):
        ...
        self._inject(
            "dish-1", STORE_1, TENANT_A, price=5000, cost=2000, total_sales=100, return_count=0, total_orders=100
        )
        score = run(engine.score_dish("dish-1", STORE_1, TENANT_A, db=None))
        assert score is not None
    ...
    def test_batch_score_sorted_ascending(self):
        """批量评分按综合分升序返回"""
        self._inject(
            "dish-bad", STORE_1, TENANT_A, price=5000, cost=4800, total_sales=1, return_count=30, total_orders=100
        )
        self._inject(
            "dish-good", STORE_1, TENANT_A, price=5000, cost=2000, total_sales=200, return_count=0, total_orders=100
        )
        engine = DishHealthScoreEngine()
...
class TestNewDishEvaluation:
    ...
    def _inject_new_dish(self, dish_id, store_id, tenant_id, launched_days_ago=7, eval_sales=20, price=5000, cost=2000):
        inject_dish_lifecycle_data(
            dish_id,
            store_id,
            tenant_id,
    ...
    def test_passing_dish_verdict_pass(self):
        """销量和毛利均达标时评测通过"""
        self._inject_new_dish("dish-pass", STORE_1, TENANT_A, eval_sales=20, price=5000, cost=2000)
    ...
    def test_low_sales_verdict_low_sales(self):
        ...
        self._inject_new_dish("dish-low-sales", STORE_1, TENANT_A, eval_sales=LOW_SALES_THRESHOLD - 1)
        svc = DishLifecycleService()
        reports = run(svc.check_new_dish_evaluations(TENANT_A))
    ...
    def test_low_margin_verdict_low_margin(self):
        ...
        # 毛利率 10% < 15%
        self._inject_new_dish("dish-low-margin", STORE_1, TENANT_A, eval_sales=20, price=5000, cost=4500)
        svc = DishLifecycleService()
        reports = run(svc.check_new_dish_evaluations(TENANT_A))
    ...
    def test_both_fail_verdict_failed(self):
        ...
        self._inject_new_dish("dish-failed", STORE_1, TENANT_A, eval_sales=0, price=5000, cost=4500)
        svc = DishLifecycleService()
        reports = run(svc.check_new_dish_evaluations(TENANT_A))
    ...
    def test_not_yet_7_days_not_included(self):
        """上架未满7天的新品不进入评测"""
        self._inject_new_dish("dish-too-new", STORE_1, TENANT_A, launched_days_ago=3)
        svc = DishLifecycleService()
        reports = run(svc.check_new_dish_evaluations(TENANT_A))
    ...
    def test_much_older_dish_not_included(self):
        """上架超过8天的菜品不进入7天评测窗口"""
        self._inject_new_dish("dish-old", STORE_1, TENANT_A, launched_days_ago=30)
        svc = DishLifecycleService()
        reports = run(svc.check_new_dish_evaluations(TENANT_A))
    ...
    def test_low_sales_generates_notification(self):
        """低销量评测结果生成通知"""
        self._inject_new_dish("dish-notify", STORE_1, TENANT_A, eval_sales=0)
        svc = DishLifecycleService()
...
class TestSelloutWarnings:
    ...
    def _inject_dish(self, dish_id, store_id, tenant_id, stock_qty=10.0, daily_avg=5.0):
        inject_dish_lifecycle_data(
            dish_id,
            store_id,
            tenant_id,
...
class TestRemovalSuggestions:
    ...
    def test_low_health_30_days_suggests_removal(self):
        """健康分低于40持续30天建议下架"""
        self._inject("dish-low", STORE_1, TENANT_A, low_health_since=_days_ago(REMOVAL_LOW_HEALTH_DAYS + 1))
        svc = DishLifecycleService()
    ...
    def test_critical_margin_suggests_removal(self):
        """毛利率低于10%建议下架"""
        # 毛利率 4% < 10%
        self._inject("dish-margin", STORE_1, TENANT_A, price_fen=5000, cost_fen=4800)
        svc = DishLifecycleService()
        suggestions = run(svc.generate_removal_suggestions(STORE_1, TENANT_A))
    ...
    def test_healthy_dish_no_removal_suggestion(self):
        """健康菜品不产生下架建议"""
        self._inject("dish-good", STORE_1, TENANT_A, price_fen=5000, cost_fen=2000, eval_period_sales=30)
        svc = DishLifecycleService()
        suggestions = run(svc.generate_removal_suggestions(STORE_1, TENANT_A))
