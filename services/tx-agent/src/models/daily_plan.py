...
class DailyPlan(TenantBase):
    ...
    store_id: Mapped[str] = mapped_column(
        String(100), nullable=False, index=True, comment="门店ID",
    )
    plan_date: Mapped[str] = mapped_column(
        String(20), nullable=False, index=True, comment="计划日期 YYYY-MM-DD",
    )
    ...
    menu_suggestions: Mapped[dict] = mapped_column(
        JSON, nullable=False, default=list, comment="排菜建议",
    )
    procurement_list: Mapped[dict] = mapped_column(
        JSON, nullable=False, default=list, comment="采购清单",
    )
    staffing_adjustments: Mapped[dict] = mapped_column(
        JSON, nullable=False, default=list, comment="排班微调",
    )
    marketing_triggers: Mapped[dict] = mapped_column(
        JSON, nullable=False, default=list, comment="营销触发",
    )
    risk_alerts: Mapped[dict] = mapped_column(
        JSON, nullable=False, default=list, comment="风险预警",
    )
    ...
    approval_notes: Mapped[dict | None] = mapped_column(
        JSON, nullable=True, comment="审批备注（含逐项批/拒）",
    )

    # 执行
    execution_status: Mapped[dict | None] = mapped_column(
        JSON, nullable=True, comment="各项执行状态",
    )
    outcome_summary: Mapped[dict | None] = mapped_column(
        JSON, nullable=True, comment="执行结果摘要",
    )
    ...
    summary: Mapped[dict | None] = mapped_column(
        JSON, nullable=True, comment="汇总统计",
    )
