...
class SOPIMInteraction(TenantBase):
    ...
    message_type: Mapped[str] = mapped_column(
        Text,
        nullable=False,
        comment="消息类型：task_card / quick_reply / photo_upload / voice_cmd / coaching_card / alert_card",
    )
...
class SOPQuickAction(TenantBase):
    ...
    action_code: Mapped[str] = mapped_column(
        Text,
        nullable=False,
        comment="操作代码（唯一标识）：confirm_task / photo_check / flag_issue / call_support / quick_note",
    )
