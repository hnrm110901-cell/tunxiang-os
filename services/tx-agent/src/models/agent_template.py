...
class AgentTemplate(TenantBase):
    ...
    __table_args__ = (UniqueConstraint("tenant_id", "name", name="uq_agent_template_tenant_name"),)

    # 基础信息
    name: Mapped[str] = mapped_column(
        String(100), nullable=False, index=True, comment="Agent模板名（如 discount_guard）"
    )
    display_name: Mapped[str | None] = mapped_column(String(200), comment="中文显示名（如 折扣守护）")
    ...
    # 运行配置
    priority: Mapped[str] = mapped_column(String(10), default="P2", server_default="P2", comment="优先级 P0/P1/P2")
    ...
    # Agent 自定义配置
    config_json: Mapped[dict | None] = mapped_column(JSON, nullable=True, comment="Agent自定义配置")
    model_preference: Mapped[str | None] = mapped_column(
        String(100), nullable=True, comment="模型偏好（如 claude-haiku）"
    )
    tool_whitelist: Mapped[list | None] = mapped_column(JSON, nullable=True, comment="允许调用的工具列表")
