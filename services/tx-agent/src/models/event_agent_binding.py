...
class EventAgentBinding(TenantBase):
    ...
    event_type: Mapped[str] = mapped_column(String(100), nullable=False, comment="事件类型（如 trade.order.paid）")
    agent_id: Mapped[str] = mapped_column(String(100), nullable=False, comment="目标 Agent ID（如 discount_guard）")
    action: Mapped[str] = mapped_column(String(100), nullable=False, comment="要调用的 action（如 log_violation）")
    ...
    description: Mapped[str | None] = mapped_column(String(500), nullable=True, comment="中文描述")
