...
class SessionRun(TenantBase):
    ...
    agent_template_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), nullable=True, comment="关联的 Agent 模板 ID",
    )
    agent_template_name: Mapped[str | None] = mapped_column(
        String(100), nullable=True, comment="冗余存储模板名（查询方便）",
    )

    # 门店
    store_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), nullable=True, index=True, comment="门店 ID",
    )

    # 触发信息
    trigger_type: Mapped[str] = mapped_column(
        String(50), nullable=False, comment="触发方式：event/manual/scheduled/api",
    )
    trigger_data: Mapped[dict | None] = mapped_column(
        JSON, nullable=True, comment="触发数据（事件内容/用户指令等）",
    )
    ...
    plan_id: Mapped[str | None] = mapped_column(
        String(100), nullable=True, comment="关联的 ExecutionPlan.plan_id",
    )
    plan_snapshot: Mapped[dict | None] = mapped_column(
        JSON, nullable=True, comment="ExecutionPlan 快照",
    )

    # 结果
    result_json: Mapped[dict | None] = mapped_column(
        JSON, nullable=True, comment="最终结果",
    )
    ...
    total_steps: Mapped[int] = mapped_column(
        Integer, default=0, server_default="0", comment="总步骤数",
    )
    completed_steps: Mapped[int] = mapped_column(
        Integer, default=0, server_default="0", comment="已完成步骤数",
    )
    failed_steps: Mapped[int] = mapped_column(
        Integer, default=0, server_default="0", comment="失败步骤数",
    )

    # 资源消耗
    total_tokens: Mapped[int] = mapped_column(
        Integer, default=0, server_default="0", comment="消耗的 token 总数",
    )
    total_cost_fen: Mapped[int] = mapped_column(
        Integer, default=0, server_default="0", comment="费用（分）",
    )
