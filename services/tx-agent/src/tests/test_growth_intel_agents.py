...
class TestNewCustomerConvertAgent:
    ...
    async def test_identify_new_customers(self, agent):
        result = await agent.execute("identify_new_customers", {
            "customers": [
                {"customer_id": "c1", "name": "张三", "total_visits": 1, "first_visit_days_ago": 3,
                 "avg_spend_fen": 12000, "has_registered": True, "source_channel": "美团"},
                {"customer_id": "c2", "name": "李四", "total_visits": 1, "first_visit_days_ago": 5,
                 "avg_spend_fen": 5000, "has_registered": False, "source_channel": "门店自然到店"},
                {"customer_id": "c3", "name": "王五", "total_visits": 10, "first_visit_days_ago": 90,
                 "avg_spend_fen": 20000, "has_registered": True},
            ],
            "lookback_days": 7,
        })
    ...
    async def test_generate_welcome_offer(self, agent):
        result = await agent.execute("generate_welcome_offer", {
            "customer_id": "c1",
            "avg_spend_fen": 16000,
            "preferences": ["火锅", "甜品"],
        })
    ...
    async def test_predict_conversion_probability(self, agent):
        result = await agent.execute("predict_conversion_probability", {
            "customers": [
                {"customer_id": "c1", "first_spend_fen": 15000, "dwell_minutes": 70,
                 "ordered_items": 5, "has_registered": True, "source_channel": "老客推荐"},
                {"customer_id": "c2", "first_spend_fen": 3000, "dwell_minutes": 20,
                 "ordered_items": 2, "has_registered": False, "source_channel": "美团"},
            ],
        })
        assert result.success
        assert result.data["total"] == 2
...
class TestDormantRecallAgent:
    ...
    async def test_detect_dormant_users(self, agent):
        result = await agent.execute("detect_dormant_users", {
            "members": [
                {"customer_id": "c1", "name": "张三", "last_visit_days_ago": 20, "total_spent_fen": 50000, "visit_count": 5},
                {"customer_id": "c2", "name": "李四", "last_visit_days_ago": 45, "total_spent_fen": 30000, "visit_count": 3},
                {"customer_id": "c3", "name": "王五", "last_visit_days_ago": 80, "total_spent_fen": 100000, "visit_count": 20},
                {"customer_id": "c4", "name": "赵六", "last_visit_days_ago": 5, "total_spent_fen": 10000, "visit_count": 2},
            ],
        })
        assert result.success
    ...
    async def test_generate_recall_strategy(self, agent):
        result = await agent.execute("generate_recall_strategy", {
            "tier": "deep",
            "reason": "价格敏感",
            "customer_id": "c3",
            "customer_name": "王五",
    ...
    async def test_predict_churn_risk(self, agent):
        result = await agent.execute("predict_churn_risk", {
            "members": [
                {"customer_id": "c1", "name": "高风险", "last_visit_days_ago": 100, "monthly_frequency": 0,
                 "spend_trend": -30, "complaint_count": 2},
                {"customer_id": "c2", "name": "低风险", "last_visit_days_ago": 5, "monthly_frequency": 8,
                 "spend_trend": 10, "complaint_count": 0},
            ],
        })
        assert result.success
...
class TestBanquetGrowthAgent:
    ...
    async def test_recommend_banquet_package(self, agent):
        result = await agent.execute("recommend_banquet_package", {
    ...
    async def test_analyze_banquet_revenue(self, agent):
        result = await agent.execute("analyze_banquet_revenue", {
            "banquets": [
...
class TestSeasonalCampaignAgent:
    ...
    async def test_recommend_seasonal_dishes(self, agent):
        result = await agent.execute("recommend_seasonal_dishes", {
            "event_id": "mid_autumn",
            "existing_menu": ["月饼"],
        })
        assert result.success
...
class TestReferralGrowthAgent:
    ...
    async def test_design_referral_campaign(self, agent):
        result = await agent.execute("design_referral_campaign", {
    ...
    async def test_select_seed_users(self, agent):
        result = await agent.execute("select_seed_users", {
            "members": [
                {"customer_id": "c1", "name": "大V", "wechat_friends": 500, "past_referrals": 10,
                 "social_shares": 20, "visit_count": 15, "avg_rating": 5},
                {"customer_id": "c2", "name": "普通", "wechat_friends": 50, "past_referrals": 0,
                 "social_shares": 0, "visit_count": 2, "avg_rating": 3},
            ],
            "top_n": 5,
        })
        assert result.success
...
class TestHighValueMemberAgent:
    ...
    async def test_identify_high_value_members(self, agent):
        result = await agent.execute("identify_high_value_members", {
            "members": [
    ...
    async def test_alert_high_value_churn(self, agent):
        result = await agent.execute("alert_high_value_churn", {
            "members": [
                {"customer_id": "c1", "name": "VIP", "annual_spend_fen": 6000000,
                 "last_visit_days_ago": 45, "frequency_decline_pct": 50, "spend_decline_pct": 30, "recent_complaint": True},
...
class TestOffPeakTrafficAgent:
    ...
    async def test_design_off_peak_offer(self, agent):
        result = await agent.execute("design_off_peak_offer", {
            "slot": "afternoon",
...
class TestContentGenerationAgent:
    ...
    async def test_generate_marketing_copy(self, agent):
        result = await agent.execute("generate_marketing_copy", {
    ...
    async def test_generate_video_script(self, agent):
        result = await agent.execute("generate_video_script", {
            "topic": "湘菜探店",
            "duration_seconds": 15,
            "brand_name": "尝在一起",
            "featured_dish": "剁椒鱼头",
...
class TestCompetitorWatchAgent:
    ...
    async def test_scan_competitor_updates(self, agent):
        result = await agent.execute("scan_competitor_updates", {
            "competitors": [
                {"name": "海底捞", "recent_events": [
                    {"type": "price_drop_major", "title": "全线降价10%", "detail": "主力菜品降价", "date": "2026-03-20"},
                    {"type": "new_product", "title": "新品上线", "detail": "春季新菜3款", "date": "2026-03-22"},
                ]},
                {"name": "西贝", "recent_events": [
    ...
    async def test_detect_price_change(self, agent):
        result = await agent.execute("detect_price_change", {
            "competitor_prices": [
...
class TestReviewInsightAgent:
    ...
    async def test_extract_review_topics(self, agent):
        result = await agent.execute("extract_review_topics", {
            "reviews": [
    ...
    async def test_analyze_bad_review_root_cause(self, agent):
        result = await agent.execute("analyze_bad_review_root_cause", {
            "bad_reviews": [
...
class TestTrendDiscoveryAgent:
    ...
    async def test_analyze_search_hot_words(self, agent):
        result = await agent.execute("analyze_search_hot_words", {
            "search_data": [
                {"keyword": "小龙虾", "search_volume": 50000, "trend": "上升", "growth_pct": 30, "category": "海鲜"},
                {"keyword": "火锅", "search_volume": 80000, "trend": "持平", "growth_pct": 2, "category": "火锅"},
                {"keyword": "轻食", "search_volume": 30000, "trend": "上升", "growth_pct": 50, "category": "轻食沙拉"},
            ],
            "platform": "美团",
        })
        assert result.success
    ...
    async def test_predict_category_trend(self, agent):
        result = await agent.execute("predict_category_trend", {
            "categories": [
...
class TestNewProductScoutAgent:
    ...
    async def test_assess_feasibility(self, agent):
        result = await agent.execute("assess_feasibility", {
            "dish_name": "松露炒饭",
    ...
    async def test_suggest_pricing(self, agent):
        result = await agent.execute("suggest_pricing", {
            "dish_name": "松露炒饭",
            "cost_fen": 2000,
...
class TestIngredientRadarAgent:
    ...
    async def test_compare_suppliers(self, agent):
        result = await agent.execute("compare_suppliers", {
            "ingredient": "五花肉",
            "suppliers": [
                {"name": "长沙肉联厂", "price_per_kg_fen": 4500, "quality_score": 90,
                 "avg_delivery_days": 1, "on_time_rate_pct": 95},
                {"name": "网上批发", "price_per_kg_fen": 3800, "quality_score": 70,
                 "avg_delivery_days": 3, "on_time_rate_pct": 80},
            ],
        })
        assert result.success
    ...
    async def test_check_compliance(self, agent):
        result = await agent.execute("check_compliance", {
            "ingredient": "辣椒酱",
            "certifications": ["SC"],
            "additives": ["山梨酸钾"],
            "origin": "贵州",
...
class TestMenuAdvisorAgent:
    ...
    async def test_analyze_dish_quadrant(self, agent):
        result = await agent.execute("analyze_dish_quadrant", {
            "dishes": [
    ...
    async def test_diagnose_menu_structure(self, agent):
        result = await agent.execute("diagnose_menu_structure", {
            "total_dishes": 120,
            "categories": [
...
class TestPilotRecommenderAgent:
    ...
    async def test_recommend_pilot_stores(self, agent):
        result = await agent.execute("recommend_pilot_stores", {
            "stores": [
                {"store_id": "s1", "store_name": "五一广场店", "city": "长沙", "district": "芙蓉区",
                 "daily_traffic": 300, "avg_rating": 4.8, "member_pct": 60, "manager_score": 90,
                 "has_full_equipment": True, "is_representative": True},
                {"store_id": "s2", "store_name": "河西店", "city": "长沙", "district": "岳麓区",
                 "daily_traffic": 150, "avg_rating": 4.2, "member_pct": 30, "manager_score": 70,
                 "has_full_equipment": True, "is_representative": False},
                {"store_id": "s3", "store_name": "星沙店", "city": "长沙", "district": "长沙县",
                 "daily_traffic": 100, "avg_rating": 3.8, "member_pct": 20, "manager_score": 50,
                 "has_full_equipment": False, "is_representative": False},
            ],
            "project_type": "新品试销",
            "pilot_count": 2,
        })
    ...
    async def test_cluster_stores(self, agent):
        result = await agent.execute("cluster_stores", {
            "stores": [
...
class TestIntelReporterAgent:
    ...
    async def test_generate_competitor_weekly(self, agent):
        result = await agent.execute("generate_competitor_weekly", {
            "week": "2026-W13",
    ...
    async def test_generate_monthly_report(self, agent):
        result = await agent.execute("generate_monthly_report", {
            "month": "2026年3月",
...
class TestAgentMetadata:
    ...
    @pytest.mark.parametrize("agent_cls,expected_id,expected_priority", [
    ...
    @pytest.mark.parametrize("agent_cls", [
        NewCustomerConvertAgent, DormantRecallAgent, BanquetGrowthAgent, SeasonalCampaignAgent,
        ReferralGrowthAgent, HighValueMemberAgent, OffPeakTrafficAgent, ContentGenerationAgent,
        CompetitorWatchAgent, ReviewInsightAgent, TrendDiscoveryAgent, NewProductScoutAgent,
        IngredientRadarAgent, MenuAdvisorAgent, PilotRecommenderAgent, IntelReporterAgent,
    ])
    @pytest.mark.asyncio
