            ...
            agent.execute(
                "daily_summary",
                {
                    ...
                    "metrics": {
                        ...
                        "margin_rate": 0.33,
                        "turnover_rate": 3.0,
                        "food_cost_fen": 2000000,
                        "labor_cost_fen": 1000000,
                        "waste_fen": 50000,
                        "complaints": 0,
                        "top_dishes": ["水煮鱼", "宫保鸡丁"],
                    },
...
class TestWeeklyPattern:
    def test_detects_weekend_uplift(self):
        ...
        # 应该检测到周末营收高于工作日
        weekend_pattern = next((p for p in result.data["patterns"] if p["type"] == "weekday_vs_weekend"), None)
    ...
    def test_insufficient_data_returns_error(self):
        ...
        result = asyncio.run(
            agent.execute(
                ...
                {
                    "store_id": "S001",
                    "daily_data": [{"revenue_fen": 5000000}] * 3,
                },
            )
        )
        assert not result.success
