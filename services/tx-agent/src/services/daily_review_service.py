...
class DailyReviewService:
    ...
    def get_today_state(cls, tenant_id: str, store_id: str) -> DailyReviewState:
        ...
        if key not in cls._states:
            ...
            cls._states[key] = DailyReviewState(
                date=today, store_id=store_id, tenant_id=tenant_id, nodes=nodes
            )
    ...
    @classmethod
    def get_multi_store_summary(
        cls, tenant_id: str, store_ids: list[str]
    ) -> list[dict]:
        """多门店日清汇总（用于总部驾驶舱）"""
        today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
        result = []
        for store_id in store_ids:
            ...
            if key in cls._states:
                ...
                result.append({
                    ...
                    "completed_count": sum(
                        1 for n in state.nodes if n.status == NodeStatus.COMPLETED
                    ),
