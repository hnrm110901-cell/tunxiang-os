...
class KnowledgeRetrievalService:
    ...
    async def index_document(
        ...
        if is_enabled(KnowledgeFlags.HYBRID_SEARCH_V2):
            asyncio.create_task(_index_to_pgvector(collection, doc_id, text, metadata, tenant_id))
    ...
    return [
        {
            ...
            "metadata": {k: v for k, v in hit.get("payload", {}).items() if k not in ("tenant_id", "text", "doc_id")},
        }
        for hit in results
    ]
        ...
        formatted.append(
            {
                ...
                "text": item.get("text", ""),
                "metadata": {
                    k: v for k, v in item.items() if k not in ("doc_id", "text", "score", "rerank_score", "tenant_id")
                },
            }
        )
    return formatted
