...
class DailyPlannerAgent:
    ...
    def _generate_menu_suggestions(self, inventory: dict, traffic: dict, history: dict) -> list:
        ...
        for item in inventory.get("surplus", []):
            suggestions.append(
                {
                    "dish_id": item["id"],
                    "dish_name": f"{item['name']}系列",
                    "action": "push",
                    "reason": f"{item['name']}库存充足+{item['surplus_pct']}%",
                    "confidence": 0.88,
                    "expected_impact_fen": 80000,
                }
            )
        for item in inventory.get("shortage", []):
            suggestions.append(
                {
                    "dish_id": item["id"],
                    "dish_name": f"{item['name']}相关",
                    "action": "reduce",
                    "reason": f"{item['name']}库存仅够{item['days_left']}天",
                    "confidence": 0.92,
                    "expected_impact_fen": 0,
                }
            )
        return suggestions
    ...
    def _generate_marketing_triggers(self, history: dict) -> list:
        ...
        if inactive > 20:
            triggers.append(
                {
                    "target": "S2_inactive_3d",
                    "action": "send_coupon",
                    "content": "午餐8折优惠券",
                    "target_count": inactive,
            ...
            triggers.append(
                {
                    ...
                    "action": "send_blessing",
                    "content": "生日祝福+赠菜券",
                    "target_count": birthday,
