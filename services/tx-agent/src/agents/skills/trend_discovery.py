...
# 餐饮搜索趋势品类
FOOD_CATEGORIES = [
    "火锅", "烧烤", "湘菜", "川菜", "粤菜", "日料", "西餐", "面食",
    "奶茶", "咖啡", "小龙虾", "螺蛳粉", "预制菜", "轻食沙拉", "露营烧烤",
]
...
class TrendDiscoveryAgent(SkillAgent):
    ...
    async def _analyze_hot_words(self, params: dict) -> AgentResult:
        ...
        return AgentResult(
            success=True, action="analyze_search_hot_words",
            data={
                "hot_words": hot_words,
                "platform": platform,
                "period": period,
    ...
    async def _detect_trend_change(self, params: dict) -> AgentResult:
        ...
        dropped = [{"keyword": t.get("keyword"), "change_type": "下榜", "previous_rank": t.get("rank")}
                  for t in previous_trends if t.get("keyword") not in current_keywords]

        return AgentResult(
            success=True, action="detect_trend_change",
            data={
    ...
    async def _discover_demand(self, params: dict) -> AgentResult:
        ...
        for text in all_text:
            for word in flavor_words:
                if word in text and word not in [d.get("signal") for d in demand_signals["flavor_demand"]]:
                    demand_signals["flavor_demand"].append({"signal": word, "mentions": sum(1 for t in all_text if word in t)})
            for word in scene_words:
                if word in text and word not in [d.get("signal") for d in demand_signals["scene_demand"]]:
                    demand_signals["scene_demand"].append({"signal": word, "mentions": sum(1 for t in all_text if word in t)})
            for word in price_words:
                if word in text and word not in [d.get("signal") for d in demand_signals["price_demand"]]:
                    demand_signals["price_demand"].append({"signal": word, "mentions": sum(1 for t in all_text if word in t)})
            for word in health_words:
                if word in text and word not in [d.get("signal") for d in demand_signals["health_demand"]]:
                    demand_signals["health_demand"].append({"signal": word, "mentions": sum(1 for t in all_text if word in t)})
        ...
        return AgentResult(
            success=True, action="discover_consumer_demand",
            data={
    ...
    async def _compare_regional(self, params: dict) -> AgentResult:
        ...
        return AgentResult(
            success=True, action="compare_regional_heat",
            data={
                "category": category,
                "regions": comparisons,
    ...
    async def _predict_category(self, params: dict) -> AgentResult:
        ...
        for cat in categories:
            ...
            predictions.append({
                ...
                "recommendation": "重点布局" if momentum > 15 else "持续关注" if momentum > 5 else "谨慎投入" if momentum > -5 else "收缩调整",
            })

        predictions.sort(key=lambda x: x["predicted_next_quarter_growth"], reverse=True)

        return AgentResult(
            success=True, action="predict_category_trend",
            data={
    ...
    async def _correlate_events(self, params: dict) -> AgentResult:
        ...
        return AgentResult(
            success=True, action="correlate_hot_events",
            data={
