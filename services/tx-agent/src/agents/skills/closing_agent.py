...
class ClosingAgent(SkillAgent):
    ...
    async def _pre_closing_check(self, params: dict) -> AgentResult:
        ...
        return AgentResult(
            success=True, action="pre_closing_check",
            data={
                "can_close": can_close,
                "status": status,
                "blockers": blockers,
    ...
    async def _validate_settlement(self, params: dict) -> AgentResult:
        ...
        return AgentResult(
            success=True, action="validate_daily_settlement",
            data={
                "passed": passed,
    ...
    async def _remind_unsettled(self, params: dict) -> AgentResult:
        ...
        if not unsettled:
            return AgentResult(
                success=True, action="remind_unsettled_orders",
                data={"count": 0, "message": "无未结单"},
                reasoning="所有订单已结算", confidence=1.0, inference_layer="edge",
            )
    ...
    async def _check_checklist(self, params: dict) -> AgentResult:
        ...
        return AgentResult(
            success=True, action="check_checklist_status",
            data={
                "type": checklist_type,
                "progress": progress,
                "total": total_items,
    ...
    async def _generate_report(self, params: dict) -> AgentResult:
        ...
        return AgentResult(
            success=True, action="generate_closing_report",
            data={
    ...
    async def _escalate_anomaly(self, params: dict) -> AgentResult:
        ...
        logger.warning("closing_anomaly_escalated",
                       store=store_name, type=anomaly_type, detail=detail)

        return AgentResult(
            success=True, action="escalate_anomaly",
            data={
                "escalated": True,
