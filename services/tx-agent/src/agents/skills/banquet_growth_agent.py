...
class BanquetGrowthAgent:
    ...
    async def forecast_demand(self, store_id: str, target_month: str) -> dict:
        ...
        for etype in ["wedding", "birthday", "business", "tour_group", "conference", "annual_party"]:
            ...
            forecasts.append(
                {"event_type": etype, "predicted_count": pred_count, "predicted_revenue_fen": pred_revenue}
            )
            # 写入预测表
            ...
            """),
                {
                    ...
                    "cnt": pred_count,
                    "rev": pred_revenue,
                    "factors": json.dumps({"growth_factor": growth_factor, "history_count": h["count"]}),
                },
            )
        await self.db.flush()
            ...
            opportunities.append(
                {
                    ...
                    "last_event_date": r["event_date"].isoformat() if r["event_date"] else None,
                    "last_amount_fen": r["total_amount_fen"],
                    "opportunity_type": "anniversary",
                    "message": f"{r['host_name']}去年{r['event_type']}客户，建议联系复购",
                }
            )
        ...
        return [
            {
                ...
                "days_since": (datetime.now(timezone.utc) - r["last_inquiry"]).days if r["last_inquiry"] else None,
            }
            for r in rows.mappings().all()
        ]
