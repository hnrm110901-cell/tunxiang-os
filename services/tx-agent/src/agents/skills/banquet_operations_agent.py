...
class BanquetOperationsAgent:
    ...
    async def pre_event_check(self, banquet_id: str) -> dict:
        ...
        plan = await self.db.execute(
            text(
                "SELECT status FROM banquet_production_plans WHERE banquet_id = :bid AND tenant_id = :tid AND is_deleted = FALSE"
            ),
        ...
        checks.append(
            {
                "item": "排产计划",
                "status": "pass" if p == "confirmed" else "fail",
                "detail": f"当前状态: {p or '未创建'}",
            }
        )
        # 2. 原料采购到位
        mat = await self.db.execute(
            text(
                "SELECT COUNT(*) AS total, SUM(CASE WHEN status IN ('received','fulfilled') THEN 1 ELSE 0 END) AS ready FROM banquet_material_requirements WHERE banquet_id = :bid AND tenant_id = :tid AND is_deleted = FALSE"
            ),
            {"bid": banquet_id, "tid": self.tenant_id},
        )
        ...
        checks.append(
            {
                "item": "原料到位",
                "status": "pass" if ready_mat >= total_mat else "warning",
                "detail": f"{ready_mat}/{total_mat}项已到位",
            }
        )
        # 3. 合同签署
        contract = await self.db.execute(
            text(
                "SELECT status FROM banquet_contracts WHERE banquet_id = :bid AND tenant_id = :tid AND is_deleted = FALSE"
            ),
            {"bid": banquet_id, "tid": self.tenant_id},
        )
        c = contract.scalar_one_or_none()
        checks.append(
            {"item": "合同签署", "status": "pass" if c == "signed" else "fail", "detail": f"当前状态: {c or '未创建'}"}
        )
        # 4. 执行SOP
        exec_plan = await self.db.execute(
            text(
                "SELECT status FROM banquet_execution_plans WHERE banquet_id = :bid AND tenant_id = :tid AND is_deleted = FALSE"
            ),
            {"bid": banquet_id, "tid": self.tenant_id},
        )
        e = exec_plan.scalar_one_or_none()
        checks.append(
            {"item": "执行计划", "status": "pass" if e else "warning", "detail": f"{'已创建' if e else '未创建'}"}
        )
        ...
        """),
            {
                ...
                "tid": self.tenant_id,
                "bid": banquet_id,
                "rec": json.dumps({"overall": overall, "checks": checks}, ensure_ascii=False, default=str),
                "conf": 0.9 if all_pass else 0.6,
            },
        )
        await self.db.flush()
    ...
    async def optimize_daily_schedule(self, store_id: str, target_date: date) -> dict:
        ...
        if len(banquets) > 1:
            ...
            if total_tables > 50:
                suggestions.append(
                    {"type": "critical", "message": f"当日共{total_tables}桌，建议启用所有厅房并增配传菜员"}
                )
        if not suggestions:
