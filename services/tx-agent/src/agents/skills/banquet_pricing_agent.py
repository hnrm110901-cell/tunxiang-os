...
SEASON_FACTORS = {
    1: 1.15, 2: 1.20, 3: 1.0, 4: 1.0, 5: 1.10, 6: 1.05,
    7: 0.95, 8: 0.95, 9: 1.05, 10: 1.20, 11: 1.10, 12: 1.25,
}
EVENT_MARGIN_TARGET = {
    "wedding": 0.45, "birthday": 0.40, "business": 0.50,
    "tour_group": 0.30, "conference": 0.35, "annual_party": 0.45,
}
...
class BanquetPricingAgent:
    ...
    async def suggest_price(self, store_id: str, event_type: str, table_count: int, guest_count: int, event_month: int = None) -> dict:
        """基于历史数据推荐报价"""
        if not event_month:
        ...
        if history_count >= 3:
            ...
            base_per_table = {"wedding": 128800, "birthday": 98800, "business": 158800, "tour_group": 58800, "conference": 48800, "annual_party": 118800}.get(event_type, 98800)
    ...
    async def suggest_menu(self, event_type: str, tier: str, budget_per_table_fen: int) -> dict:
        ...
        return {"budget_per_table_fen": budget_per_table_fen, "recommended_templates": templates, "best_match": templates[0] if templates else None}
