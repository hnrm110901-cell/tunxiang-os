...
## Stamp 表
`gateway_alembic_version` — 与其他 service 的 alembic_version 表互不冲突。

## 起手新 migration

```bash
cd services/gateway/db-migrations
...
DATABASE_URL=postgresql://... \
  alembic -c services/gateway/db-migrations/alembic.ini \
  upgrade head
```
...
- 仅放 gateway 拥有的表（per `docs/migration-architecture-route-a-ownership-audit.md`）
