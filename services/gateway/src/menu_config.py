...
DEFAULT_MODULES = [
    MenuModule(
        ...
        groups=[
            MenuGroup(
                ...
                items=[
                    MenuItem(
                        id="hq-dashboard", label="经营驾驶舱", icon="📊", path="/dashboard", roles=["admin", "manager"]
                    ),
                    MenuItem(
                        id="store-health", label="门店健康", icon="🏥", path="/store-health", roles=["admin", "manager"]
                    ),
        ...
        groups=[
            MenuGroup(
                ...
                items=[
                    ...
                    MenuItem(
                        id="ranking", label="菜单排名", icon="🏆", path="/menu/ranking", roles=["admin", "manager"]
                    ),
                ],
            ),
        ],
    ...
    MenuModule(
        ...
        label="财务",
        priority=5,
        requires_domain="tx-finance",
        groups=[
            MenuGroup(
                label="财务",
                items=[
                    MenuItem(
                        id="daily-profit", label="日利润", icon="💰", path="/finance/daily", roles=["admin", "manager"]
                    ),
                    MenuItem(
                        id="cost-rate", label="成本率", icon="📉", path="/finance/cost-rate", roles=["admin", "manager"]
                    ),
    ...
    MenuModule(
        ...
        label="组织",
        priority=6,
        requires_domain="tx-org",
        groups=[
            MenuGroup(
                label="人力",
                items=[
                    MenuItem(
                        id="employees", label="员工管理", icon="👥", path="/org/employees", roles=["admin", "manager"]
                    ),
                    MenuItem(id="schedule", label="排班", icon="📅", path="/org/schedule", roles=["admin", "manager"]),
                    MenuItem(
                        id="attendance", label="考勤", icon="⏰", path="/org/attendance", roles=["admin", "manager"]
                    ),
                ],
            )
        ],
