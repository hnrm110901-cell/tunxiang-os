# tx-malaysia alembic migrations

service 独立 alembic — Phase 4a 路线 a (per-service migrations)。

## Stamp 表
`tx_malaysia_alembic_version` — 与其他 service 的 alembic_version 表互不冲突。

## 起手新 migration

```bash
cd services/tx-malaysia/db-migrations
...
DATABASE_URL=postgresql://... \
  alembic -c services/tx-malaysia/db-migrations/alembic.ini \
  upgrade head
```
...
- 仅放 tx-malaysia 拥有的表（per `docs/migration-architecture-route-a-ownership-audit.md`）
