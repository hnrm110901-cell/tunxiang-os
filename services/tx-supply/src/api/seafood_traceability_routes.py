...
async def batch_receive(
    ...
    """), {
        ...
        "sup_id": body.supplier_id, "sup_name": body.supplier_name,
        "species": body.species, "batch_no": body.batch_no,
        "qty": body.quantity, "wg": body.weight_g,
        "price": body.unit_price_fen, "cost": total_cost, "origin": body.origin,
    })
    row = result.mappings().first()
    await db.commit()
    return {"ok": True, "data": {"id": str(row["id"]), "batch_no": body.batch_no,
            "total_cost_fen": total_cost}}


@router.get("/batches", summary="批次列表")
async def list_batches(
    store_id: Optional[str] = None, species: Optional[str] = None,
    ...
    if store_id:
        wheres.append("store_id = :sid::UUID"); params["sid"] = store_id
    if species:
        wheres.append("species ILIKE :sp"); params["sp"] = f"%{species}%"
    if status:
        wheres.append("status = :st"); params["st"] = status
    w = " AND ".join(wheres)
...
async def batch_trace(
    ...
    return {"ok": True, "data": {
        "batch": {k: (str(v) if isinstance(v, uuid.UUID) else v.isoformat() if isinstance(v, (datetime, date)) else v)
...
async def record_mortality(
    ...
    """), {
        "tid": tenant_id, "sid": body.store_id,
        "bid": body.batch_id, "tkid": body.tank_id,
        "sp": body.species, "dq": body.dead_qty, "dw": body.dead_weight_g,
        "cause": body.cause, "notes": body.notes,
    })
    row = result.mappings().first()
    ...
    await db.commit()
    return {"ok": True, "data": {"id": str(row["id"]), "species": body.species,
            "dead_qty": body.dead_qty}}
...
async def loss_report(
    ...
    return {"ok": True, "data": {"items": items, "total_dead_qty": total_dead,
