...
class ProductionPlanORM(TenantBase):
    ...
    kitchen_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), nullable=False, comment="中央厨房 ID"
    )
    ...
    total_items: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, comment="汇总食材种类数"
    )
    created_by: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), nullable=True, comment="创建人 ID"
    )

    # relationships
    tasks: Mapped[list[ProductionTaskORM]] = relationship(
        "ProductionTaskORM", back_populates="plan", lazy="selectin",
    )
    trips: Mapped[list[DeliveryTripORM]] = relationship(
        "DeliveryTripORM", back_populates="plan", lazy="selectin",
    )
...
class ProductionTaskORM(TenantBase):
    ...
    ingredient_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), nullable=False, comment="食材 ID"
    )
    planned_qty: Mapped[float] = mapped_column(
        Numeric(10, 3), nullable=False, comment="计划加工量"
    )
    unit: Mapped[str] = mapped_column(
        String(20), nullable=False, comment="单位"
    )
    assigned_station: Mapped[str | None] = mapped_column(
        String(50), nullable=True, comment="加工档口"
    )
    status: Mapped[str] = mapped_column(
    ...
    actual_qty: Mapped[float | None] = mapped_column(
        Numeric(10, 3), nullable=True, comment="实际产量"
    )
    ...
    plan: Mapped[ProductionPlanORM] = relationship(
        "ProductionPlanORM", back_populates="tasks",
    )

    __table_args__ = (
        Index("ix_production_tasks_tenant_plan", "tenant_id", "plan_id"),
    )
...
class DeliveryTripORM(TenantBase):
    ...
    trip_no: Mapped[str] = mapped_column(
        String(30), nullable=False, comment="配送单号 TRP-YYMMDD-NN"
    )
    ...
    # relationships
    plan: Mapped[ProductionPlanORM] = relationship(
        "ProductionPlanORM", back_populates="trips",
    )
    items: Mapped[list[DeliveryItemORM]] = relationship(
        "DeliveryItemORM", back_populates="trip", lazy="selectin",
    )

    __table_args__ = (
        Index("ix_delivery_trips_tenant_plan", "tenant_id", "plan_id"),
    )
...
class DeliveryItemORM(TenantBase):
    ...
    planned_qty: Mapped[float] = mapped_column(
        Numeric(10, 3), nullable=False, comment="计划配送量"
    )
    received_qty: Mapped[float | None] = mapped_column(
        Numeric(10, 3), nullable=True, comment="实收量"
    )
    variance_qty: Mapped[float | None] = mapped_column(
        Numeric(10, 3), nullable=True, comment="差异量"
    )
    ...
    trip: Mapped[DeliveryTripORM] = relationship(
        "DeliveryTripORM", back_populates="items",
    )
