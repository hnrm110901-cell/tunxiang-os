"""Tier 1 — tx-supply main.py 容器布局 import 烟测

防止 production main.py 启动失败的回归门禁。复用 shared/test_infra/main_import_smoke.py
helper（构造 mktemp 容器布局 + subprocess 隔离）。

为什么 Tier 1：services/tx-supply/src/main.py 是 production 入口
（Dockerfile CMD: uvicorn services.tx_supply.src.main:app），main.py import 失败 = 服务无法启动。
本烟测捕捉所有 main.py 内 absolute import 解析问题（容器路径 PYTHONPATH=/app 真实模拟）。

**当前 xfail**：本服务 main.py 在容器布局下存在已知 import 失败
（root cause: missing-export-Header-from-shared.ontology.src.database）。决策 77 production 端 codemod / 缺失导出修复后
...
@pytest.mark.xfail(
    reason="missing-export-Header-from-shared.ontology.src.database；决策 77 production 端 codemod 修复后翻 xfail",
    strict=False,
)
def test_main_module_loads_in_container_layout() -> None:
    """tx_supply main.py 必须在 Docker 容器布局下干净 import。"""
