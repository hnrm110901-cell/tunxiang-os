...
def _setup_store_demands():
    """注入三个门店的原料需求"""
    inject_store_demand(STORE_A, TENANT, [
        {"ingredient_id": "ing-pork", "ingredient_name": "五花肉", "quantity": 20.0, "unit": "kg", "category": "meat"},
        {"ingredient_id": "ing-lettuce", "ingredient_name": "生菜", "quantity": 10.0, "unit": "kg", "category": "vegetable"},
    ])
    inject_store_demand(STORE_B, TENANT, [
        {"ingredient_id": "ing-pork", "ingredient_name": "五花肉", "quantity": 15.0, "unit": "kg", "category": "meat"},
        {"ingredient_id": "ing-rice", "ingredient_name": "大米", "quantity": 30.0, "unit": "kg", "category": "grain"},
    ])
    inject_store_demand(STORE_C, TENANT, [
        {"ingredient_id": "ing-lettuce", "ingredient_name": "生菜", "quantity": 8.0, "unit": "kg", "category": "vegetable"},
        {"ingredient_id": "ing-rice", "ingredient_name": "大米", "quantity": 20.0, "unit": "kg", "category": "grain"},
    ])


def _setup_store_geo():
    """注入门店地理信息"""
    inject_store_geo(STORE_A, TENANT, {
        "store_name": "五一广场店",
        "lat": 28.1978,
        "lng": 112.9762,
        "address": "长沙市五一广场",
    ...
    inject_store_geo(STORE_B, TENANT, {
        "store_name": "橘子洲店",
        "lat": 28.2104,
        "lng": 112.9539,
        "address": "长沙市橘子洲",
    ...
    inject_store_geo(STORE_C, TENANT, {
        "store_name": "星沙店",
        "lat": 28.2455,
        "lng": 113.0802,
        "address": "长沙县星沙",
...
class TestConfirmAndCompleteTasks:
    ...
    async def test_complete_task(self):
        ...
        result = await svc.complete_task(
            task_id=task_id, actual_qty=19.5, tenant_id=TENANT
        )
    ...
    async def test_complete_task_negative_qty_raises(self):
        ...
        with pytest.raises(ValueError, match="负数"):
            await svc.complete_task(
                task_id=task_id, actual_qty=-1.0, tenant_id=TENANT
            )
...
class TestGenerateDeliveryTrips:
    ...
    async def _make_confirmed_plan(self):
        ...
        for task in plan["tasks"]:
            await svc.complete_task(
                task_id=task["id"], actual_qty=task["planned_qty"], tenant_id=TENANT
            )
        return plan
    ...
    async def test_generate_trips_returns_list(self):
        ...
        trips = await svc.generate_delivery_trips(
            plan_id=plan["id"], tenant_id=TENANT
        )
    ...
    async def test_trips_wrong_tenant_raises(self):
        ...
        svc = ProductionPlanService()
        with pytest.raises(ValueError):
            await svc.generate_delivery_trips(
                plan_id=plan["id"], tenant_id=OTHER_TENANT
            )
    ...
    async def test_trips_nonexistent_plan_raises(self):
        ...
        with pytest.raises(ValueError):
            await svc.generate_delivery_trips(
                plan_id="nonexistent-plan", tenant_id=TENANT
            )
...
class TestRouteOptimization:
    ...
    def test_cluster_stores_by_region(self):
        ...
        groups = route_svc.cluster_stores_by_region(
            [STORE_A, STORE_B, STORE_C], TENANT
        )
    ...
    def test_cluster_respects_max_per_trip(self):
        ...
        groups = route_svc.cluster_stores_by_region(
            many_stores, TENANT, max_per_trip=3
        )
        for group in groups:
            assert len(group) <= 3
...
class TestSignReceipt:
    ...
    async def _make_trip_with_items(self):
        ...
        for task in plan["tasks"]:
            await plan_svc.complete_task(
                task_id=task["id"], actual_qty=task["planned_qty"], tenant_id=TENANT
            )
        trips = await plan_svc.generate_delivery_trips(
            plan_id=plan["id"], tenant_id=TENANT
        )
        return plan, trips
...
class TestUpdateStoreInventory:
    ...
    async def test_update_inventory_returns_records(self):
        ...
        route_svc = DeliveryRouteService()
        result = await route_svc.update_store_inventory(
            trip_id=trip["id"], tenant_id=TENANT
        )
    ...
    async def test_update_inventory_unsigned_items_raises(self):
        ...
        with pytest.raises(ValueError, match="未签收"):
            await route_svc.update_store_inventory(
                trip_id=trips[0]["id"], tenant_id=TENANT
            )
...
class TestVarianceReport:
    ...
    async def test_variance_report_basic(self):
        ...
        report = await plan_svc.get_variance_report(
            plan_id=plan["id"], tenant_id=TENANT
        )
    ...
    async def test_variance_report_wrong_plan_raises(self):
        plan_svc = ProductionPlanService()
        with pytest.raises(ValueError):
            await plan_svc.get_variance_report(
                plan_id="nonexistent", tenant_id=TENANT
            )
...
class TestTenantIsolation:
    ...
    async def test_complete_task_wrong_tenant_raises(self):
        ...
        with pytest.raises(ValueError):
            await svc.complete_task(
                task_id=task_id, actual_qty=10.0, tenant_id=OTHER_TENANT
            )
