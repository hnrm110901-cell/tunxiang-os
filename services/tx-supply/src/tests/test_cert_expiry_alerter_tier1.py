...
测试基于真实餐厅场景（CLAUDE.md §20）：
  ...
  8. tenant 无 webhook URL 时跳过该 channel 并 log warn（不 raise）
     GIVEN  tenant.systems_config 无 safety_director_webhook（issue #708 修法 B）
...
def _mk_db(
    *,
    ...
    async def execute_side_effect(query, params=None):
        ...
        # tenants webhook 查询（issue #708 修法 B: extra_data → systems_config）
        if "tenants" in sql and "systems_config" in sql:
            row_mock = MagicMock()
...
class TestIssue708SystemsConfigSqlPathRegression:
    """Issue #708 修法 B 回归 — 防 silent OFF 再次回归。

    Issue #708 根因：tenants 表 v006 无 extra_data 列 (v232 才加 systems_config)，
    旧代码 `extra_data->>'safety_director_webhook'` 触 SQL ProgrammingError，
    外层 except Exception 兜底 fail-open → IM 推送 silent OFF 自 PR #608
    (2026-05-14) 起 ~2 天未被发现。

    本类用 inspect.getsource 静态扫源码，确保读路径只走 systems_config，
    不会回归 extra_data。任何未来 refactor 把读路径改回 extra_data 都会
    被本测试集 fail-fast 阻止。
    """

    def test_get_tenant_webhook_urls_reads_systems_config(self):
        """读路径必须用 systems_config（v232 JSONB）作 source。"""
        import inspect
        from services.tx_supply.src.workers.cert_expiry_alerter import (
            _get_tenant_webhook_urls,
        )

        source = inspect.getsource(_get_tenant_webhook_urls)
        assert "systems_config->>'safety_director_webhook'" in source, (
            "issue #708 修法 B：必须用 systems_config->>'safety_director_webhook'，"
            "不能用 extra_data（v006 schema 无此列）"
        )
        assert "systems_config->>'purchaser_webhook'" in source, (
            "issue #708 修法 B：必须用 systems_config->>'purchaser_webhook'，"
            "不能用 extra_data"
        )

    def test_get_tenant_webhook_urls_does_not_reference_extra_data(self):
        """读路径禁止再回归到 extra_data（v006 tenants schema 无此列）。"""
        import inspect
        from services.tx_supply.src.workers.cert_expiry_alerter import (
            _get_tenant_webhook_urls,
        )
        ...
        source = inspect.getsource(_get_tenant_webhook_urls)
        # 不允许 SELECT extra_data->>'...' 形态（docstring 中 #708 引用允许，
        # 用空格 + ->> 双 token 精确匹配 SQL 投影列表达式）
        assert "extra_data->>'safety_director_webhook'" not in source, (
            "issue #708 修法 B 回归：检测到 extra_data 读路径回归，"
            "应改用 systems_config (v232 已有)"
        )
        assert "extra_data->>'purchaser_webhook'" not in source, (
            "issue #708 修法 B 回归：检测到 extra_data 读路径回归，"
            "应改用 systems_config"
        )

    @pytest.mark.asyncio
    async def test_im_channel_returns_real_webhooks_when_configured(self):
        """端到端契约：systems_config 有 webhook → _get_tenant_webhook_urls 返回真值。

        防 silent OFF 回归：如果 SQL 投影改回不存在的 extra_data 列，
        本 mock 不识别 → row=None → 返回 {None, None} → 推送跳过。
        新读路径必须命中 mock 的 systems_config 分支。
        """
        from services.tx_supply.src.workers.cert_expiry_alerter import (
            _get_tenant_webhook_urls,
        )
        ...
        async def execute_side_effect(query, params=None):
            sql = str(query)
            result = MagicMock()
            # 只匹配 systems_config 投影（修法 B 后路径）
            if "tenants" in sql and "systems_config->>'safety_director_webhook'" in sql:
                row_mock = MagicMock()
                row_mock.mappings.return_value.first.return_value = {
                    "safety_director_webhook": "https://qyapi.weixin.qq.com/safety/real",
                    "purchaser_webhook": "https://qyapi.weixin.qq.com/purchaser/real",
                }
                return row_mock
            return result
        ...
        result = await _get_tenant_webhook_urls(conn, _TENANT_XUJI)
        assert result == {
            "safety_director_webhook": "https://qyapi.weixin.qq.com/safety/real",
            "purchaser_webhook": "https://qyapi.weixin.qq.com/purchaser/real",
        }, (
            f"issue #708 修法 B silent OFF 回归：systems_config 配置真值后未正确读出 "
            f"(实际 {result})"
        )

    @pytest.mark.asyncio
    async def test_im_channel_fails_open_when_systems_config_missing(self):
        """端到端契约：tenant 无 systems_config 配置 → 返回 {None, None}（fail-open 跳过该 channel）。

        这是预期 fail-open 行为：JSONB ->> 不存在 key 返回 NULL → 跳过推送 + log warn，
        不应 raise 让整次 alerter 崩。
        """
        from services.tx_supply.src.workers.cert_expiry_alerter import (
            _get_tenant_webhook_urls,
        )
        ...
        async def execute_side_effect(query, params=None):
            ...
            if "tenants" in sql and "systems_config" in sql:
                row_mock = MagicMock()
                # PG JSONB ->> 不存在 key 返回 None
                row_mock.mappings.return_value.first.return_value = {
                    "safety_director_webhook": None,
                    "purchaser_webhook": None,
                }
                return row_mock
            return result
