"""tx-sync-worker · 品智POS 自动数据同步任务 (W2 P1 issue #758).

**业务函数 0 diff** copy from services/gateway/src/sync_scheduler.py:1-577
(Phase 1 双轨并行, gateway scheduler 仍跑; Phase 2 follow-up 关 gateway 切单轨).

调度计划（Asia/Shanghai）— 与 gateway 完全一致 (Q3 决议 A):
...
失败重试 3 次，重试间隔 5 分钟（指数退避）。
同步结果写入 sync_logs 表（RLS via set_config('app.tenant_id')）。

**dry_run 模式 (Q3 决议 A)**:
  RUN_MODE=dry_run (env unset 默认 true) → cron fire 时只 log + metric, 不调 pinzhi adapter
...
def _is_dry_run() -> bool:
    """Phase 1 dry_run 模式开关.

    默认 true (env unset = dry_run); Phase 2 follow-up 翻 RUN_MODE=live 切单轨.
    严格小写比较, "live" 才走真路径; 其他任何值 (包括拼写错误) 都 fall-back dry_run.
    """
...
async def _with_retry(coro_factory: Any, sync_type: str, merchant_code: str) -> dict:
    ...
    for attempt in range(1, RETRY_TIMES + 1):
        ...
        if attempt < RETRY_TIMES:
            ...
            sync_retry_total.labels(job=sync_type, attempt=str(attempt)).inc()
...
def _record_dry_run(job_id: str, duration_s: float) -> None:
    ...
    logger.info(
        "dry_run_skip",
        job=job_id,
        action="would_call_pinzhi_adapter",
...
async def _run_dishes_sync() -> None:
    ...
    sync_executions_total.labels(job="daily_dishes_sync", status="success").inc()
    sync_last_run_timestamp_seconds.labels(job="daily_dishes_sync").set(time.time())
    sync_duration_seconds.labels(job="daily_dishes_sync").observe(duration_s)
    log.info("daily_dishes_sync_finished", duration_s=duration_s)
...
async def _run_master_data_sync() -> None:
    ...
    await asyncio.gather(*tasks, return_exceptions=True)
    duration_s = time.monotonic() - fire_start
    sync_executions_total.labels(job="daily_master_data_sync", status="success").inc()
    sync_last_run_timestamp_seconds.labels(job="daily_master_data_sync").set(time.time())
    sync_duration_seconds.labels(job="daily_master_data_sync").observe(duration_s)
    log.info("daily_master_data_sync_finished", duration_s=duration_s)
...
async def _run_orders_incremental_sync() -> None:
    ...
    duration_s = time.monotonic() - fire_start
    sync_executions_total.labels(job="hourly_orders_incremental_sync", status="success").inc()
    sync_last_run_timestamp_seconds.labels(job="hourly_orders_incremental_sync").set(time.time())
    sync_duration_seconds.labels(job="hourly_orders_incremental_sync").observe(duration_s)
    log.info("hourly_orders_incremental_sync_finished", duration_s=duration_s)
...
async def _run_members_incremental_sync() -> None:
    ...
    duration_s = time.monotonic() - fire_start
    sync_executions_total.labels(job="quarter_members_incremental_sync", status="success").inc()
    sync_last_run_timestamp_seconds.labels(job="quarter_members_incremental_sync").set(time.time())
    sync_duration_seconds.labels(job="quarter_members_incremental_sync").observe(duration_s)
    log.info("quarter_members_incremental_sync_finished", duration_s=duration_s)
