...
class Invoice(TenantBase):
    ...
    invoice_type: Mapped[str] = mapped_column(
        String(20), nullable=False, default=InvoiceType.VAT_GENERAL.value, comment="发票类型，参见 InvoiceType 枚举"
    )
    ...
    invoice_date: Mapped[Optional[date]] = mapped_column(Date, nullable=True, comment="开票日期")

    # ── 销售方信息 ────────────────────────────────────────────────────────────
    seller_name: Mapped[Optional[str]] = mapped_column(String(200), nullable=True, comment="销售方名称")
    seller_tax_id: Mapped[Optional[str]] = mapped_column(
        String(20), nullable=True, comment="销售方纳税人识别号（18位统一社会信用代码）"
    )

    # ── 购买方信息 ────────────────────────────────────────────────────────────
    buyer_name: Mapped[Optional[str]] = mapped_column(String(200), nullable=True, comment="购买方名称")
    ...
    total_amount: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True, comment="价税合计，单位：分(fen)，展示时除以100转元"
    )
    tax_amount: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True, comment="税额合计，单位：分(fen)，展示时除以100转元"
    )
    amount_without_tax: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True, comment="不含税金额合计，单位：分(fen)，展示时除以100转元"
    )
    ...
    file_type: Mapped[str] = mapped_column(
        String(50), nullable=False, comment="文件 MIME 类型，如 image/jpeg、application/pdf"
    )
    ...
    ocr_status: Mapped[str] = mapped_column(
        ...
        index=True,
        comment="OCR 识别状态，参见 OcrStatus 枚举",
    )
    ...
    ocr_confidence: Mapped[Optional[Numeric]] = mapped_column(
        Numeric(5, 4), nullable=True, comment="OCR 整体置信度（0-1），低于阈值时触发人工复核"
    )
    ...
    verify_status: Mapped[str] = mapped_column(
        ...
        index=True,
        comment=("金税核验状态，参见 VerifyStatus 枚举。verified_fake 时前端高亮警告，不自动驳回，由审批人人工决策。"),
    )
    ...
    is_duplicate: Mapped[bool] = mapped_column(
        Boolean, default=False, nullable=False, comment="是否为重复发票（dedup_hash 碰撞时标记）"
    )
    ...
    category_confidence: Mapped[Optional[Numeric]] = mapped_column(
        Numeric(5, 4), nullable=True, comment="AI 科目建议置信度（0-1），供前端展示参考"
    )

    # ── 合规信息 ──────────────────────────────────────────────────────────────
    is_within_period: Mapped[Optional[bool]] = mapped_column(
        Boolean, nullable=True, comment="发票日期是否在报销期限内（None=待核查，True/False=已核查结果）"
    )
...
class InvoiceItem(TenantBase):
    ...
    item_name: Mapped[str] = mapped_column(String(200), nullable=False, comment="货物或应税劳务/服务名称")
    item_spec: Mapped[Optional[str]] = mapped_column(String(100), nullable=True, comment="规格型号")
    unit: Mapped[Optional[str]] = mapped_column(String(20), nullable=True, comment="计量单位，如「件」「箱」「次」")
    quantity: Mapped[Optional[Numeric]] = mapped_column(Numeric(14, 4), nullable=True, comment="数量（支持小数）")
    unit_price: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True, comment="含税单价，单位：分(fen)，展示时除以100转元"
    )
    amount: Mapped[int] = mapped_column(Integer, nullable=False, comment="不含税金额，单位：分(fen)，展示时除以100转元")
    tax_rate: Mapped[Optional[Numeric]] = mapped_column(Numeric(5, 4), nullable=True, comment="税率（如0.13表示13%）")
    tax_amount: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True, comment="税额，单位：分(fen)，展示时除以100转元"
    )

    # ── 关系 ──────────────────────────────────────────────────────────────────
    invoice: Mapped["Invoice"] = relationship("Invoice", back_populates="items", foreign_keys=[invoice_id])
