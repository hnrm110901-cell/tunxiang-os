...
class Budget(TenantBase):
    ...
    budget_name: Mapped[str] = mapped_column(
        String(200), nullable=False, comment="预算名称"
    )
    budget_year: Mapped[int] = mapped_column(
        Integer, nullable=False, comment="预算年份，如 2026"
    )
    budget_month: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True,
        comment="预算月份（NULL=年度预算，1-12=月度预算）"
    )
    ...
    approved_by: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True), nullable=True, comment="审批人员工ID"
    )
    notes: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="备注"
    )
    created_by: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True), nullable=True, comment="创建人员工ID"
    )
...
class BudgetAllocation(TenantBase):
    ...
    category_code: Mapped[Optional[str]] = mapped_column(
        ...
        comment="费用科目代码（对应 ExpenseCategoryCode）"
    )
    allocated_amount: Mapped[int] = mapped_column(
        BigInteger, nullable=False, comment="分配金额（分）"
    )
    used_amount: Mapped[int] = mapped_column(
        BigInteger, nullable=False, default=0, server_default="0",
        comment="已使用金额（分）"
    ...
    budget: Mapped["Budget"] = relationship(
        "Budget", back_populates="allocations"
    )
...
class BudgetAdjustment(TenantBase):
    ...
    adjustment_type: Mapped[Optional[str]] = mapped_column(
        ...
        comment="调整类型：increase/decrease/reallocate"
    )
    ...
    # 关系
    budget: Mapped["Budget"] = relationship(
        "Budget", back_populates="adjustments"
    )
...
class BudgetSnapshot(TenantBase):
    ...
    total_amount: Mapped[Optional[int]] = mapped_column(
        BigInteger, nullable=True, comment="快照时预算总额（分）"
    )
    used_amount: Mapped[Optional[int]] = mapped_column(
        BigInteger, nullable=True, comment="快照时已使用金额（分）"
    )
    execution_rate: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(7, 4), nullable=True, comment="执行率，如 0.8567"
    )
    snapshot_data: Mapped[Optional[dict]] = mapped_column(
        JSONB, nullable=True, comment="完整快照 JSON（含分配明细等）"
    )

    # 关系
    budget: Mapped["Budget"] = relationship(
        "Budget", back_populates="snapshots"
    )
