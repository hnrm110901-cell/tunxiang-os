...
from .expense_enums import ApprovalNodeStatus, ApprovalRoutingType
...
class ApprovalRoutingRule(TenantBase):
    ...
    brand_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), nullable=False, index=True, comment="所属品牌ID（路由规则按品牌隔离）"
    )
    ...
    routing_type: Mapped[str] = mapped_column(
        String(30),
        nullable=False,
        default=ApprovalRoutingType.AMOUNT_BASED.value,
        comment="路由类型，参见 ApprovalRoutingType 枚举",
    )
    amount_min: Mapped[int] = mapped_column(Integer, nullable=False, default=0, comment="金额下限（含），单位：分(fen)")
    amount_max: Mapped[int] = mapped_column(
        Integer, nullable=False, default=-1, comment="金额上限（含），单位：分(fen)，-1 表示无上限"
    )
    approver_role: Mapped[str] = mapped_column(
        String(50), nullable=False, comment="审批人角色标识，如 store_manager / region_manager / hq_finance"
    )
    approver_count: Mapped[int] = mapped_column(
        Integer, nullable=False, default=1, comment="需要几位该角色审批人通过（合同类场景用于双签）"
    )
...
class ApprovalInstance(TenantBase):
    ...
    __table_args__ = (UniqueConstraint("application_id", name="uq_approval_instance_application"),)
    ...
    current_node_index: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, comment="当前待处理节点的0-based索引"
    )
    total_nodes: Mapped[int] = mapped_column(
        Integer, nullable=False, comment="审批节点总数（由 routing_snapshot 决定，创建时固化）"
    )
    status: Mapped[str] = mapped_column(
        String(20),
        nullable=False,
        default=ApprovalNodeStatus.PENDING.value,
        index=True,
...
class ApprovalNode(TenantBase):
    ...
    node_index: Mapped[int] = mapped_column(Integer, nullable=False, comment="节点在审批链中的0-based顺序索引")
    approver_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), nullable=False, index=True, comment="审批人员工ID"
    )
    approver_role: Mapped[str] = mapped_column(
        String(50), nullable=False, comment="审批人角色标识，如 store_manager / region_manager / hq_finance"
    )
    ...
    comment: Mapped[Optional[str]] = mapped_column(Text, nullable=True, comment="审批意见/驳回原因")
    acted_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True, comment="审批操作时间")
