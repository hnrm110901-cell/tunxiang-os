...
class TravelRequest(TenantBase):
    ...
    brand_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False, index=True, comment="所属品牌ID")
    ...
    applicant_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), nullable=False, index=True, comment="申请人（可能不同于出行人）"
    )

    # 关联巡店任务（核心打通点）
    inspection_task_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True), nullable=True, index=True, comment="关联的巡店任务ID（可空，非巡店差旅为NULL）"
    )
    ...
    # 行程信息
    departure_city: Mapped[Optional[str]] = mapped_column(String(50), nullable=True, comment="出发城市")
    destination_cities: Mapped[list] = mapped_column(
        JSON, default=list, nullable=False, comment="目的地城市列表（多城市行程）"
    )
    planned_stores: Mapped[list] = mapped_column(JSON, default=list, nullable=False, comment="计划巡店的门店ID列表")
    planned_start_date: Mapped[date] = mapped_column(Date, nullable=False, comment="计划出发日期")
    planned_end_date: Mapped[date] = mapped_column(Date, nullable=False, comment="计划返回日期")
    planned_days: Mapped[int] = mapped_column(Integer, nullable=False, default=1, comment="计划天数")

    # 适用差标
    staff_level: Mapped[Optional[str]] = mapped_column(
        String(30), nullable=True, comment="出行人职级（提交时固化），参见 StaffLevel 枚举"
    )
    applicable_standards: Mapped[dict] = mapped_column(
        JSON, default=dict, nullable=False, comment="适用的差标快照（提交时固化）"
    )

    # 交通方式与预估费用
    transport_mode: Mapped[str] = mapped_column(
        String(20), nullable=False, default="train", comment="交通方式，参见 TransportMode 枚举"
    )
    estimated_cost_fen: Mapped[Optional[int]] = mapped_column(
        BigInteger, nullable=True, default=0, comment="预估总费用，单位：分(fen)，展示时除以100转元"
    )
    ...
    status: Mapped[str] = mapped_column(
        String(20),
        nullable=False,
        default=TravelStatus.DRAFT.value,
        index=True,
    ...
    # 实际数据（行程完成后填写）
    actual_start_date: Mapped[Optional[date]] = mapped_column(Date, nullable=True, comment="实际出发日期")
    actual_end_date: Mapped[Optional[date]] = mapped_column(Date, nullable=True, comment="实际返回日期")
    actual_days: Mapped[Optional[int]] = mapped_column(Integer, nullable=True, comment="实际天数")
    total_mileage_km: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(10, 2), nullable=True, default=0, comment="实际里程（公里，GPS计算）"
    )
    total_cost_fen: Mapped[Optional[int]] = mapped_column(
        BigInteger, nullable=True, default=0, comment="实际总费用，单位：分(fen)，展示时除以100转元"
    )
    mileage_allowance_fen: Mapped[Optional[int]] = mapped_column(
        BigInteger, nullable=True, default=0, comment="里程补贴，单位：分(fen)，展示时除以100转元，GPS核实后计算"
    )

    # 报销单关联
    expense_application_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True), nullable=True, comment="关联的费用报销申请ID"
    )

    notes: Mapped[Optional[str]] = mapped_column(Text, nullable=True, comment="申请备注")
...
class TravelItinerary(TenantBase):
    ...
    travel_request_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), nullable=False, index=True, comment="所属差旅申请ID"
    )
    store_id: Mapped[uuid.UUID] = mapped_column(
    ...
    checkin_location: Mapped[Optional[dict]] = mapped_column(
        JSON, nullable=True, comment='签到位置 {"lat": float, "lng": float, "accuracy": float}'
    )
    checkout_location: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True, comment="签退位置")
    gps_track: Mapped[list] = mapped_column(
        JSON, default=list, nullable=False, comment="轨迹点列表（精简存储，每5分钟1个点）"
    )
    ...
    leg_mileage_km: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(10, 2), nullable=True, default=0, comment="本段里程（到达本门店的路程，公里）"
    )
    is_mileage_anomaly: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False, comment="GPS里程异常标记（绕路>30%）"
    )
    anomaly_reason: Mapped[Optional[str]] = mapped_column(Text, nullable=True, comment="里程异常说明")
    ...
    skip_reason: Mapped[Optional[str]] = mapped_column(Text, nullable=True, comment="未到访原因")

    sequence_order: Mapped[int] = mapped_column(Integer, nullable=False, default=0, comment="行程顺序")
...
class TravelAllocation(TenantBase):
    ...
    store_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False, index=True, comment="分摊到的门店")
    brand_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False, index=True, comment="所属品牌ID")
    ...
    basis_value: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(10, 4), nullable=True, comment="分摊基准值（如停留分钟数）"
    )
    allocation_rate: Mapped[Decimal] = mapped_column(
        Numeric(7, 6), nullable=False, comment="分摊比例（0.000000-1.000000）"
    )

    # 分摊金额（单位：分）
    total_travel_cost_fen: Mapped[int] = mapped_column(
        BigInteger, nullable=False, comment="本次差旅总费用，单位：分(fen)，展示时除以100转元"
    )
    allocated_amount_fen: Mapped[int] = mapped_column(
        BigInteger, nullable=False, comment="分摊到本门店的金额，单位：分(fen)，展示时除以100转元"
    )
    ...
    is_attributed: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False, comment="是否已归入门店P&L")
