...
from .expense_enums import ExpenseStatus
...
class ExpenseCategory(TenantBase):
    ...
    code: Mapped[str] = mapped_column(
        String(50), nullable=False, index=True, comment="科目代码，参见 ExpenseCategoryCode 枚举"
    )
    ...
    sort_order: Mapped[int] = mapped_column(Integer, default=0, nullable=False, comment="排序权重，越小越靠前")
    is_system: Mapped[bool] = mapped_column(
        Boolean, default=False, nullable=False, comment="是否系统预置科目（系统科目不可删除）"
    )
...
class ExpenseScenario(TenantBase):
    ...
    code: Mapped[str] = mapped_column(
        String(50), nullable=False, unique=True, index=True, comment="场景代码，参见 ExpenseScenarioCode 枚举"
    )
    name: Mapped[str] = mapped_column(String(100), nullable=False, comment="场景名称，如「日常费用报销」")
    description: Mapped[Optional[str]] = mapped_column(Text, nullable=True, comment="场景说明")
    icon: Mapped[Optional[str]] = mapped_column(String(100), nullable=True, comment="前端图标标识符")
    ...
    required_fields: Mapped[list] = mapped_column(
        JSON, default=list, nullable=False, comment='该场景必填字段列表，如 ["purpose", "invoice_count"]'
    )
    approval_routing_hint: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True, comment="审批路由提示，如 amount_based / scenario_fixed"
    )
...
class ExpenseApplication(TenantBase):
    ...
    store_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False, index=True, comment="所属门店ID")
    ...
    total_amount: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, comment="申请总金额，单位：分(fen)，展示时除以100转元"
    )
    currency: Mapped[str] = mapped_column(String(3), nullable=False, default="CNY", comment="货币代码，默认 CNY")
    status: Mapped[str] = mapped_column(
        String(20),
        nullable=False,
        default=ExpenseStatus.DRAFT.value,
        index=True,
        comment="申请状态，参见 ExpenseStatus 枚举",
    )
    legal_entity_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True), nullable=True, comment="付款主体法人ID（预留，对接 tx-finance 法人账户）"
    )
    purpose: Mapped[Optional[str]] = mapped_column(Text, nullable=True, comment="费用用途说明")
    notes: Mapped[Optional[str]] = mapped_column(Text, nullable=True, comment="申请人备注")
    metadata: Mapped[dict] = mapped_column(
        JSON, default=dict, nullable=False, comment="扩展字段，存储场景特定的附加信息（如出差目的地、招待人数等）"
    )
    submitted_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True, comment="提交时间")
    ...
    # 关系
    scenario: Mapped["ExpenseScenario"] = relationship("ExpenseScenario", foreign_keys=[scenario_id], lazy="select")
...
class ExpenseItem(TenantBase):
    ...
    description: Mapped[str] = mapped_column(String(500), nullable=False, comment="费用描述")
    amount: Mapped[int] = mapped_column(Integer, nullable=False, comment="明细金额，单位：分(fen)，展示时除以100转元")
    quantity: Mapped[Numeric] = mapped_column(
        Numeric(10, 3), nullable=False, default=1, comment="数量（如天数、件数，支持小数）"
    )
    unit: Mapped[Optional[str]] = mapped_column(String(20), nullable=True, comment="单位，如「天」「次」「件」")
    invoice_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True), nullable=True, comment="关联发票ID（预留P1阶段对接发票模块）"
    )
    expense_date: Mapped[date] = mapped_column(Date, nullable=False, comment="费用发生日期")
    notes: Mapped[Optional[str]] = mapped_column(Text, nullable=True, comment="明细备注")

    # 关系
    application: Mapped["ExpenseApplication"] = relationship(
        "ExpenseApplication", back_populates="items", foreign_keys=[application_id]
    )
    category: Mapped["ExpenseCategory"] = relationship("ExpenseCategory", foreign_keys=[category_id], lazy="select")
...
class ExpenseAttachment(TenantBase):
    ...
    file_url: Mapped[str] = mapped_column(String(1000), nullable=False, comment="文件存储 URL（COS 对象路径）")
    file_type: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True, comment="文件 MIME 类型，如 image/jpeg、application/pdf"
    )
    file_size: Mapped[Optional[int]] = mapped_column(Integer, nullable=True, comment="文件大小（字节）")
    uploaded_by: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False, comment="上传人员工ID")
