...
class ProcurementPayment(TenantBase):
    ...
    purchase_order_id: Mapped[uuid.UUID] = mapped_column(
        ...
        comment="tx-supply 采购订单ID（唯一键，幂等保护）"
    )
    ...
    payment_type: Mapped[str] = mapped_column(
        String(32), nullable=False, default="purchase",
        comment="付款类型：purchase / deposit / final"
    )
    total_amount: Mapped[int] = mapped_column(
        BigInteger, nullable=False,
        comment="总金额，单位：分(fen)"
    )
...
class ProcurementReconciliation(TenantBase):
    ...
    payment_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("procurement_payments.id"),
        nullable=True, index=True,
        comment="所属付款单ID"
    )
