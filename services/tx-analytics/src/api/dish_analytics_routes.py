...
async def top_selling_dishes(
    ...
    try:
        async with async_session_factory() as session:
            ...
            log.info("dishes.top_selling", tenant=x_tenant_id, days=days, store_id=store_id, count=len(dishes))
    ...
    try:
        async with async_session_factory() as session:
            ...
            heat_index: dict[tuple[int, int], float] = {
                (int(r["day_of_week"]), int(r["hour"])): float(r["count"]) for r in rows
            }
            heatmap = [
                {
                    "day_of_week": dow,
            ...
            log.info("dishes.time_heatmap", tenant=x_tenant_id, dish_id=dish_id, nonzero_cells=len(rows))
...
async def dish_pairing(
    ...
    try:
        async with async_session_factory() as session:
            ...
            log.info("dishes.pairing", tenant=x_tenant_id, dish_id=dish_id, pairings_count=len(pairings))
...
async def underperforming_dishes(
    ...
    try:
        async with async_session_factory() as session:
            ...
            log.info(
                "dishes.underperforming", tenant=x_tenant_id, days=days, threshold=min_sales_threshold, count=len(items)
            )
