"""SplitAttributionProjector lifespan 激活 Tier 1 测试 (PRD-11 sub-C 激活 PR / 2026-05-16 / Tier 1 邻接第 35 例).

镜像 tx-supply `test_lifespan_index_split_tier1.py` (sub-B.2 激活 PR #698 模板).
...
- §19 round-1 P1-1 mirror: shutdown 用 stop_all_split_attribution_projectors() 兜底
- §19 round-1 P1-2 mirror: list_active_tenants SQL 用 `status='active'` 而非 `is_deleted=FALSE`

mock 风格: patch registry 模块函数，AsyncMock，无真 DB. 避免触发 main.py 全 router import.
"""
...
# 避免触发 main.py 所有 router import（pre-existing tx-analytics 大量 router import 副作用）

from services.tx_analytics.src.projectors.registry import (  # noqa: E402
    is_enabled,
    list_active_tenants,
...
async def _test_lifespan():
    ...
    from services.tx_analytics.src.projectors.registry import (
        ...
        stop_split_attribution_projector as _stop,
    )
    import structlog
...
_REGISTRY_PATH = "services.tx_analytics.src.projectors.registry"
...
async def test_lifespan_env_on_two_tenants(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert started_args == {_T1, _T2}, (
        f"Expected start for both tenants, got: {started_args}"
    )

    # §19 round-1 P1-1 mirror: shutdown 应调一次 stop_all (兜底), 不靠 started_tenants 闭包
...
async def test_lifespan_tenant_delta(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert _T1 in stopped_args, "T1 应在轮 3 移除时被 stop_split_attribution_projector"
    # §19 round-1 P1-1 mirror: shutdown 兜底 stop_all
    mock_stop_all.assert_awaited_once()


# ─── §19 round-1 P1-2 mirror regression ──────────────────────────────────────


@pytest.mark.asyncio
async def test_list_active_tenants_uses_status_filter() -> None:
    """§19 round-1 P1-2 mirror regression (PR #698 教训复用): SQL 必须用 `status = 'active'`.
    ...
    见 feedback_tenants_v006_schema_no_is_deleted.md + PR #698 / #707.
    """
    from services.tx_analytics.src.projectors import registry as _registry
    import inspect
    import re
    ...
    assert "status = 'active'" in body or 'status = "active"' in body, (
        ...
        "WHERE is_deleted = FALSE 是 cert_expiry_alerter pre-existing bug, PR #707 已修."
    )
