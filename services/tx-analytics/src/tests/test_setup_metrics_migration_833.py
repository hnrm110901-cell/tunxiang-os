"""#833 Lane A — tx-analytics setup_metrics helper 迁移烟测
...
def test_setup_metrics_helper_used() -> None:
    """main.py 必须调用 setup_metrics(app, service_name="tx-analytics")."""
    src = _source()
    assert 'setup_metrics(app, service_name="tx-analytics")' in src, (
        "tx-analytics/main.py 未调用 setup_metrics helper (#833 Lane A 迁移未完成)"
    )
...
def test_observability_import_present() -> None:
    ...
    assert "from shared.observability import setup_metrics" in src, (
        "tx-analytics/main.py 缺少 shared.observability setup_metrics import"
    )
...
def test_raw_instrumentator_removed() -> None:
    ...
    assert "Instrumentator().instrument(app).expose(app)" not in src, (
        "tx-analytics/main.py 仍含裸 Instrumentator 调用 — 迁移未完成"
    )
...
def test_metrics_auth_middleware_retained() -> None:
    ...
    assert "app.add_middleware(MetricsAuthMiddleware)" in src, (
        "tx-analytics/main.py 丢失 MetricsAuthMiddleware mount — 鉴权层被意外移除"
    )
