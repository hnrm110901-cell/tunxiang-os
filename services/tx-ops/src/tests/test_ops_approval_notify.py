...
for _entity_name in ["DailySummary", "EmployeeDailyPerformance", "InspectionReport",
...
class TestApprovalCenterPending:
    ...
    def test_list_pending_success(self):
        ...
        row_data = {
            "id": APPROVAL_ID, "business_type": "discount", "business_id": "ORD-001",
            "title": "折扣审批", "description": "10%折扣", "amount_fen": 800,
            "initiator_id": "EMP-001", "initiator_name": "张三",
            "current_step": 1, "total_steps": 2, "status": "pending",
    ...
    def test_take_action_approve_success(self):
        ...
        instance_row = _mock_row({
            ...
            "business_type": "discount", "title": "折扣审批",
        })
...
class TestApprovalWorkflowTemplates:
    ...
    def test_list_templates_with_type_filter(self):
        ...
        row = {"id": str(uuid.uuid4()), "template_name": "折扣模板",
               "business_type": "discount", "steps": [], "is_active": True,
               "created_at": None, "updated_at": None, "tenant_id": TENANT_ID}
        db = self._make_db()
...
class TestNotificationSend:
    ...
    def setup_method(self):
        ...
        self._svc_instance.send_sms = AsyncMock(return_value={
            "channel": "sms", "status": "sent", "message_id": str(uuid.uuid4()),
        })
        self._svc_instance.send_wechat = AsyncMock(return_value={
            "channel": "wechat", "status": "sent", "message_id": str(uuid.uuid4()),
        })
        self._svc_instance.send_wecom = AsyncMock(return_value={
            "channel": "wecom", "status": "sent", "message_id": str(uuid.uuid4()),
        })
        self._svc_instance.list_notifications = AsyncMock(return_value={
