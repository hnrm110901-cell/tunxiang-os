...
class ForgeMCPService:
    ...
    async def list_servers(
        self, app_id: Optional[str], health_status: Optional[str], page: int, size: int,
    ) -> Dict[str, Any]:
        ...
        if health_status:
            clauses.append("health_status = :hs"); params["hs"] = health_status
    ...
    async def list_tools(
        self, server_id: Optional[str], entity: Optional[str],
        trust_tier: Optional[str], page: int, size: int,
    ) -> Dict[str, Any]:
        clauses, params = ["tenant_id = :tid"], {"tid": self.tenant_id, "limit": size, "offset": (page - 1) * size}
        if server_id:
            clauses.append("server_id = :server_id"); params["server_id"] = server_id
        if entity:
            clauses.append("ontology_bindings ? :entity"); params["entity"] = entity
        if trust_tier:
            clauses.append("trust_tier_required = :tier"); params["tier"] = trust_tier
        where = " AND ".join(clauses)
        rows = await self.db.execute(
    ...
    async def get_tool_schema(self, tool_id: str) -> Dict[str, Any]:
        row = await self.db.execute(
            text("SELECT id, tool_name, input_schema, ontology_bindings, trust_tier_required FROM forge.mcp_tools WHERE tenant_id = :tid AND id = :tool_id"),
