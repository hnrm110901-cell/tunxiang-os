...
def test_refresh_segment_cache_http_error():
    """tx-member 调用失败 → 502"""
    _fake_svc.refresh_segment_cache = AsyncMock(
...
def test_get_segment_members_ok():
    """正常分页返回分群成员列表"""
    _fake_svc.get_segment_members = AsyncMock(return_value={
...
def test_get_segment_members_not_found():
    ...
    _fake_svc.get_segment_members = AsyncMock(
        side_effect=ValueError("分群不存在: nonexistent")
    )
    resp = client.get(
...
def test_count_segment_not_found():
    """分群不存在 ValueError → 404"""
    _fake_svc.count_segment = AsyncMock(
        side_effect=ValueError("分群不存在: nonexistent")
    )
    resp = client.post(
