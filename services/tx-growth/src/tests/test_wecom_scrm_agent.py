...
class TestBirthdayUpcomingList:
    ...
    def test_each_member_has_recommend_template(self):
        ...
        for member in items:
            assert "recommend_template" in member, \
                f"会员 {member['name']} 缺少 recommend_template 字段"
    ...
    def test_each_member_has_required_fields(self):
        resp = client.get("/api/v1/growth/scrm-agent/birthday/upcoming?days_ahead=7")
        items = resp.json()["data"]["items"]
        required = {"member_id", "name", "phone_masked", "birthday", "days_until", "level",
                    "last_spend_fen", "recommend_template", "send_status", "wecom_connected"}
        for member in items:
...
class TestDormantMemberList:
    ...
    def test_each_member_has_required_fields(self):
        resp = client.get("/api/v1/growth/scrm-agent/dormant/list?dormant_days=60")
        items = resp.json()["data"]["items"]
        required = {"member_id", "name", "dormant_days", "total_spend_fen",
                    "predicted_response_rate", "suggest_offer", "favorite_dish"}
        for member in items:
            missing = required - set(member.keys())
            assert len(missing) == 0, f"会员 {member.get('name')} 缺少字段: {missing}"
    ...
    def test_wake_endpoint_skips_over_180_days(self):
        ...
        assert any(r["member_id"] == "mem-d04" for r in skipped), \
            "沉睡229天的会员应被系统自动跳过"
    ...
    def test_dormant_list_excludes_unsubscribed(self):
        ...
        items = resp.json()["data"]["items"]
        for member in items:
            assert not member.get("unsubscribed"), \
                f"已退订的会员 {member['name']} 不应出现在列表中"
