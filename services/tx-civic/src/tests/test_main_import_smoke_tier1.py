"""Tier 1 — tx-civic main.py 容器布局 import 烟测

防止 production main.py 启动失败的回归门禁。复用 shared/test_infra/main_import_smoke.py
helper（构造 mktemp 容器布局 + subprocess 隔离）。

为什么 Tier 1：services/tx-civic/src/main.py 是 production 入口
（Dockerfile CMD: uvicorn services.tx_civic.src.main:app），main.py import 失败 = 服务无法启动。
...
def test_main_module_loads_in_container_layout() -> None:
    """tx_civic main.py 必须在 Docker 容器布局下干净 import。"""
