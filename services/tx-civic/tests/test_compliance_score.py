...
class TestCalculateStoreScore:
    ...
    async def test_calculate_store_score_happy_path(self):
        ...
        with (
            ...
            # Mock TenantSession — 四次内部 execute:
            ...
            # 调用 4: INSERT score     → no scalar needed (INSERT result ignored)
            mock_session = AsyncMock()
            mock_session.execute = AsyncMock(side_effect=[
                MagicMock(scalar=MagicMock(return_value=0)),  # critical check
                MagicMock(scalar=MagicMock(return_value=7)),  # waste days
                MagicMock(scalar=MagicMock(return_value=4)),  # patrol count
                MagicMock(),                                  # INSERT score
            ])
            mock_cm = MagicMock()
