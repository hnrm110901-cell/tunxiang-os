...
import api.group_routes  # 确保 api 包已初始化
...
def test_gdpr_create_request_success():
    ...
    fake_svc = MagicMock()
    fake_svc.create_request = AsyncMock(return_value={
        "id": str(uuid.uuid4()),
...
def test_gdpr_review_request_value_error():
    ...
    fake_svc = MagicMock()
    fake_svc.review_request = AsyncMock(
        side_effect=ValueError("请求已审核，不可重复操作")
    )
