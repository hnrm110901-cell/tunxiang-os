...
async def lifespan(app: FastAPI):
    ...
    _tax_filing_mod.get_db = _shared_get_db
    ...
    if is_enabled(OrgFlags.HR_REVENUE_SCHEDULE):
        _flag_logger.info(
            ...
            flag=OrgFlags.HR_REVENUE_SCHEDULE,
            note="营收驱动智能排班已激活，HRAgentScheduler将运行排班优化任务",
        )
    else:
        _flag_logger.info(
            "feature_flag_disabled", flag=OrgFlags.HR_REVENUE_SCHEDULE, note="营收驱动排班已关闭，排班优化任务将跳过"
        )
    ...
    if is_enabled(OrgFlags.HR_CONTRIBUTION_SCORE):
        _flag_logger.info(
            "feature_flag_enabled",
            flag=OrgFlags.HR_CONTRIBUTION_SCORE,
    ...
    if is_enabled(OrgFlags.HR_ATTRITION_MODEL):
        _flag_logger.info(
            "feature_flag_enabled",
            flag=OrgFlags.HR_ATTRITION_MODEL,
            note="离职预警模型已激活，HRAgentScheduler将运行离职风险扫描",
        )
    else:
        _flag_logger.info(
            "feature_flag_disabled", flag=OrgFlags.HR_ATTRITION_MODEL, note="离职预警模型已关闭，离职风险扫描任务将跳过"
        )
...
app.include_router(staffing_template_router)  # 门店编制模板CRUD（v206表）人力中枢S1
app.include_router(staffing_analysis_router)  # 编制对标分析（v206表）人力中枢S1
app.include_router(dri_workorder_router)  # DRI工单中心（v206表）人力中枢S1
app.include_router(ai_alert_router)  # AI预警记录（v206表）人力中枢S1
app.include_router(mentorship_router)  # 带教关系管理（v206表）人力中枢S2
app.include_router(onboarding_path_router)  # 新员工训练路径（v206表）人力中枢S2
app.include_router(certification_router)  # 岗位认证与通关（v206表）人力中枢S2
app.include_router(store_readiness_router)  # 门店就绪度评分（v206表）人力中枢S3
app.include_router(peak_guard_router)  # 高峰保障指挥（v206表）人力中枢S3
...
app.include_router(transfer_db_router)  # 借调管理+成本分摊（v251表）DB持久化版
app.include_router(e_signature_router)  # 电子签约模块（v252表）合同模板+签署流程
...
app.include_router(daily_scorecard_router)  # 日KPI得分卡（v377表）Sprint G5
app.include_router(bonus_router)  # 绩效奖金计算（v377表）Sprint G5
app.include_router(lifecycle_router)  # 门店生命周期（v377表）Sprint G5
