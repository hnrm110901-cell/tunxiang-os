...
def test_recommend_success(client: TestClient):
    r = client.post(
        "/api/v1/org/salary-advisor/recommend",
        json={
            "role": "店长",
            "region": "长沙",
            "years_of_service": 5,
...
def test_batch_success(client: TestClient):
    r = client.post(
        "/api/v1/org/salary-advisor/batch",
        json={
            "employees": [
...
def test_role_tiers_endpoint(client: TestClient):
    ...
    assert tier_codes == {"L1_basic", "L2_skilled", "L3_senior_skilled", "L4_supervisor", "L5_manager", "L6_regional"}
    # 字段命名:不得再暴露 tier2
    for t in data["tiers"]:
