...
class FoodCourt(TenantBase):
    ...
    store_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), nullable=False, index=True, comment="所属主门店ID（外键逻辑引用 stores.id）"
    )
    name: Mapped[str] = mapped_column(String(100), nullable=False, comment="商街名称，如：XX美食广场")
    description: Mapped[Optional[str]] = mapped_column(String(500), comment="描述/简介")
    status: Mapped[str] = mapped_column(
        String(20), nullable=False, default="active", comment="active=营业中 / inactive=停用"
    )
    unified_cashier: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=True, comment="是否启用统一收银台（True=顾客在统一收银点结账）"
    )
    # 扩展配置：营业时间、收银台数量等
    config: Mapped[Optional[dict]] = mapped_column(JSON, default=dict, comment="扩展配置JSON")
...
class FoodCourtVendor(TenantBase):
    ...
    food_court_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("food_courts.id"), nullable=False, index=True, comment="所属商街ID"
    )
    vendor_code: Mapped[str] = mapped_column(
        String(20), nullable=False, comment="档口编号，如：A1 / B2（同一商街内唯一）"
    )
    vendor_name: Mapped[str] = mapped_column(String(100), nullable=False, comment="档口名称，如：阿强烧腊、李姐粉面")
    category: Mapped[Optional[str]] = mapped_column(String(50), comment="档口分类：烧腊/粉面/饮料/小吃/快餐等")
    owner_name: Mapped[Optional[str]] = mapped_column(String(50), comment="档主姓名")
    ...
    kds_station_id: Mapped[Optional[str]] = mapped_column(String(50), comment="对应KDS档口站点ID，KDS推送时使用")
    status: Mapped[str] = mapped_column(
        String(20),
        nullable=False,
        default="active",
    ...
    settlement_account: Mapped[Optional[dict]] = mapped_column(
        JSON, default=dict, comment="结算账户信息JSON：{type, account_last4, holder_name, bank_name}"
    )
    display_order: Mapped[int] = mapped_column(Integer, default=1, comment="排列顺序（收银台菜单分组顺序）")
...
class FoodCourtOrder(TenantBase):
    ...
    order_no: Mapped[str] = mapped_column(
        String(50), nullable=False, unique=True, index=True, comment="订单号，格式：FC{YYYYMMDD}{6位序号}"
    )
    ...
    cashier_id: Mapped[Optional[str]] = mapped_column(String(50), comment="收银员工号/ID")
    notes: Mapped[Optional[str]] = mapped_column(String(500), comment="订单备注")
    # 幂等键，防止重复支付
    idempotency_key: Mapped[Optional[str]] = mapped_column(String(128), unique=True, index=True, comment="支付幂等键")
...
class FoodCourtOrderItem(TenantBase):
    ...
    order_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("food_court_orders.id"), nullable=False, index=True, comment="所属商街订单ID"
    )
    vendor_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("food_court_vendors.id"),
        nullable=False,
        index=True,
    ...
    dish_id: Mapped[Optional[str]] = mapped_column(String(50), comment="菜品ID（可空：档口有自己菜单时填写）")
    ...
    subtotal_fen: Mapped[int] = mapped_column(Integer, nullable=False, comment="小计（分）= quantity × unit_price_fen")
    notes: Mapped[Optional[str]] = mapped_column(String(200), comment="菜品备注/做法要求")
    status: Mapped[str] = mapped_column(
        String(20),
        nullable=False,
        default="pending",
    ...
    # KDS确认出餐时间戳
    ready_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), comment="档口标记出餐时间")
...
class FoodCourtVendorSettlement(TenantBase):
    ...
    vendor_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("food_court_vendors.id"), nullable=False, index=True, comment="档口ID"
    )
    food_court_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("food_courts.id"), nullable=False, index=True, comment="商街ID"
    )
    settlement_date: Mapped[date] = mapped_column(Date, nullable=False, index=True, comment="结算日期")
    order_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0, comment="当日订单笔数")
    item_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0, comment="当日菜品份数")
    gross_amount_fen: Mapped[int] = mapped_column(Integer, nullable=False, default=0, comment="当日营业总额（分）")
    commission_fen: Mapped[int] = mapped_column(Integer, nullable=False, default=0, comment="平台抽成（分）")
    net_amount_fen: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, comment="档口实得金额（分）= gross - commission"
    )
    status: Mapped[str] = mapped_column(
        String(20), nullable=False, default="pending", comment="pending=待结算 / settled=已结算"
    )
    settled_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), comment="结算确认时间")
    operator_id: Mapped[Optional[str]] = mapped_column(String(50), comment="财务确认人ID")
    # 结算明细快照
    details: Mapped[Optional[dict]] = mapped_column(
        JSON, default=dict, comment="结算明细快照JSON：{orders: [], payment_breakdown: {}}"
    )
...
class CreateVendorReq(BaseModel):
    ...
    commission_rate: Optional[float] = Field(default=None, ge=0.0, le=1.0, description="抽成比例（0.0–1.0）")
...
class PayFoodCourtOrderReq(BaseModel):
    payment_method: str = Field(description="支付方式：cash/wechat/alipay/unionpay/member_balance")
...
class GenerateSettlementReq(BaseModel):
    ...
    vendor_ids: Optional[list[str]] = Field(default=None, description="指定档口ID列表，不传则结算所有档口")
