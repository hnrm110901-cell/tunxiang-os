...
class BanquetLiveOrder(TenantBase):
    ...
    order_type: Mapped[str] = mapped_column(String(30), nullable=False, comment="add_dish/add_drink/special_request/cancel_dish/upgrade_dish/extra_service")
    ...
    def to_dict(self) -> dict:
        return {"id": str(self.id), "banquet_id": str(self.banquet_id), "order_type": self.order_type, "items_json": self.items_json, "amount_fen": self.amount_fen, "quantity": self.quantity, "requested_name": self.requested_name, "status": self.status, "notes": self.notes, "created_at": self.created_at.isoformat() if self.created_at else None}
...
class BanquetGuestCheckIn(TenantBase):
    ...
    def to_dict(self) -> dict:
        return {"id": str(self.id), "banquet_id": str(self.banquet_id), "table_no": self.table_no, "guest_name": self.guest_name, "check_in_time": self.check_in_time.isoformat() if self.check_in_time else None, "vip_flag": self.vip_flag, "dietary_notes": self.dietary_notes, "seat_assignment": self.seat_assignment}
