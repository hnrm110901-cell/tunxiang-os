...
class ProductionDept(TenantBase):
    ...
    dept_name: Mapped[str] = mapped_column(String(50), nullable=False, comment="部门名称：热菜间/凉菜间/面点/海鲜/吧台")
    dept_code: Mapped[str] = mapped_column(String(20), nullable=False, comment="部门编码", index=True)
    brand_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False, index=True, comment="品牌ID")
    ...
    printer_type: Mapped[Optional[str]] = mapped_column(
        String(20), server_default="network", comment="打印机类型：network（网络打印机）/ usb / bluetooth"
    )
    ...
    display_color: Mapped[Optional[str]] = mapped_column(
        String(20), server_default="blue", comment="KDS屏颜色标识：red/orange/green/blue/purple"
    )
    fixed_fee_type: Mapped[Optional[str]] = mapped_column(
        String(30), comment="固定费用类型：茶位费/服务费/包间费/餐位费/无"
    )
    default_timeout_minutes: Mapped[int] = mapped_column(Integer, default=15, comment="默认出品时限(分钟)")
    sort_order: Mapped[int] = mapped_column(Integer, default=0, comment="排序序号")
    is_active: Mapped[bool] = mapped_column(
        Boolean, nullable=False, server_default="true", comment="档口是否启用（停用后不接收新的分单任务）"
    )
...
class DishDeptMapping(TenantBase):
    ...
    dish_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False, index=True, comment="菜品ID")
    ...
    kds_terminal_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), comment="关联KDS终端ID")
    sort_order: Mapped[int] = mapped_column(Integer, default=0, comment="菜品在该档口内的排序")
    is_primary: Mapped[bool] = mapped_column(
        Boolean, nullable=False, server_default="true", comment="是否为菜品主档口（优先用于路由分单）"
    )
