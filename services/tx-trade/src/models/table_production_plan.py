...
class TableProductionPlan(TenantBase):
    ...
    target_completion: Mapped[DateTime] = mapped_column(
        DateTime(timezone=True), nullable=False, comment="协调基准时间（最慢档口预计完成时间）"
    )
    ...
    dept_readiness: Mapped[dict] = mapped_column(
        JSONB, nullable=False, default=dict, comment="JSON: {dept_id: ready_bool} 各档口就绪状态"
    )
    dept_delays: Mapped[dict] = mapped_column(
        JSONB, nullable=False, default=dict, comment="JSON: {dept_id: delay_seconds} 各档口延迟开始时间(秒)"
    )
