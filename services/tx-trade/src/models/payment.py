...
class Payment(TenantBase):
    ...
    method: Mapped[str] = mapped_column(String(20), nullable=False, default=PaymentMethod.cash.value)
    ...
    # 实收属性
    is_actual_revenue: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True, comment="是否计入实收")
    actual_revenue_ratio: Mapped[float] = mapped_column(
        Float, nullable=False, default=1.0, comment="实收比例(0-1)，例如团购券按面额的0.9计入实收"
    )
...
class Refund(TenantBase):
    ...
    refund_type: Mapped[str] = mapped_column(String(20), nullable=False, default=RefundType.full.value)
