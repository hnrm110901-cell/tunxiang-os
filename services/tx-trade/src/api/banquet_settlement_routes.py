...
async def generate(req: GenerateReq, r: Request, db: AsyncSession = Depends(_db)):
    try:
        return _ok(await BanquetSettlementService(db, _tid(r)).generate_settlement(req.banquet_id))
...
async def get_settlement(settlement_id: str, r: Request, db: AsyncSession = Depends(_db)):
    try:
        return _ok(await BanquetSettlementService(db, _tid(r)).get_settlement(settlement_id))
    except ValueError as e:
        _err(str(e), 404)
