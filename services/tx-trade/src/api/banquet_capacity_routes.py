...
async def daily_overview(
    store_id: str, date: str = Query(...), request: Request = None, db: AsyncSession = Depends(_get_db_session)
):
...
async def list_conflicts(
    ...
    svc = BanquetCapacityService(db=db, tenant_id=_get_tenant_id(request))
    return _ok(
        await svc.list_conflicts(store_id, date_cls.fromisoformat(date_from), date_cls.fromisoformat(date_to), status)
    )


@router.patch("/conflicts/{conflict_id}/resolve")
async def resolve_conflict(
    conflict_id: str, req: ResolveConflictReq, request: Request = None, db: AsyncSession = Depends(_get_db_session)
):
    svc = BanquetCapacityService(db=db, tenant_id=_get_tenant_id(request))
    try:
