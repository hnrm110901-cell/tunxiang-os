...
async def _record_metric(db: AsyncSession, tenant_id: str, platform: str, success: bool, duration_ms: float, error_code: Optional[str] = None) -> None:
    try:
        await db.execute(
...
async def receive_webhook(
    ...
    row = await db.execute(
        ...
        """),
        {
            "tid": tenant_id, "platform": platform, "poid": payload.platform_order_id,
            ...
            "raw": json.dumps(payload.model_dump()), "extra": json.dumps(payload.extra or {}),
        },
    )
    result = row.fetchone()
    ...
    return {"ok": True, "data": {"aggregator_order_id": agg_id, "is_new": is_new, "platform_ack": PLATFORM_CONFIG[platform]["accept_ack"]}, "error": None}
...
async def list_aggregator_orders(
    platform: Optional[str] = Query(None), status: Optional[str] = Query(None),
    store_id: Optional[str] = Query(None), page: int = Query(1, ge=1), size: int = Query(20, ge=1, le=100),
    db: AsyncSession = Depends(_get_db),
) -> dict:
    conds = ["1=1"]
...
async def get_platforms_status(db: AsyncSession = Depends(_get_db)) -> dict:
    ...
    for pid, cfg in PLATFORM_CONFIG.items():
        ...
        platforms.append({"platform": pid, "label": cfg["label"], "color": cfg["color"], "online": True,
...
async def get_metrics(hours: int = Query(24, ge=1, le=168), db: AsyncSession = Depends(_get_db)) -> dict:
    ...
    if not records:
        return {"ok": True, "data": {"total_requests": 0, "success_rate": 1.0, "avg_latency_ms": 0, "by_platform": {}}, "error": None}
    total = len(records)
    ...
    for pid in SUPPORTED_PLATFORMS:
        ...
        if not pr: continue
        pt = len(pr); ps = sum(1 for r in pr if r.success); pl = sorted(r.duration_ms for r in pr)
        pp = max(0, int(len(pl) * 0.99) - 1)
        ec: dict = {}
        for r in pr:
            if not r.success and r.error_code: ec[r.error_code] = ec.get(r.error_code, 0) + 1
        by_platform[pid] = {"label": PLATFORM_CONFIG[pid]["label"], "total": pt, "success_rate": round(ps/pt, 4),
                            "avg_ms": round(sum(pl)/len(pl), 2), "p99_ms": round(pl[pp], 2), "errors": ec}
    return {"ok": True, "data": {"total_requests": total, "success_rate": round(ok/total, 4),
            "avg_latency_ms": round(sum(lats)/len(lats), 2), "p99_latency_ms": round(lats[p99i], 2),
            "by_platform": by_platform, "window_hours": hours}, "error": None}
