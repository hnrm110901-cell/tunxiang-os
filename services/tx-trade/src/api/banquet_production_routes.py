...
async def confirm_plan(
    plan_id: str, req: ConfirmPlanReq, request: Request, db: AsyncSession = Depends(_get_db_session)
):
    svc = BanquetProductionService(db=db, tenant_id=_get_tenant_id(request))
    try:
...
async def update_task_status(
    task_id: str, req: UpdateTaskStatusReq, request: Request, db: AsyncSession = Depends(_get_db_session)
):
    svc = BanquetProductionService(db=db, tenant_id=_get_tenant_id(request))
    try:
