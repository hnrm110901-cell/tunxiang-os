...
async def create(req: CreateReq, r: Request, db: AsyncSession = Depends(_db)):
    return _ok(await BanquetLiveOrderService(db, _tid(r)).create_live_order(req.banquet_id, req.order_type, req.items_json, req.amount_fen, req.quantity, notes=req.notes, requested_name=req.requested_name))

@router.get("/{banquet_id}")
async def list_orders(banquet_id: str, status: Optional[str] = None, r: Request = None, db: AsyncSession = Depends(_db)):
...
async def approve(live_order_id: str, r: Request, db: AsyncSession = Depends(_db)):
    try: return _ok(await BanquetLiveOrderService(db, _tid(r)).approve(live_order_id, _tid(r)))
...
async def reject(live_order_id: str, req: RejectReq, r: Request, db: AsyncSession = Depends(_db)):
    try: return _ok(await BanquetLiveOrderService(db, _tid(r)).reject(live_order_id, req.reason))
    except ValueError as e: _err(str(e))
