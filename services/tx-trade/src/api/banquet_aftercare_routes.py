...
async def submit_feedback(req: FeedbackReq, r: Request, db: AsyncSession = Depends(_db)):
    try: return _ok(await BanquetAftercareService(db, _tid(r)).submit_feedback(**req.model_dump()))
    except ValueError as e: _err(str(e))

@router.get("/feedbacks")
async def list_feedbacks(banquet_id: Optional[str] = None, min_score: Optional[int] = None, page: int = 1, size: int = 20, r: Request = None, db: AsyncSession = Depends(_db)):
    return _ok(await BanquetAftercareService(db, _tid(r)).list_feedbacks(banquet_id=banquet_id, min_score=min_score, page=page, size=size))
...
async def reply_feedback(feedback_id: str, req: ReplyReq, r: Request, db: AsyncSession = Depends(_db)):
    return _ok(await BanquetAftercareService(db, _tid(r)).reply_feedback(feedback_id, req.reply_content, req.replied_by))
...
async def list_referrals(banquet_id: Optional[str] = None, status: Optional[str] = None, r: Request = None, db: AsyncSession = Depends(_db)):
