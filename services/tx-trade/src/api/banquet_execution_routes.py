...
async def create_plan(req: CreatePlanReq, r: Request, db: AsyncSession = Depends(_db)):
    try: return _ok(await BanquetExecutionService(db, _tid(r)).create_plan(req.banquet_id))
...
async def start(plan_id: str, r: Request, db: AsyncSession = Depends(_db)):
    try: return _ok(await BanquetExecutionService(db, _tid(r)).start_execution(plan_id))
    except ValueError as e: _err(str(e))

@router.get("/plans/{plan_id}/progress")
async def progress(plan_id: str, r: Request, db: AsyncSession = Depends(_db)):
    try: return _ok(await BanquetExecutionService(db, _tid(r)).get_progress(plan_id))
...
async def complete_cp(log_id: str, req: CompleteCheckpointReq, r: Request, db: AsyncSession = Depends(_db)):
    try: return _ok(await BanquetExecutionService(db, _tid(r)).complete_checkpoint(log_id, req.executor_id, req.executor_name, req.issue_note))
    except ValueError as e: _err(str(e))
...
async def escalate_cp(log_id: str, req: EscalateReq, r: Request, db: AsyncSession = Depends(_db)):
    try: return _ok(await BanquetExecutionService(db, _tid(r)).escalate_checkpoint(log_id, req.issue_note))
    except ValueError as e: _err(str(e))
