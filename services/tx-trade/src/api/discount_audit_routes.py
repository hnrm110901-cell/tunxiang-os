...
async def get_audit_log(
    ...
    user: UserContext = Depends(require_role_audited("discount.audit.read", "admin", "tenant_admin", "auditor", "audit_admin")),
):
...
async def get_audit_summary(
    ...
    db: AsyncSession = Depends(get_db),
    user: UserContext = Depends(require_role_audited("discount.audit.summary", "admin", "tenant_admin", "auditor", "audit_admin")),
):
...
async def get_high_risk(
    ...
    size: int = Query(20, ge=1, le=100),
    db: AsyncSession = Depends(get_db),
    user: UserContext = Depends(require_role_audited("discount.audit.high_risk", "admin", "tenant_admin", "auditor", "audit_admin")),
):
