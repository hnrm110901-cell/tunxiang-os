...
class BanquetAftercareService:
    ...
    async def submit_feedback(self, banquet_id: str, overall_score: int, food_score: int = 0, service_score: int = 0, venue_score: int = 0, value_score: int = 0, comments: str = None, improvement_suggestions: str = None, would_recommend: bool = True, customer_name: str = None, customer_phone: str = None) -> dict:
        if not 1 <= overall_score <= 5: raise ValueError("评分范围1-5")
        fid = str(uuid.uuid4())
        ...
        """), {"id": fid, "tid": self.tenant_id, "bid": banquet_id, "name": customer_name, "phone": customer_phone, "overall": overall_score, "food": food_score, "svc": service_score, "venue": venue_score, "value": value_score, "comments": comments, "improve": improvement_suggestions, "recommend": would_recommend})
        await self.db.flush()
    ...
    async def reply_feedback(self, feedback_id: str, reply_content: str, replied_by: str) -> dict:
        ...
        await self.db.execute(text("UPDATE banquet_feedbacks SET reply_content = :reply, replied_by = :by, replied_at = :now WHERE id = :id AND tenant_id = :tid AND is_deleted = FALSE"), {"id": feedback_id, "tid": self.tenant_id, "reply": reply_content, "by": replied_by, "now": now})
        await self.db.flush()
        return {"id": feedback_id, "replied": True}

    async def list_feedbacks(self, banquet_id: str = None, store_id: str = None, min_score: int = None, page: int = 1, size: int = 20) -> dict:
        ...
        rows = await self.db.execute(text(f"SELECT * FROM banquet_feedbacks bf WHERE {where} ORDER BY bf.created_at DESC OFFSET :offset LIMIT :limit"), params)
    ...
    async def get_satisfaction_stats(self, store_id: str = None) -> dict:
        ...
        total = r["total"] or 0
        return {"total_feedbacks": total, "avg_overall": round(float(r["avg_score"] or 0), 1), "avg_food": round(float(r["avg_food"] or 0), 1), "avg_service": round(float(r["avg_service"] or 0), 1), "avg_venue": round(float(r["avg_venue"] or 0), 1), "nps_rate": round(r["recommend_count"] / max(total, 1) * 100, 1)}

    async def create_referral(self, referrer_banquet_id: str, referred_name: str, referred_phone: str, referrer_name: str = None, referrer_phone: str = None, reward_type: str = "coupon", reward_value_fen: int = 0) -> dict:
        rid = str(uuid.uuid4())
        await self.db.execute(text("""
        ...
        """), {"id": rid, "tid": self.tenant_id, "rbid": referrer_banquet_id, "rname": referrer_name, "rphone": referrer_phone, "dname": referred_name, "dphone": referred_phone, "rtype": reward_type, "rvalue": reward_value_fen})
        await self.db.flush()
