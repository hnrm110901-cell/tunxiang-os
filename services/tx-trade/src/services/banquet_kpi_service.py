...
class BanquetKPIService:
    ...
    async def generate_snapshot(self, store_id: str, period: str, period_date: date) -> dict:
        ...
        # 线索
        leads = await self.db.execute(text("SELECT COUNT(*) FROM banquet_leads WHERE store_id = :sid AND tenant_id = :tid AND created_at::date BETWEEN :df AND :dt AND is_deleted = FALSE"), {"sid": store_id, "tid": self.tenant_id, "df": df, "dt": dt})
        leads_count = leads.scalar_one() or 0
        won = await self.db.execute(text("SELECT COUNT(*) FROM banquet_leads WHERE store_id = :sid AND tenant_id = :tid AND status = 'won' AND created_at::date BETWEEN :df AND :dt AND is_deleted = FALSE"), {"sid": store_id, "tid": self.tenant_id, "df": df, "dt": dt})
        won_count = won.scalar_one() or 0
        conversion = round(won_count / max(leads_count, 1) * 100, 1)
        # 宴会
        bq = await self.db.execute(text("""
        ...
        b = bq.mappings().first()
        # 满意度
        sat = await self.db.execute(text("SELECT COALESCE(AVG(overall_score), 0) FROM banquet_feedbacks bf JOIN banquets bq ON bq.id = bf.banquet_id WHERE bq.store_id = :sid AND bf.tenant_id = :tid AND bq.event_date BETWEEN :df AND :dt AND bf.is_deleted = FALSE"), {"sid": store_id, "tid": self.tenant_id, "df": df, "dt": dt})
        ...
        """), {"id": sid, "tid": self.tenant_id, "sid": store_id, "period": period, "pdate": period_date, "leads": leads_count, "conv": conversion, "bookings": b["cnt"], "rev": b["rev"], "avg": int(b["avg_table"]), "tables": b["tables"], "guests": b["guests"], "sat": satisfaction})
        await self.db.flush()
        return {"store_id": store_id, "period": period, "date": period_date.isoformat(), "leads": leads_count, "conversion_rate": conversion, "bookings": b["cnt"], "revenue_fen": b["rev"], "avg_per_table_fen": int(b["avg_table"]), "satisfaction": satisfaction}

    async def get_dashboard(self, store_id: str, period: str = "monthly", date_from: date = None, date_to: date = None) -> dict:
        """经营看板"""
    ...
    async def generate_benchmarks(self, store_id: str, period_date: date) -> list:
        ...
        for metric in metrics:
            ...
            store_row = await self.db.execute(text(f"SELECT {metric} FROM banquet_kpi_snapshots WHERE store_id = :sid AND tenant_id = :tid AND period = 'monthly' AND period_date = :d AND is_deleted = FALSE"), {"sid": store_id, "tid": self.tenant_id, "d": period_date})
            ...
            """), {"id": bid, "tid": self.tenant_id, "sid": store_id, "d": period_date, "metric": metric, "sv": float(store_val), "ba": float(brand["brand_avg"] or 0), "bb": float(brand["brand_best"] or 0)})
            results.append({"metric": metric, "store_value": float(store_val), "brand_avg": float(brand["brand_avg"] or 0), "brand_best": float(brand["brand_best"] or 0)})
        await self.db.flush()
        return results
