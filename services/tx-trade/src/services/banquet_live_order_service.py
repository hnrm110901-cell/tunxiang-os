...
class BanquetLiveOrderService:
    ...
    async def approve(self, live_order_id: str, approved_by: str) -> dict:
        ...
        result = await self.db.execute(
            text(
                "UPDATE banquet_live_orders SET status = 'approved', approved_by = :by, approved_at = :now, updated_at = :now WHERE id = :id AND tenant_id = :tid AND status = 'pending' AND is_deleted = FALSE RETURNING id"
            ),
            {"id": live_order_id, "tid": self.tenant_id, "by": approved_by, "now": now},
        )
        if not result.mappings().first():
            raise ValueError(f"现场订单不存在或已处理: {live_order_id}")
        await self.db.flush()
    ...
    async def reject(self, live_order_id: str, reason: str) -> dict:
        result = await self.db.execute(
            text(
                "UPDATE banquet_live_orders SET status = 'rejected', reject_reason = :reason, updated_at = NOW() WHERE id = :id AND tenant_id = :tid AND status = 'pending' AND is_deleted = FALSE RETURNING id"
            ),
            {"id": live_order_id, "tid": self.tenant_id, "reason": reason},
        )
        if not result.mappings().first():
            raise ValueError(f"现场订单不存在或已处理: {live_order_id}")
        await self.db.flush()
    ...
    async def fulfill(self, live_order_id: str) -> dict:
        ...
        await self.db.execute(
            text(
                "UPDATE banquet_live_orders SET status = 'fulfilled', fulfilled_at = :now, updated_at = :now WHERE id = :id AND tenant_id = :tid AND status = 'approved' AND is_deleted = FALSE"
            ),
            {"id": live_order_id, "tid": self.tenant_id, "now": now},
        )
        await self.db.flush()
    ...
    async def get_live_total(self, banquet_id: str) -> dict:
        row = await self.db.execute(
            text(
                "SELECT COALESCE(SUM(amount_fen), 0) AS total, COUNT(*) AS count FROM banquet_live_orders WHERE banquet_id = :bid AND tenant_id = :tid AND status IN ('approved','fulfilled') AND is_deleted = FALSE"
            ),
