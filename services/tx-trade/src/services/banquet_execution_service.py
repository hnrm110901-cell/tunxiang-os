...
SOP_TEMPLATES = {"wedding": WEDDING_SOP, "birthday": WEDDING_SOP, "business": WEDDING_SOP, "tour_group": TOUR_GROUP_SOP, "conference": CONFERENCE_SOP, "annual_party": WEDDING_SOP}
...
class BanquetExecutionService:
    ...
    async def create_plan(self, banquet_id: str) -> dict:
        row = await self.db.execute(text("SELECT id, store_id, event_type FROM banquets WHERE id = :bid AND tenant_id = :tid AND is_deleted = FALSE"), {"bid": banquet_id, "tid": self.tenant_id})
        ...
        """), {"id": plan_id, "tid": self.tenant_id, "bid": banquet_id, "sid": str(banquet["store_id"]), "cp": json.dumps(checkpoints, ensure_ascii=False), "total": len(checkpoints)})
        for i, cp in enumerate(checkpoints):
            ...
            """), {"id": str(uuid.uuid4()), "tid": self.tenant_id, "pid": plan_id, "idx": i, "name": cp["name"], "ctype": cp["type"], "stime": cp.get("time")})
        await self.db.flush()
    ...
    async def start_execution(self, plan_id: str) -> dict:
        ...
        result = await self.db.execute(text("UPDATE banquet_execution_plans SET status = 'executing', started_at = :now, updated_at = :now WHERE id = :id AND tenant_id = :tid AND status = 'planned' AND is_deleted = FALSE RETURNING id"), {"id": plan_id, "tid": self.tenant_id, "now": now})
        if not result.mappings().first(): raise ValueError(f"计划不存在或状态不允许: {plan_id}")
        await self.db.flush()
        return {"id": plan_id, "status": "executing"}

    async def complete_checkpoint(self, log_id: str, executor_id: str = None, executor_name: str = None, issue_note: str = None) -> dict:
        now = datetime.now(timezone.utc)
        row = await self.db.execute(text("SELECT plan_id, scheduled_time FROM banquet_execution_logs WHERE id = :id AND tenant_id = :tid AND status IN ('pending','in_progress') AND is_deleted = FALSE"), {"id": log_id, "tid": self.tenant_id})
        log = row.mappings().first()
        if not log: raise ValueError(f"检查点不存在或已完成: {log_id}")
        delay_min = 0
        ...
        """), {"id": log_id, "tid": self.tenant_id, "now": now, "delay": delay_min, "eid": executor_id, "ename": executor_name, "note": issue_note})
        await self.db.execute(text("UPDATE banquet_execution_plans SET completed_checkpoints = completed_checkpoints + 1, updated_at = :now WHERE id = :pid AND tenant_id = :tid"), {"pid": str(log["plan_id"]), "tid": self.tenant_id, "now": now})
        await self.db.flush()
        return {"id": log_id, "status": "completed", "delay_min": delay_min}

    async def get_progress(self, plan_id: str) -> dict:
        plan = await self.db.execute(text("SELECT * FROM banquet_execution_plans WHERE id = :id AND tenant_id = :tid AND is_deleted = FALSE"), {"id": plan_id, "tid": self.tenant_id})
        p = plan.mappings().first()
        if not p: raise ValueError(f"计划不存在: {plan_id}")
        logs = await self.db.execute(text("SELECT * FROM banquet_execution_logs WHERE plan_id = :pid AND tenant_id = :tid AND is_deleted = FALSE ORDER BY checkpoint_index"), {"pid": plan_id, "tid": self.tenant_id})
        return {"plan": dict(p), "logs": [dict(l) for l in logs.mappings().all()], "progress_pct": round(p["completed_checkpoints"] / max(p["total_checkpoints"], 1) * 100, 1)}

    async def escalate_checkpoint(self, log_id: str, issue_note: str) -> dict:
        await self.db.execute(text("UPDATE banquet_execution_logs SET status = 'escalated', issue_note = :note, updated_at = NOW() WHERE id = :id AND tenant_id = :tid AND status IN ('pending','in_progress') AND is_deleted = FALSE"), {"id": log_id, "tid": self.tenant_id, "note": issue_note})
        await self.db.flush()
