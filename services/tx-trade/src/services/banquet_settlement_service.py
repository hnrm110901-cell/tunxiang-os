...
class BanquetSettlementService:
    ...
    async def generate_settlement(self, banquet_id: str) -> dict:
        b = await self.db.execute(
            text("SELECT * FROM banquets WHERE id = :bid AND tenant_id = :tid AND is_deleted = FALSE"),
        ...
        live_row = await self.db.execute(
            text(
                "SELECT COALESCE(SUM(amount_fen), 0) AS total FROM banquet_live_orders WHERE banquet_id = :bid AND tenant_id = :tid AND status IN ('approved','fulfilled') AND is_deleted = FALSE"
            ),
            {"bid": banquet_id, "tid": self.tenant_id},
        )
        ...
        if live_amount > 0:
            items.append(
                {"type": "live_add", "name": "现场加菜/酒水", "qty": 1, "price": live_amount, "source": "live_order"}
            )
        for item in items:
            await self.db.execute(
                text("""
        ...
        return {
            ...
            "contract_amount_fen": contract_amount,
            "deposit_paid_fen": deposit_paid,
            "live_order_amount_fen": live_amount,
    ...
    async def finalize(self, settlement_id: str, payment_method: str, payment_ref: str = None) -> dict:
        ...
        if bid:
            await self.db.execute(
                text(
                    "UPDATE banquets SET status = 'settled', settled_at = :now, updated_at = :now WHERE id = :bid AND tenant_id = :tid AND status = 'completed'"
                ),
                {"bid": str(bid), "tid": self.tenant_id, "now": now},
            )
        await self.db.flush()
    ...
    async def get_settlement(self, settlement_id: str) -> dict:
        row = await self.db.execute(
            text("SELECT * FROM banquet_settlements WHERE id = :id AND tenant_id = :tid AND is_deleted = FALSE"),
            {"id": settlement_id, "tid": self.tenant_id},
        )
        ...
        items = await self.db.execute(
            text(
                "SELECT * FROM banquet_settlement_items WHERE settlement_id = :sid AND tenant_id = :tid AND is_deleted = FALSE"
            ),
            {"sid": settlement_id, "tid": self.tenant_id},
        )
        result = dict(s)
    ...
    async def get_by_banquet(self, banquet_id: str) -> dict | None:
        row = await self.db.execute(
            text(
                "SELECT id FROM banquet_settlements WHERE banquet_id = :bid AND tenant_id = :tid AND is_deleted = FALSE ORDER BY created_at DESC LIMIT 1"
            ),
            {"bid": banquet_id, "tid": self.tenant_id},
        )
    ...
    async def request_invoice(self, settlement_id: str) -> dict:
        await self.db.execute(
            text(
                "UPDATE banquet_settlements SET invoice_status = 'requested', updated_at = NOW() WHERE id = :id AND tenant_id = :tid AND invoice_status = 'none'"
            ),
            {"id": settlement_id, "tid": self.tenant_id},
        )
        await self.db.flush()
