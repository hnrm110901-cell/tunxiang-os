...
def test_predict_busy_periods_success():
    ...
    _pred_svc.get_busy_period_forecast = AsyncMock(return_value=[_BusyPeriod("午高峰"), _BusyPeriod("晚高峰")])
...
def test_list_all_suggestions_success():
    ...
    with patch(
        "src.api.proactive_service_routes.get_table_suggestions",
...
def test_get_order_suggestions_success():
    ...
    with patch(
        "src.api.proactive_service_routes.get_service_suggestions", new=AsyncMock(return_value=[_SuggestionStub()])
    ):
        client = TestClient(_make_proactive_app())
        resp = client.get(
...
def test_get_constraint_status_success():
    ...
    with patch(
        "src.api.proactive_service_routes.get_constraint_status", new=AsyncMock(return_value=_ConstraintStatusStub())
    ):
        client = TestClient(_make_proactive_app())
        resp = client.get(
...
def test_gift_dish_success():
    ...
    with patch(
        "src.api.order_ops_routes.ext.gift_dish", new=AsyncMock(return_value={"gifted": True, "dish_id": "dish-001"})
    ):
...
def test_split_order_success():
    ...
    with patch(
        "src.api.order_ops_routes.ext.split_order", new=AsyncMock(return_value={"orders": ["order-A", "order-B"]})
    ):
        app = _make_order_ops_app(db)
        client = TestClient(app)
        resp = client.post(
...
def test_merge_orders_value_error():
    ...
    with patch("src.api.order_ops_routes.ext.merge_orders", new=AsyncMock(side_effect=ValueError("订单不在同一桌台"))):
        app = _make_order_ops_app(db)
        client = TestClient(app)
        resp = client.post(
            "/api/v1/orders/merge",
...
def test_approve_order_change_success():
    ...
    with patch(
        "src.api.order_ops_routes.ext.approve_order_change",
...
def test_create_receiving_success():
    ...
    with (
        patch(
            "src.api.supply_chain_mobile_routes.create_receiving_order",
            new=AsyncMock(return_value={"order_id": "recv-001"}),
        ),
        patch(
            "src.api.supply_chain_mobile_routes.confirm_receiving",
        ...
        resp = client.post(
            ...
            json={
                "store_id": STORE_ID,
                "supplier_name": "天府食材",
                "items": [{"ingredient_name": "猪肉", "received_qty": "5.0"}],
            },
...
def test_receiving_history_success():
    ...
    with patch(
        "src.api.supply_chain_mobile_routes.get_receiving_history",
...
def test_start_stocktake_success():
    ...
    with patch(
        "src.api.supply_chain_mobile_routes.start_stocktake",
...
def test_approve_purchase_not_found():
    ...
    with (
        patch(
            "src.api.supply_chain_mobile_routes.approve_purchase",
            new=AsyncMock(side_effect=_PurchaseOrderNotFoundError("采购单不存在")),
        ),
        patch("src.api.supply_chain_mobile_routes.PurchaseOrderNotFoundError", _PurchaseOrderNotFoundError),
    ):
        app = _make_sc_app(db)
        client = TestClient(app)
        resp = client.post(
            "/api/v1/supply/purchase/pur-not-exist/approve",
            json={"approved": True},
