...
class TestCashierEngineUnit:
    ...
    def test_gen_order_no_format(self):
        """订单号格式: TX + 14位时间 + 4位随机"""
        from services.tx_trade.src.services.cashier_engine import _gen_order_no
...
class TestDailySettlementUnit:
    ...
    def test_settlement_status_flow(self):
        """日结状态流转规则"""
        from services.tx_trade.src.services.daily_settlement import DailySettlementService
...
class TestOrderCancelAndRefund:
    ...
    def test_cancel_releases_table(self):
        """取消订单应释放桌台"""
        # 验证状态流转逻辑
        from services.tx_trade.src.services.state_machine import sync_table_on_order_change
...
class TestStateMachineIntegration:
    ...
    def test_order_lifecycle(self):
        """订单完整生命周期"""
        from services.tx_trade.src.services.state_machine import validate_order_lifecycle
