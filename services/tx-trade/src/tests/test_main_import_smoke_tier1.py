...
（root cause: bare-import-services.permission_service）。决策 77 production 端 codemod / 缺失导出修复后
...
@pytest.mark.xfail(
    reason="bare-import-services.permission_service；决策 77 production 端 codemod 修复后翻 xfail",
    strict=False,
)
def test_main_module_loads_in_container_layout() -> None:
    """tx_trade main.py 必须在 Docker 容器布局下干净 import。"""
