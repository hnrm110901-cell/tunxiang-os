...
# 本 codebase 专用 helper：_apply_safe_rls / _enable_rls / _apply_rls
# (后者由 v407/v408/v409 chain rescue revive PR 系列引入，命名变体)
# 等价于 ENABLE RLS + CREATE POLICY；string literal 直接传入
APPLY_RLS_CALL_RE = re.compile(
    r"(?:_apply_safe_rls|_enable_rls|_apply_rls)\s*\(\s*['\"]([a-zA-Z_][a-zA-Z0-9_]*)['\"]",
)
...
APPLY_RLS_INDIRECT_RE = re.compile(r"(?:_apply_safe_rls|_enable_rls|_apply_rls)\s*\(\s*_TABLE\s*\)")
# list 循环调用：_NEW_TABLES = [...] + for t in _NEW_TABLES: _apply_rls(t)
# (v057 / v346 / v408 模式)
NEW_TABLES_LIST_RE = re.compile(
    r"^_NEW_TABLES\s*=\s*\[(.*?)\]",
    re.DOTALL | re.MULTILINE,
)
APPLY_RLS_LIST_LOOP_RE = re.compile(
    r"for\s+\w+\s+in\s+_NEW_TABLES\s*:\s*[^\n]*\n\s*(?:_apply_safe_rls|_enable_rls|_apply_rls)\s*\(",
)
LIST_STRING_LITERAL_RE = re.compile(r"['\"]([a-zA-Z_][a-zA-Z0-9_]*)['\"]")
...
def _scan_migration(source: str) -> tuple[set[str], set[str], set[str]]:
    ...
    2. 直接 helper 调用：_apply_safe_rls("name") / _enable_rls("name") / _apply_rls("name")
    3. 间接 helper 调用：_TABLE = "name" + _apply_safe_rls(_TABLE)（v374/v375 模式）
    4. list 循环调用：_NEW_TABLES = [...] + for t in _NEW_TABLES: _apply_rls(t)
       (v057 / v346 / v408 模式)
    """
    ...
    # 模式2：_apply_safe_rls("name") / _enable_rls("name") / _apply_rls("name")
    ...
    if APPLY_RLS_INDIRECT_RE.search(source):
        for m in TABLE_CONST_RE.finditer(source):
            ...
            rls_enabled.add(table)
            policies.add(table)
    # 模式4：_NEW_TABLES = [...] + for X in _NEW_TABLES: _apply_rls(X)
    list_match = NEW_TABLES_LIST_RE.search(source)
    if list_match and APPLY_RLS_LIST_LOOP_RE.search(source):
        for m in LIST_STRING_LITERAL_RE.finditer(list_match.group(1)):
            table = m.group(1).lower()
            rls_enabled.add(table)
            policies.add(table)
