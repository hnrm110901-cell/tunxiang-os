...
echo ""
echo "====== 下一步 ======"
echo "1. 将 $ENV_FILE 添加到 infra/compose/envs/prod.yml 的 env_file 配置"
echo "2. 或执行: source $ENV_FILE && docker compose -f infra/compose/base.yml -f infra/compose/envs/prod.yml up -d"
echo "3. 运行数据库迁移: docker compose -f infra/compose/base.yml -f infra/compose/envs/prod.yml exec gateway alembic upgrade head"
echo "4. PII数据迁移（加密现有手机号）: docker compose -f infra/compose/base.yml -f infra/compose/envs/prod.yml exec gateway python scripts/migrate_pii_encryption.py"
echo ""
echo "====== 完成 ======"
