...
const StoreManagementPage: React.FC = () => {
  ...
  const loadStores = useCallback(async () => {
    ...
    try {
      ...
      if (storesRes.status === 'fulfilled') setStores(storesRes.value.stores || storesRes.value || []);
      if (countRes.status === 'fulfilled') setStoreCount(countRes.value);
    } catch (err: any) {
  ...
  const viewStats = async (record: any) => {
    ...
    try {
      ...
      setStoreStats({ ...res, name: record.name });
