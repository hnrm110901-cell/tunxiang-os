...
const AIEvolutionPage: React.FC = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (adoptionRes.status === 'fulfilled') setAdoptionMetrics(adoptionRes.value || []);
      if (trendRes.status === 'fulfilled') setWeeklyTrend(trendRes.value || []);
      if (perfRes.status === 'fulfilled') setAgentPerformance(perfRes.value || []);
      if (hitlRes.status === 'fulfilled') setHitlEscalations(hitlRes.value || []);
    } catch (err) {
      handleApiError(err);
    } finally {
      setLoading(false);
