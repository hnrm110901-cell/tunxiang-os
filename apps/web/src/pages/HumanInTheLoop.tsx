...
const HumanInTheLoop: React.FC = () => {
  ...
  const loadTrustData = useCallback(async () => {
    ...
    try {
      ...
      if (phase.status === 'fulfilled') setTrustPhase(phase.value);
      if (metrics.status === 'fulfilled') setTrustMetrics(metrics.value);
      if (rules.status === 'fulfilled') setRiskRules(rules.value?.rules || rules.value || []);
    } catch (err: any) {
