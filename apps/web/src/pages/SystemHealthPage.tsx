...
const SystemHealthPage: React.FC = () => {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      if (agentsRes.status === 'fulfilled') setAgents(agentsRes.value?.agents || agentsRes.value || []);
      if (externalRes.status === 'fulfilled') setExternalSystems(externalRes.value?.systems || externalRes.value || []);
      if (readyRes.status === 'fulfilled') setReadyStatus(readyRes.value?.status || 'ok');
      if (liveRes.status === 'fulfilled') setLiveStatus(liveRes.value?.status || 'ok');
    } catch (err) {
