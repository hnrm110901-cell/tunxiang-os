...
const BusinessEventsPage: React.FC = () => {
  ...
  const waterfallOption = useMemo(() => {
    ...
    const items = [
      { name: '销售收入',   value: revenue.gross_revenue_yuan,       color: '#52c41a' },
      { name: '退款',       value: -revenue.refund_yuan,             color: '#f5222d' },
      { name: '食材成本',   value: -costs.food_cost_yuan,            color: '#fa8c16' },
      { name: '损耗',       value: -costs.waste_cost_yuan,           color: '#fa541c' },
      { name: '平台抽佣',   value: -costs.platform_commission_yuan,  color: '#722ed1' },
      { name: '人工费用',   value: -costs.labor_cost_yuan,           color: '#1890ff' },
      { name: '其他费用',   value: -costs.other_expense_yuan,        color: '#13c2c2' },
      { name: '毛利润',     value: profit.gross_profit_yuan,         color: profit.gross_profit_yuan >= 0 ? '#FF6B2C' : '#f5222d' },
    ];
    return {
      tooltip: {
        trigger: 'axis',
