...
const OpsAgentPage: React.FC = () => {
  ...
  const loadHealth = useCallback(async () => {
    ...
    try {
      ...
      setHealthResult(res?.data?.check_advice || '');
  ...
  const loadAssets = useCallback(async () => {
    try {
      ...
      setAssetAdvice(res?.data?.asset_advice || '');
    } catch { /* silent */ }
  }, [selectedStore]);
  ...
  const handleRunbook = async (values: any) => {
    ...
    try {
      ...
      setRunbookResult(res?.data?.runbook || '');
  ...
  const handleMaintenance = async () => {
    ...
    try {
      ...
      setMaintenanceResult(res?.data?.maintenance_advice || '');
  ...
  const handleSecurity = async () => {
    ...
    try {
      ...
      setSecurityResult(res?.data?.security_advice || '');
