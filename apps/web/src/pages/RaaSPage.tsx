...
const RaaSPage: React.FC = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (baseRes.status === 'fulfilled') setBaseline(baseRes.value);
      if (effectRes.status === 'fulfilled') setEffectMetrics(effectRes.value);
      if (billRes.status === 'fulfilled') setMonthlyBill(billRes.value);
      if (valueRes.status === 'fulfilled') setValueProposition(valueRes.value);
    } catch (err: any) {
