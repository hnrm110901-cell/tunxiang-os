...
const ModelMarketplacePage: React.FC = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (modelsRes.status === 'fulfilled') setModels(modelsRes.value?.models || modelsRes.value || []);
      if (myRes.status === 'fulfilled') setMyModels(myRes.value?.models || myRes.value || []);
      if (netRes.status === 'fulfilled') setNetworkEffect(netRes.value);
    } catch (err: any) {
