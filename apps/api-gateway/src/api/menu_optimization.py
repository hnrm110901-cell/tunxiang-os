...
async def list_recommendations(
    ...
    recs = await get_menu_recommendations(db, store_id, period,
                                          rec_type=rec_type, status=status, limit=limit)
...
async def meta_types():
    return {
        "rec_types": [
            {"key": rt, "label": REC_LABELS[rt], "title": REC_TITLES[rt], "action": REC_ACTIONS[rt]}
            for rt in REC_TYPES
        ]
    }
