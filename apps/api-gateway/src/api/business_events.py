...
async def get_event_stream(
    ...
    params = {
        "store_id": store_id, "df": date_from, "dt": date_to,
        "etype": event_type, "status": status,
        "limit": limit, "offset": offset,
    }
...
async def get_event_stats(
    ...
    rows = (await db.execute(text("""
        ...
        GROUP BY event_type
        ORDER BY event_type
    """), {"store_id": store_id, "period": period})).fetchall()
    ...
    total_cost  = sum(
        by_type.get(t, {}).get("total_yuan", 0.0)
        for t in ("purchase", "waste", "expense", "settlement")
    )
...
async def create_mapping_rule(
    ...
    """), {"sys": body.source_system, "set": body.source_event_type})).fetchone()
    ...
    if existing:
        ...
        await db.commit()
        return {"action": "updated", "source_system": body.source_system,
...
async def reprocess_event(
    ...
    return {"event_id": event_id, "store_id": row.store_id,
            "period": row.period, "queued": True}
...
async def get_profit_attribution(
    ...
    """), {"sid": store_id, "period": period, "today": today_str})).fetchone()
