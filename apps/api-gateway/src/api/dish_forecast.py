...
async def generate(
    ...
    幂等，全量覆盖。
    """
    return await generate_dish_forecasts(db, store_id, base_period,
...
async def list_forecasts(
    ...
    if lifecycle_phase and lifecycle_phase not in _VALID_PHASES:
        raise HTTPException(status_code=400,
                            detail=f"lifecycle_phase 必须是 {_VALID_PHASES} 之一")
    recs = await get_dish_forecasts(db, store_id, forecast_period,
                                     lifecycle_phase=lifecycle_phase, limit=limit)
    return {'store_id': store_id, 'forecast_period': forecast_period,
            'count': len(recs), 'forecasts': recs}
...
async def accuracy(
    ...
    return {'store_id': store_id, 'forecast_period': forecast_period,
            'count': len(results), 'accuracy': results}
...
async def meta_model():
    return {
        ...
        'lifecycle_adjustments': {
            k: f'{v*100:+.0f}%' for k, v in LIFECYCLE_ADJUSTMENT.items()
        },
        'confidence_interval': {
