...
from ..services.settlement_risk_service import (
...
async def get_record(
    ...
    row = (await db.execute(text(
        "SELECT * FROM settlement_records WHERE id = :id"
...
async def verify_record(
    ...
    row = (await db.execute(text(
        "SELECT id, status FROM settlement_records WHERE id = :id"
    ), {"id": record_id})).fetchone()
    if not row:
...
async def add_settlement_item(
    ...
    """添加结算明细行项"""
    row = (await db.execute(text(
        "SELECT id, store_id FROM settlement_records WHERE id = :id"
    ), {"id": record_id})).fetchone()
    if not row:
        raise HTTPException(404, f"Record {record_id} not found")
    ...
    """), {
        "id": iid, "sid": record_id, "store": row.store_id,
        "type": body.item_type, "desc": body.item_desc,
        "amt":  body.amount_yuan, "ref": body.ref_event_id,
    })
    await db.commit()
...
async def list_risk_tasks(
    ...
    if severity and risk_type and status:
        rows = (await db.execute(text("""
            ...
            LIMIT :limit
        """), {"sid": store_id, "sev": severity, "rt": risk_type, "st": status, "limit": limit})).fetchall()
        ...
        rows = (await db.execute(text("""
            ...
            ORDER BY CASE severity WHEN 'critical' THEN 0 WHEN 'high' THEN 1
                     WHEN 'medium' THEN 2 ELSE 3 END, created_at DESC
            LIMIT :limit
        """), {"sid": store_id, "sev": severity, "rt": risk_type, "limit": limit})).fetchall()
    elif severity:
        rows = (await db.execute(text("""
            SELECT * FROM risk_tasks
            ...
            ORDER BY CASE severity WHEN 'critical' THEN 0 WHEN 'high' THEN 1
                     WHEN 'medium' THEN 2 ELSE 3 END, created_at DESC
            LIMIT :limit
        """), {"sid": store_id, "sev": severity, "limit": limit})).fetchall()
    elif status:
        rows = (await db.execute(text("""
            SELECT * FROM risk_tasks
            ...
            ORDER BY CASE severity WHEN 'critical' THEN 0 WHEN 'high' THEN 1
                     WHEN 'medium' THEN 2 ELSE 3 END, created_at DESC
            LIMIT :limit
        """), {"sid": store_id, "st": status, "limit": limit})).fetchall()
    else:
        rows = (await db.execute(text("""
            SELECT * FROM risk_tasks
            WHERE store_id = :sid
...
async def resolve_risk_task(
    ...
    row = (await db.execute(text(
        "SELECT id, status FROM risk_tasks WHERE id = :id"
    ), {"id": task_id})).fetchone()
    if not row:
...
async def get_settlement_summary(
    ...
    # 结算汇总
    settle_row = (await db.execute(text("""
