...
async def api_compute(
    ...
    target_fcr_reduction: float = Query(2.0, ge=0.5, le=10.0,
                                         description='目标降低几个百分点（默认2pp）'),
    db: AsyncSession = Depends(get_db),
):
    """触发成本压缩机会计算并写入数据库（幂等）。"""
    result = await compute_cost_compression(db, store_id, period,
                                             target_fcr_reduction=target_fcr_reduction)
...
async def api_get_compression(
    ...
    action: Optional[str] = Query(None,
        description='renegotiate/reformulate/adjust_portion/monitor'),
    ...
    rows = await get_cost_compression(db, store_id, period,
                                       action=action, priority=priority, limit=limit)
    ...
    return {
        ...
        'data': [
            {'value': 'renegotiate',    'label': '重新谈判',
             'description': 'FCR超标>5pp且持续恶化，与供应商重新谈判食材采购价'},
            {'value': 'reformulate',    'label': '调整配方',
             'description': 'FCR超标3-5pp，优化配方、替换原料或调整用量比例'},
            {'value': 'adjust_portion', 'label': '调整份量',
             'description': 'FCR超标1-3pp，微调标准份量或精细化称重管控'},
            {'value': 'monitor',        'label': '持续监控',
             'description': 'FCR已达目标或缺口<1pp，维持现状定期复查'},
        ],
    }
