...
async def webhook_bad_review(
    ...
    try:
        ...
        logger.error("signal_bus_api.bad_review_failed",
...
async def trigger_near_expiry(
    ...
    try:
        ...
        logger.error("signal_bus_api.near_expiry_failed",
                     store_id=store_id, error=str(exc))
        raise HTTPException(status_code=500, detail=str(exc))
...
async def route_large_table(
    ...
    try:
        ...
        logger.error("signal_bus_api.large_table_failed",
                     store_id=store_id, error=str(exc))
        raise HTTPException(status_code=500, detail=str(exc))
...
async def manual_scan(
    ...
    try:
        ...
        logger.error("signal_bus_api.manual_scan_failed",
                     store_id=store_id, error=str(exc))
        raise HTTPException(status_code=500, detail=str(exc))
