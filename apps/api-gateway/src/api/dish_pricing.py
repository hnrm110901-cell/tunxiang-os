...
async def list_recommendations(
    ...
    if rec_action and rec_action not in REC_ACTIONS:
        raise HTTPException(status_code=400,
                            detail=f"rec_action 必须是 {REC_ACTIONS} 之一")
    if status and status not in STATUSES:
        raise HTTPException(status_code=400,
                            detail=f"status 必须是 {STATUSES} 之一")
    recs = await get_pricing_recommendations(
        db, store_id, period, rec_action=rec_action, status=status, limit=limit
    )
...
async def adopt(
    ...
    """将定价建议标记为已采纳（可传实际定价）。"""
    result = await update_pricing_status(db, rec_id, 'adopt',
                                          adopted_price=adopted_price)
    if not result['updated']:
        raise HTTPException(status_code=404,
                            detail=result.get('reason', 'update_failed'))
    return result
