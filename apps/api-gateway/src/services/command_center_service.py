...
class CommandCenterService:
    ...
    async def get_event_stream(
        self, db: AsyncSession, brand_id: str, limit: int = 50
    ) -> List[Dict[str, Any]]:
        """从多张表取最新事件，合并排序后返回。"""
    ...
    async def get_kpi_matrix(
        ...
        # ── 运营 KPI ──
        completed_count = await self._order_count_by_status(
            db, brand_id, today_start, OrderStatus.COMPLETED.value
        )
        total_count = await self._order_count_total(db, brand_id, today_start)
        fulfillment_rate = (
            round(completed_count / total_count * 100, 1) if total_count else 100.0
        )
    ...
    async def get_system_pulse(self, db: AsyncSession) -> Dict[str, Any]:
        ...
        # 品牌数
        brand_count_q = await db.execute(
            select(func.count(func.distinct(Store.brand_id))).where(
        ...
        store_count_q = await db.execute(
            select(func.count(Store.id)).where(Store.is_active.is_(True))
        )
        ...
        integration_summary = {
            row[0]: row[1] for row in integration_q.all()
        }

        # 最近系统错误（集成层）
        recent_errors_q = await db.execute(
            select(IntegrationHubStatus).where(
                IntegrationHubStatus.last_error_at.isnot(None)
            ).order_by(desc(IntegrationHubStatus.last_error_at)).limit(5)
        )
        recent_errors = [
            {
    ...
    async def _order_stats_today(
        self, db: AsyncSession, brand_id: str, today_start: datetime
    ) -> Dict[str, Any]:
        """今日订单统计：营收/数量/客单价"""
        stmt = select(
        ...
        ).join(Store, Store.id == Order.store_id).where(
    ...
    async def _latest_compliance(
        ...
        stmt = (
            ...
            .where(
                ComplianceScore.score_date == select(
                    func.max(ComplianceScore.score_date)
                ).where(ComplianceScore.brand_id == brand_id).correlate(None).scalar_subquery()
            )
        )
        row = (await db.execute(stmt)).one()
        avg_score = round(float(row.avg_score)) if row.avg_score else 0
        # 推导评级
        grade = "A+" if avg_score >= 95 else (
    ...
    async def _pending_procurement(
        self, db: AsyncSession, brand_id: str
    ) -> int:
        """待执行的采购规则数"""
        stmt = select(func.count(ProcurementRule.id)).where(
            ProcurementRule.brand_id == brand_id
        )
        return (await db.execute(stmt)).scalar() or 0

    async def _unread_review_count(
        self, db: AsyncSession, brand_id: str
    ) -> int:
        """未回复评论数"""
    ...
    async def _unresolved_reconciliation(
        self, db: AsyncSession, brand_id: str
    ) -> int:
        """有差异的对账记录数"""
        stmt = (
    ...
    async def _recent_integration_events(
        self, db: AsyncSession, cutoff: datetime, limit: int
    ) -> List[Dict[str, Any]]:
        stmt = (
        ...
        events = []
        for r in rows:
            severity = "info" if r.status == "healthy" else (
                "critical" if r.status == "error" else "warning"
            )
            events.append({
    ...
    async def _order_count_by_status(
        self, db: AsyncSession, brand_id: str,
        since: datetime, status: str
    ) -> int:
        stmt = (
    ...
    async def _order_count_total(
        self, db: AsyncSession, brand_id: str, since: datetime
    ) -> int:
        stmt = (
            select(func.count(Order.id))
            .join(Store, Store.id == Order.store_id)
    ...
    async def _integration_kpi(self, db: AsyncSession) -> Dict[str, Any]:
        ...
        success_rate = (
            round((total_syncs - total_errors) / total_syncs * 100, 1)
            if total_syncs else 100.0
        )
        return {
    ...
    async def _dispatch_sync_all(
        self, db: AsyncSession, brand_id: str, params: Dict[str, Any]
    ) -> Dict[str, Any]:
        """触发全量同步：重置今日同步计数并标记为"同步中"。"""
        stmt = (
            select(IntegrationHubStatus)
            .where(IntegrationHubStatus.status.in_(["healthy", "degraded", "error"]))
        )
        rows = (await db.execute(stmt)).scalars().all()
        triggered = 0
        for row in rows:
    ...
    async def _dispatch_run_closing(
        self, db: AsyncSession, brand_id: str, params: Dict[str, Any]
    ) -> Dict[str, Any]:
        """触发日结：统计今日营收数据。"""
        today_start = datetime.combine(date.today(), datetime.min.time())
    ...
    async def _dispatch_check_procurement(
        self, db: AsyncSession, brand_id: str, params: Dict[str, Any]
    ) -> Dict[str, Any]:
        """检查采购规则，返回待执行数量。"""
    ...
    async def _dispatch_generate_alerts(
        self, db: AsyncSession, brand_id: str, params: Dict[str, Any]
    ) -> Dict[str, Any]:
        """扫描合规风险并返回告警数。"""
