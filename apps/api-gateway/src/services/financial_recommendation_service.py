...
async def _upsert_recommendation(
    ...
    await db.execute(
        ...
        """),
        {
            ...
            "rtype": rec["rec_type"], "metric": rec["metric"],
            "title": rec["title"], "desc": rec.get("description", ""),
            "action": rec.get("action", ""),
            "impact": rec.get("expected_yuan_impact"),
            "conf": rec["confidence_pct"], "urgency": rec["urgency"],
            "score": rec["priority_score"],
            "stype": rec.get("source_type"), "sref": rec.get("source_ref"),
            "now": now,
        },
    )
