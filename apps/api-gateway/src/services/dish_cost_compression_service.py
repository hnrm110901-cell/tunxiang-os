...
def compute_target_fcr(store_avg_fcr: float,
                        reduction_pp: float = 2.0) -> float:
...
def compute_compression_opportunity(revenue_yuan: float,
                                     fcr_gap: float) -> float:
...
def compute_expected_saving(compression_opportunity: float,
                             months: int = _MONTHS_PER_YEAR) -> float:
    """年化预期节省（默认 ×12）。"""
    return round(compression_opportunity * months, 2)


def classify_fcr_trend(current_fcr: float,
                        prev_fcr: Optional[float]) -> str:
    """
...
def determine_priority(compression_opportunity: float,
                        fcr_trend: str) -> str:
    """
    ...
    low   : 其他
    """
    if (compression_opportunity > 1000.0 or
            (compression_opportunity > 500.0 and fcr_trend == 'worsening')):
        return 'high'
...
async def compute_cost_compression(
    ...
    prev_fcr_map: dict[str, float] = {
        r[0]: float(r[5] or 0) for r in prev_rows if r[5] is not None
    }
    ...
    if not curr_rows:
        ...
        return {
            ...
            'dish_count': 0, 'store_avg_fcr': 0.0, 'target_fcr': 0.0,
            'total_opportunity_yuan': 0.0, 'total_expected_saving_yuan': 0.0,
            'action_counts': {}, 'priority_counts': {}, 'worsening_count': 0,
        }
    ...
    for r in curr_rows:
        ...
        rec = build_compression_record(
            store_id, period,
            dish_id, r[1], r[2],
            float(r[4] or 0), int(r[3] or 0),
            current_fcr, current_gpm,
            target_fcr, store_avg_fcr,
        ...
        action_counts[rec['compression_action']] = \
            action_counts.get(rec['compression_action'], 0) + 1
        priority_counts[rec['action_priority']] = \
            priority_counts.get(rec['action_priority'], 0) + 1
...
async def get_cost_compression(
    ...
    if action:
        sql = text(f"SELECT {_cols} {base} AND compression_action = :action "
...
async def get_compression_summary(db: AsyncSession, store_id: str,
                                   period: str) -> dict:
    """按行动类型和趋势聚合统计。"""
    sql_action = text("""
        SELECT
...
async def get_top_opportunities(db: AsyncSession, store_id: str,
                                 period: str, limit: int = 10) -> list[dict]:
    """压缩机会最大的 Top-N 菜品。"""
    sql = text("""
        SELECT
...
async def get_dish_fcr_history(db: AsyncSession, store_id: str,
                                dish_id: str,
