...
def classify_lifecycle_phase(bcg_quadrant: str, revenue_trend_pct: float,
                               order_trend_pct: float, is_new: bool) -> str:
    """
    ...
    # 退出：dog 且营收销量双双暴跌
    if (bcg_quadrant == 'dog'
            and revenue_trend_pct <= EXIT_REVENUE_THRESHOLD
            and order_trend_pct  <= EXIT_ORDER_THRESHOLD):
        return 'exit'
...
def compute_phase_duration(current_phase: str, prev_phase: Optional[str],
                            prev_duration: int) -> int:
    """
    当前处于本阶段的连续月数。
...
def compute_lifecycle_impact(phase: str, revenue_yuan: float,
                              revenue_trend_pct: float) -> float:
    """
    估算采取阶段建议动作的¥潜在收益。
...
def compute_lifecycle_confidence(phase: str, phase_duration_months: int,
...
def build_lifecycle_record(store_id: str, period: str,
                            current: dict, prev: Optional[dict],
                            prev_lifecycle: Optional[dict]) -> dict:
    """
    为单道菜构造完整生命周期记录。
...
async def _fetch_prev_lifecycles(db: AsyncSession, store_id: str,
    ...
    return {
        r[0]: {'phase': r[1], 'phase_duration_months': int(r[2] or 1)}
        for r in rows
    }
...
async def compute_lifecycle_analysis(db: AsyncSession, store_id: str,
                                      period: str) -> dict:
    """
    门店当期生命周期分析主入口。幂等。
    ...
    if not current_list:
        ...
        return {
            ...
            'dish_count': 0, 'phase_counts': {}, 'transition_count': 0,
...
async def get_lifecycle_records(db: AsyncSession, store_id: str, period: str,
...
async def get_phase_transition_alerts(db: AsyncSession, store_id: str,
                                       period: str) -> list[dict]:
...
async def get_dish_lifecycle_history(db: AsyncSession, store_id: str, dish_id: str,
    ...
    cols = [
        'period', 'bcg_quadrant', 'phase', 'prev_phase', 'phase_changed',
        'phase_duration_months', 'revenue_yuan', 'order_count',
        'revenue_trend_pct', 'order_trend_pct',
        'action_label', 'expected_impact_yuan',
    ]
