...
def classify_price_elasticity(bcg_quadrant: str, order_count: int,
                               store_avg_order_count: float) -> str:
    """
    基于 BCG 象限和销量判断需求弹性。
...
def compute_demand_change(current_orders: int, price_change_pct: float,
                           elasticity_class: str) -> float:
    """预测价格变动后的期望销量。"""
...
def compute_revenue_delta(current_price: float, suggested_price: float,
                           current_orders: int, expected_orders: float) -> float:
    """预期营收变化 = 新营收 - 旧营收。"""
...
def compute_pricing_confidence(bcg_quadrant: str, order_count: int,
                                rec_action: str) -> float:
    """
...
def build_pricing_record(store_id: str, period: str, dish: dict,
                          store_avg_order_count: float) -> dict:
    """
    为单道菜构造完整的定价建议记录。
    ...
    rev_delta     = compute_revenue_delta(current_price, rec['suggested_price'],
                                          order_count, expected_cnt)
...
async def _fetch_dishes_for_pricing(db: AsyncSession, store_id: str,
...
async def generate_pricing_recommendations(db: AsyncSession, store_id: str,
                                            period: str) -> dict:
    """
    ...
    if not dishes:
        ...
        return {
            ...
            'dish_count': 0, 'increase_count': 0,
            'decrease_count': 0, 'maintain_count': 0,
...
async def get_pricing_recommendations(db: AsyncSession, store_id: str, period: str,
    ...
    if rec_action and status:
        ...
        params = {'store_id': store_id, 'period': period,
                  'rec_action': rec_action, 'status': status, 'limit': limit}
    elif rec_action:
        sql = text("""
            SELECT id, dish_id, dish_name, category, bcg_quadrant,
                   current_price, order_count, revenue_yuan,
        ...
        """)
        params = {'store_id': store_id, 'period': period,
                  'rec_action': rec_action, 'limit': limit}
    elif status:
        sql = text("""
            SELECT id, dish_id, dish_name, category, bcg_quadrant,
                   current_price, order_count, revenue_yuan,
...
async def get_pricing_summary(db: AsyncSession, store_id: str, period: str) -> dict:
    ...
    return {
        ...
        'adoption_rate':         round(total_adopted / total_dishes * 100, 1)
                                 if total_dishes else 0.0,
        'by_action':             by_action,
...
async def update_pricing_status(db: AsyncSession, rec_id: int,
                                 action: str,
                                 adopted_price: Optional[float] = None) -> dict:
    """
...
async def get_pricing_history(db: AsyncSession, store_id: str, dish_id: str,
    ...
    cols = [
        'period', 'current_price', 'suggested_price', 'price_change_pct',
        'rec_action', 'elasticity_class',
        'expected_revenue_delta_yuan', 'expected_profit_delta_yuan',
        'confidence_pct', 'status', 'adopted_price',
    ]
