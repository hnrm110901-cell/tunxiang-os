...
async def _all_store_ids(db: AsyncSession) -> List[str]:
    try:
        rows = (await db.execute(
            text("SELECT id FROM stores WHERE is_active = true ORDER BY id")
        )).fetchall()
...
async def _store_health(store_id: str, db: AsyncSession) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "store_id": store_id, "total_score": 0,
            "grade": {"level": "未知", "color": "default"},
            "dimensions": [], "top_actions": [],
        }
    ...
    if not dry_run:
        try:
            ...
            if wechat_service:
                ...
                pushed = True
                logger.info("hq_briefing.pushed",
                            stores=briefing["store_count"],
