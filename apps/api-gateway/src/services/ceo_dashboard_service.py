...
async def get_ceo_dashboard(
    ...
    pending_l2 = 0
    try:
        pending_l2 = (await db.execute(text("""
...
async def get_region_dashboard(
    ...
    # 利润对比
    profit_rows = (await db.execute(text("""
        ...
        ORDER BY gross_profit_yuan DESC
    """), {"period": period, "sids": store_ids})).fetchall()
    ...
    if stores_data:
        summary = {
            ...
            "avg_margin_pct":     round(
                sum(s["profit_margin_pct"] for s in stores_data) / len(stores_data), 2
            ),
    ...
    risk_rows = (await db.execute(text("""
        ...
        GROUP BY store_id
    """), {"sids": store_ids})).fetchall()
    risk_by_store = {r.store_id: {"open": r.open_count, "high": r.high_count}
                     for r in risk_rows}
