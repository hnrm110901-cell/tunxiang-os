...
def build_dish_alerts(
    ...
    def _make(alert_type: str, severity: str,
              current_val: float, prev_val: float,
              change_pp: float, yuan_impact: float) -> dict:
        return {
            'store_id':          store_id,
            'period':            period,
            'dish_id':           dish_id,
            ...
            'message':           generate_alert_message(
                alert_type, dish_name,
                current_val, prev_val, change_pp, yuan_impact,
            ),
        }
    ...
    if fcr_change >= FCR_SPIKE_INFO:
        ...
        alerts.append(_make(
            'fcr_spike', classify_fcr_severity(fcr_change),
            curr_fcr, prev_fcr, fcr_change, yuan_imp,
        ))
    ...
    if gpm_drop >= MARGIN_DROP_INFO:
        ...
        alerts.append(_make(
            'margin_drop', classify_margin_severity(gpm_drop),
            curr_gpm, prev_gpm, gpm_drop, yuan_imp,
        ))
...
async def _fetch_period_records(
    ...
    for row in rows:
        ...
        for k in ('food_cost_rate', 'gross_profit_margin',
                  'revenue_yuan', 'gross_profit_yuan', 'avg_selling_price'):
...
async def generate_dish_cost_alerts(
    db: AsyncSession, store_id: str, period: str
) -> dict:
...
async def get_dish_cost_alerts(
    ...
    if severity is not None and status is not None:
        ...
        params = {'sid': store_id, 'period': period,
                  'sev': severity, 'st': status, 'lim': limit}
    elif severity is not None:
        sql = text(
            _ALERT_SELECT
            + "WHERE store_id=:sid AND period=:period AND severity=:sev "
...
async def get_alert_summary(
    ...
    by_type: dict[str, dict] = {
        t: {'count': 0, 'total_impact': 0.0, 'open': 0, 'resolved': 0}
    ...
    for row in rows:
        at, sev, st, cnt, impact = (
            row[0], row[1], row[2], int(row[3]), _to_float(row[4])
        )
        if at in by_type:
...
async def get_store_cost_trend(
    db: AsyncSession, store_id: str, period: str, periods: int = 6
) -> list[dict]:
    ...
    rows = result.fetchall()
    keys = ['period', 'dish_count', 'avg_fcr', 'avg_gpm',
...
async def get_dish_alert_history(
