...
async def _fetch_budget_data(
    db: AsyncSession, store_id: str, period: str, actual_revenue: float,
) -> tuple[float, float]:
...
async def compute_health_score(
    ...
    if existing_row:
        ...
        """), {
            ...
            "p": p_score, "c": c_score, "t": t_score, "s": s_score, "b": b_score,
            "margin": margin_pct, "rev": net_revenue, "gap": cash_gap,
            "tax_dev": avg_tax_dev, "hr_sr": high_risk_sr, "bud_ach": budget_ach_pct,
            "now": now, "sid_id": score_id,
        })
    else:
        ...
        """), {
            "id": score_id, "sid": store_id, "period": period,
            "total": total, "grade": grade,
            "p": p_score, "c": c_score, "t": t_score, "s": s_score, "b": b_score,
            "margin": margin_pct, "rev": net_revenue, "gap": cash_gap,
            "tax_dev": avg_tax_dev, "hr_sr": high_risk_sr, "bud_ach": budget_ach_pct,
    ...
    insight_list = generate_insights(
        ...
        scores={
            "profit_score": p_score, "cash_score": c_score,
            "tax_score": t_score, "settlement_score": s_score, "budget_score": b_score,
        },
    )
    for ins in insight_list:
        ...
        """), {
            "id": str(uuid.uuid4()), "sid": store_id, "period": period,
            "itype": ins["insight_type"], "priority": ins["priority"],
            "content": ins["content"], "now": now,
        })
...
async def get_health_score(
    ...
    if not row:
        return None
    keys = ["id", "store_id", "period", "total_score", "grade",
            "profit_score", "cash_score", "tax_score", "settlement_score", "budget_score",
            "profit_margin_pct", "net_revenue_yuan", "cash_gap_days",
            "avg_tax_deviation_pct", "high_risk_settlement", "budget_achievement_pct",
            "computed_at"]
...
async def get_health_trend(
    db: AsyncSession, store_id: str, periods: int = 6,
) -> List[Dict]:
...
async def get_profit_trend(
    ...
    res = await db.execute(text("""
        ...
        ORDER BY period DESC LIMIT :n
    """), {"sid": store_id, "n": periods})
    keys = ["period", "net_revenue_yuan", "gross_profit_yuan", "profit_margin_pct",
            "food_cost_yuan", "total_cost_yuan"]
    rows = [dict(zip(keys, r)) for r in res.fetchall()]
...
async def get_brand_health_summary(
    db: AsyncSession, brand_id: Optional[str], period: str,
) -> Dict:
    ...
    keys = ["store_id", "total_score", "grade", "profit_score", "cash_score",
            "net_revenue_yuan", "profit_margin_pct"]
