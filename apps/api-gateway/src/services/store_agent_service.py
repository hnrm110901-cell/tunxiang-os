...
class StoreAgentService:
    ...
    async def get_store_scorecard(
        ...
        suggestions = self._generate_suggestions(
            growth_rate, cost_rate, s1_s2_rate, turnover, margin,
        )
        ...
        return {
            ...
            "status": status,
            "dimensions": {
                "revenue": {"score": round(rev_score, 1), "growth_rate": round(growth_rate, 4), "current_yuan": round(current_rev, 2)},
    ...
    async def _get_revenue(
        ...
        result = await db.scalar(
            select(func.coalesce(func.sum(Order.total_amount), 0)).where(and_(*where))
        )
