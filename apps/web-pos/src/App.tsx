...
function AppLayout() {
  ...
  useEffect(() => {
    if (!STORE_ID) return;  // storeId 未注入时跳过（开发环境）

    let cancelled = false;
    const poll = async () => {
      try {
        const status = await fetchTrainingModeStatus(STORE_ID);
        if (!cancelled && status.is_demo_mode) {
          setServerWatermarkText(status.watermark_text || '演示模式');
        }
      } catch {
        // 网络失败时静默降级，不影响收银
      }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#111827' }}>
      ...
      {/* 训练模式全屏水印 — 叠加在所有内容之上，不干扰触控 */}
      {isTrainingMode && (
        <TrainingModeWatermark text={serverWatermarkText} />
      )}
