...
import com.tunxiang.pos.data.remote.TableResponse
...
private fun TableResponse.toLocal(storeId: String): LocalTableState {
    return LocalTableState(
        ...
        openedAt = opened_at?.let {
            try { SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss", Locale.US).parse(it)?.time }
