...
| 关键操作按钮 | 72×72px | 确认支付/完成出餐 / KDS 完成出餐 / KDS 加急 |
| 按钮间距 | ≥12px | 防误触 |
| 滑动触发阈值 | 30px | 区分点击和滑动 |

### KDS 终端字体强制规则（2026-05 修订）

```
桌号/订单号：    ≥ 32px 粗体（厨师3米外看）
区域/档口标题：  ≥ 28px 粗体
菜品行：         ≥ 20px
倒计时数字：     ≥ 32px 粗体
状态徽标(VIP)：  ≥ 16px
...
KDS 屏幕通常挂墙 1.5–2.5 米，操作员视距 1–3 米，戴老花镜或戴手套是常态。任何低于此规则的字体在 CI lint 中拒绝合并。
