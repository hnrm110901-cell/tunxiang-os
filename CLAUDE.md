# CLAUDE.md — 屯象OS 项目宪法 V3.1（务实混合架构）

> 本文件是 Claude Code 的开发行为准则。所有代码生成、架构决策、重构操作必须遵守本文件。
>
> **V3.1 同步（2026-05-12）**：第五节"项目结构"与现状对齐 — 服务 14→23（含 4 个 W2 待删/收敛）、前端 16→18、迁移 v229→v413、tunxiang-api dedup。同步基线见 `docs/service-health/2026-W20.md`。本次只对齐事实，不改业务规则、Tier 分级、Agent 规范等核心宪法。
...
```
tunxiang-os/
  apps/                         # 18 个前端应用（React 18 + TypeScript + Vite）— V3.1 同步
    ...
    web-devforge/               # React — DevForge 内部研发运维平台前端（V3.1 同步补录）
    web-forge-admin/            # React — Forge 外部 ISV 市场 admin 前端（V3.1 同步补录）
  services/                     # 23 个目录（22 业务 + 1 mcp-server）— V3.1 同步
                                # W2 计划砍 3（indonesia/malaysia/vietnam）→ 20
                                # W12 目标 17（详见 docs/service-health 周报）
    ...
    tx-civic/          :8014    # 城市监管平台（9 路由文件：食安追溯/明厨亮灶/环保/消防/证照/上报/合规评分；W11 计划回路由模块）
    # ── V3.1 同步补录（CLAUDE.md V3.0 未列出的服务）──
    tx-forge/          :8013    # 外部 ISV 应用市场（与 tx-devforge :8017 严格区分）
    tx-expense/        :8015    # 费用/合同台账
    tx-pay/            :8016    # 支付中枢（最干净的服务，9/10 健康度，4 个 Tier1 测试文件）
    tx-devforge/       :8017    # 内部研发运维平台（与 tx-forge :8013 严格区分）
    tx-predict/        :8019    # 预测引擎（base.yml 已迁 8019；main.py/README 仍写 8013，待修）
    tx-indonesia/               # 🔴 W2 计划删除 — 国际化过早抽象，无业务，重启等首批 3 家国内客户稳定后
    tx-malaysia/                # 🔴 W2 计划删除 — 同上
    tx-vietnam/                 # 🔴 W2 计划删除 — 同上
    # ──────────────────────────────────────────────────
    mcp-server/                 # MCP Protocol Server（对接 Claude Code）
    # tunxiang-api/  ❌ 已 R1 dedup 删除（2026-05-06），V3.1 同步移除此条
  ...
  shared/
    ...
    db-migrations/              # Alembic 迁移（V3.1 同步：511 文件，最新 v413）
      # chain integrity: 5/9 (B') 已修复，1 base / 1 head / 511 unique revision
      # 校验命令：python3 scripts/check_alembic_chain.py --versions-dir shared/db-migrations/versions
      ...
      # v265: Ontology 事件总线 Outbox Relay 配套表（event_outbox_cursor + processed_events）
      # v266: financial_voucher_lines（GL 凭证行雏形 — 12 周计划 W3 起接入正式 GL 内核）
      # v413: member_identity_map（最新，2026-05 节奏每周新增 ~10 个迁移）
...
| POS 数据写入与结算 | tx-trade | adapters/pinzhi_pos / aoqiwei / meituan |
