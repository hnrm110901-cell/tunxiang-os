...
```typescript
// 三种 cool path 运行环境
const isAndroidPOSWebView = () => !!window.TXBridge;            // android-pos 内嵌 WebView
const isIPadWKWebView = () => !window.TXBridge && /iPad/.test(navigator.userAgent);
const isBrowser = () => !window.TXBridge && !isIPadWKWebView(); // 总部 Web

// 外设调用统一入口（cool path 通用）
const printReportFromCoolPath = async (data: ReportData) => {
  if (isAndroidPOSWebView()) {
