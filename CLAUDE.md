...
- **复用现有代码**：693K 行 Python + 416K 行 TypeScript 全部保留。
...
L2  业务中台层      21 个微服务 × 9 大产品域 = 700+ 路由模块
...
```
tunxiang-os/
  apps/                         # 18 个前端应用（React 18 + TypeScript + Vite）
  ...
  services/                     # 19 个业务微服务 + 2 个支撑服务（FastAPI + SQLAlchemy 2.0 + asyncpg）
  ...
  shared/
    ...
    db-migrations/              # Alembic 迁移（481 个版本，v001-v384）
...
| LWW + 终态保护 | edge/sync-engine | conflict_resolver.py |
