...
class TestSkillRegistryScan:
    ...
    def test_scan_finds_all_skills(self, registry) -> None:
        ...
        assert len(skills) >= 22, (
            f"期望 >= 22 个 Skill，实际只加载了 {len(skills)} 个。"
            "请检查 SERVICES_ROOT 路径或 SKILL.yaml 格式。"
        )
    ...
    def test_skill_has_required_fields(self, registry) -> None:
        ...
        for skill in registry.list_skills():
            ...
            assert "." in skill.meta.version, (
                f"Skill '{skill.meta.name}' 的 version '{skill.meta.version}' "
                "不符合 semver 格式"
            )
            # display_name 不能为空（SKILL.yaml 规范要求填写中文名）
            assert skill.meta.display_name, (
                f"Skill '{skill.meta.name}' 缺少 meta.display_name"
            )
    ...
    def test_order_core_skill_exists(self, registry) -> None:
        ...
        assert "terminal.checkout.requested" in event_types, (
            "order-core 缺少 terminal.checkout.requested 触发器，"
            "挂账结账流程依赖此事件"
        )
    ...
    def test_credit_account_skill_exists(self, registry) -> None:
        ...
        assert skill.degradation.offline is not None, (
            "credit-account 缺少 degradation.offline 配置"
        )
        # 挂账必须联网校验信用额度，离线不可操作
        assert skill.degradation.offline.can_operate is False, (
            "credit-account 离线时必须 can_operate=False，"
            "防止超额授信风险"
        )
    ...
    def test_wine_storage_skill_exists(self, registry) -> None:
        ...
        assert skill.degradation.offline is not None, (
            "wine-storage 缺少 degradation.offline 配置"
        )
        # 存酒操作可离线（本地 sqlite_wal 缓存，联网后同步）
        assert skill.degradation.offline.can_operate is True, (
            "wine-storage 离线时应 can_operate=True"
        )
...
class TestSkillRouterEventMatching:
    ...
    def test_route_order_paid_to_inventory(self, router) -> None:
        ...
        assert "inventory-core" in skill_names, (
            f"order.paid 未路由到 inventory-core，实际命中: {skill_names}"
        )
    ...
    def test_route_order_checkout_corporate(self, router) -> None:
        ...
        skill_names = [m.skill.meta.name for m in matches]
        assert "credit-account" in skill_names, (
            f"企业客户结账请求未路由到 credit-account，实际命中: {skill_names}"
        )

    def test_route_order_checkout_non_corporate_excludes_credit(
        self, router
    ) -> None:
        ...
        # 非企业客户不应触发挂账流程
        assert "credit-account" not in skill_names, (
            "非企业客户结账不应路由到 credit-account"
        )
    ...
    def test_route_member_tier_changed(self, router) -> None:
        ...
        skill_names = [m.skill.meta.name for m in matches]
        assert "wine-storage" in skill_names, (
            f"gold 级别升级未路由到 wine-storage，实际命中: {skill_names}"
        )

    def test_route_member_tier_changed_low_tier_excludes_wine(
        self, router
    ) -> None:
        ...
        assert "wine-storage" not in skill_names, (
            "silver 级别升级不应触发 wine-storage 延期逻辑"
        )
    ...
    def test_wine_storage_offline_full(self, registry) -> None:
        ...
        for action in ("store", "retrieve", "extend"):
            assert action in cap_map, (
                f"wine-storage 离线能力缺少 action '{action}'"
            )
            assert cap_map[action] == "full", (
                f"wine-storage 离线 '{action}' 应为 full 模式，实际: {cap_map[action]}"
            )
...
class TestOntologyRegistry:
    ...
    def test_order_owned_by_order_core(self, ontology) -> None:
        ...
        assert owner == "order-core", (
            f"Order 实体所有者应为 order-core，实际: {owner}"
        )
    ...
    def test_no_duplicate_entity_ownership(self, ontology) -> None:
        ...
        assert len(conflicts) == 0, (
            f"发现实体所有权冲突（违反 Ontology 单一所有者原则）:\n"
            + "\n".join(conflicts)
        )
    ...
    def test_wine_storage_entity_owned(self, ontology) -> None:
        ...
        assert owner == "wine-storage", (
            f"WineStorage 实体所有者应为 wine-storage，实际: {owner}"
        )
    ...
    def test_credit_entities_owned_by_credit_account(self, ontology) -> None:
        ...
        for entity_name in ("CreditAgreement", "CreditBill"):
            ...
            assert owner == "credit-account", (
                f"{entity_name} 实体所有者应为 credit-account，实际: {owner}"
            )
...
class TestMCPBridgeToolGeneration:
    ...
    def test_tool_naming_convention(self, bridge, registry) -> None:
        ...
        for tool in tools:
            ...
            assert "-" not in tool.name, (
                f"工具名 '{tool.name}' 含有连字符，应统一使用下划线"
            )
    ...
    def test_tool_skill_name_field(self, bridge, registry) -> None:
        ...
        for tool in tools:
            assert tool.skill_name == "wine-storage", (
                f"工具 '{tool.name}' 的 skill_name 错误，"
                f"期望 'wine-storage'，实际 '{tool.skill_name}'"
            )
    ...
    def test_role_filter_cashier(self, bridge, registry) -> None:
        ...
        # 验证工具数量 < 所有工具数量
        assert len(cashier_tools) <= len(all_tools), (
            "cashier 过滤后工具数不应多于全量工具"
        )
        # 具体验证：cashier 不应看到 GET 查询类工具（view/report 权限不在 cashier actions 中）
        for tool in cashier_tools:
            assert tool.method.upper() != "GET", (
                f"cashier 不应获得 GET 工具 '{tool.name}'（需要 view/report 权限）"
            )
    ...
    def test_role_filter_brand_admin(self, bridge, registry) -> None:
        ...
        assert len(admin_tools) == len(all_tools), (
            f"brand_admin 应获得全部 {len(all_tools)} 个工具，"
            f"实际只得到 {len(admin_tools)} 个"
        )
    ...
    def test_generate_all_tools_covers_all_skills(self, bridge, registry) -> None:
        ...
        skills_with_api = [
            ...
            if s.triggers and s.triggers.api and s.triggers.api.endpoints
        ]
        # 所有工具数应 >= 有 API 的 skill 数（每个至少生成 1 个工具）
        assert len(all_tools) >= len(skills_with_api), (
            f"generate_all_tools 工具数 ({len(all_tools)}) "
            f"少于有 API 的 skill 数 ({len(skills_with_api)})"
        )
    ...
    def test_tool_to_dict_serializable(self, bridge, registry) -> None:
        ...
        for tool in tools:
            ...
            # 必须包含所有约定字段
            for field_name in (
                "name", "description", "skill_name", "method", "path",
                "required_role_actions"
            ):
                assert field_name in d, (
                    f"to_dict() 缺少字段 '{field_name}'"
                )
