 ...
 * v3 设计系统 — 深色背景 + 暖橙 #FF6B35 CTA 色
 *
 * 使用方式:
 *   import { colors, typography, spacing } from '@tunxiang/design-system/tokens';
 *   // CSS中用 var(--tx-brand-500) 等变量
 */

export { brand, mint, warm, neutral, semantic, action, dark, navy, colors } from './colors';
...
export { shellTokens } from './shell';
export type { ShellType } from './shell';
...
import { kdsTheme } from '../themes/kds';

// ── CSS 变量注入（在 main.tsx 中调用 injectTokens()）──
export function injectTokens(shell?: ShellType) {
  ...
  // ── 语义变量（主题感知）──
  const themeVars: Record<string, string> = currentTheme === 'light' ? {
    // 背景
    '--bg':              neutral[50],
    '--surface':         neutral[0],
  ...
  // ── tx-* 主题变量（新规范前缀）──
  const txThemeVars = currentTheme === 'kds' ? kdsTheme : currentTheme === 'light' ? lightTheme : darkTheme;

  // ── Shell 变量（终端差异）──
  const shellVars = shell ? shellTokens[shell] : {};

  Object.entries({ ...themeVars, ...staticVars, ...txThemeVars, ...shellVars }).forEach(([k, v]) => {
