...
def upgrade() -> None:
    ...
    op.execute("""
        DO $$ BEGIN
            IF (SELECT COUNT(*) FROM information_schema.columns 
                WHERE table_name='daily_pnl' AND column_name IN ('store_id', 'pnl_date')) = 2 THEN
                EXECUTE 'CREATE INDEX IF NOT EXISTS ix_daily_pnl_store_date ON daily_pnl (store_id, pnl_date)';
            END IF;
        END $$;
    """)
    op.execute("""
        DO $$ BEGIN
            IF (SELECT COUNT(*) FROM information_schema.columns 
                WHERE table_name='daily_pnl' AND column_name IN ('tenant_id', 'pnl_date')) = 2 THEN
                EXECUTE 'CREATE INDEX IF NOT EXISTS ix_daily_pnl_tenant_date ON daily_pnl (tenant_id, pnl_date)';
            END IF;
        END $$;
    """)
    op.execute("""
        DO $$ BEGIN
            IF (SELECT COUNT(*) FROM information_schema.columns 
                WHERE table_name='daily_pnl' AND (column_name = 'status')) = 1 THEN
                EXECUTE 'CREATE INDEX IF NOT EXISTS ix_daily_pnl_status ON daily_pnl (status)';
            END IF;
        END $$;
    """)

    op.execute("ALTER TABLE daily_pnl ENABLE ROW LEVEL SECURITY;")
    op.execute("DROP POLICY IF EXISTS daily_pnl_tenant_isolation ON daily_pnl;")
    op.execute("""
    ...
    # ── cost_items 成本明细 ─────────────────────────────────────────────────
    if "cost_items" not in _existing:
        op.create_table(
            "cost_items",
            sa.Column(
                "id",
    ...
    op.execute("""
        DO $$ BEGIN
            IF (SELECT COUNT(*) FROM information_schema.columns 
                WHERE table_name='cost_items' AND column_name IN ('store_id', 'cost_date')) = 2 THEN
                EXECUTE 'CREATE INDEX IF NOT EXISTS ix_cost_items_store_date ON cost_items (store_id, cost_date)';
            END IF;
        END $$;
    """)
    op.execute("""
        DO $$ BEGIN
            IF (SELECT COUNT(*) FROM information_schema.columns 
                WHERE table_name='cost_items' AND column_name IN ('tenant_id', 'cost_date')) = 2 THEN
                EXECUTE 'CREATE INDEX IF NOT EXISTS ix_cost_items_tenant_date ON cost_items (tenant_id, cost_date)';
            END IF;
        END $$;
    """)
    op.execute("""
        DO $$ BEGIN
            IF (SELECT COUNT(*) FROM information_schema.columns 
                WHERE table_name='cost_items' AND (column_name = 'cost_type')) = 1 THEN
                EXECUTE 'CREATE INDEX IF NOT EXISTS ix_cost_items_cost_type ON cost_items (cost_type)';
            END IF;
        END $$;
    """)
    op.execute("""
        DO $$ BEGIN
            IF (SELECT COUNT(*) FROM information_schema.columns 
                WHERE table_name='cost_items' AND (column_name = 'reference_id')) = 1 THEN
                EXECUTE 'CREATE INDEX IF NOT EXISTS ix_cost_items_reference_id ON cost_items (reference_id)';
            END IF;
        END $$;
    """)

    op.execute("ALTER TABLE cost_items ENABLE ROW LEVEL SECURITY;")
    op.execute("DROP POLICY IF EXISTS cost_items_tenant_isolation ON cost_items;")
    op.execute("""
    ...
    if "revenue_records" not in _existing:
        op.create_table(
            "revenue_records",
            sa.Column(
                "id",
    ...
    op.execute("""
        DO $$ BEGIN
            IF (SELECT COUNT(*) FROM information_schema.columns 
                WHERE table_name='revenue_records' AND column_name IN ('store_id', 'record_date')) = 2 THEN
                EXECUTE 'CREATE INDEX IF NOT EXISTS ix_revenue_records_store_date ON revenue_records (store_id, record_date)';
            END IF;
        END $$;
    """)
    op.execute("""
        DO $$ BEGIN
            IF (SELECT COUNT(*) FROM information_schema.columns 
                WHERE table_name='revenue_records' AND column_name IN ('tenant_id', 'record_date')) = 2 THEN
                EXECUTE 'CREATE INDEX IF NOT EXISTS ix_revenue_records_tenant_date ON revenue_records (tenant_id, record_date)';
            END IF;
        END $$;
    """)
    op.execute("""
        DO $$ BEGIN
            IF (SELECT COUNT(*) FROM information_schema.columns 
                WHERE table_name='revenue_records' AND (column_name = 'channel')) = 1 THEN
                EXECUTE 'CREATE INDEX IF NOT EXISTS ix_revenue_records_channel ON revenue_records (channel)';
            END IF;
        END $$;
    """)
    op.execute("""
        DO $$ BEGIN
            IF (SELECT COUNT(*) FROM information_schema.columns 
                WHERE table_name='revenue_records' AND (column_name = 'order_id')) = 1 THEN
                EXECUTE 'CREATE INDEX IF NOT EXISTS ix_revenue_records_order_id ON revenue_records (order_id)';
            END IF;
        END $$;
    """)

    op.execute("ALTER TABLE revenue_records ENABLE ROW LEVEL SECURITY;")
    op.execute("DROP POLICY IF EXISTS revenue_records_tenant_isolation ON revenue_records;")
    op.execute("""
    ...
    if "finance_configs" not in _existing:
        op.create_table(
            "finance_configs",
            sa.Column(
                "id",
    ...
    op.execute("""
        DO $$ BEGIN
            IF (SELECT COUNT(*) FROM information_schema.columns 
                WHERE table_name='finance_configs' AND column_name IN ('tenant_id', 'store_id', 'config_type')) = 3 THEN
                EXECUTE 'CREATE INDEX IF NOT EXISTS ix_finance_configs_store_type ON finance_configs (tenant_id, store_id, config_type)';
            END IF;
        END $$;
    """)
    op.execute("""
        DO $$ BEGIN
            IF (SELECT COUNT(*) FROM information_schema.columns 
                WHERE table_name='finance_configs' AND column_name IN ('effective_from', 'effective_until')) = 2 THEN
                EXECUTE 'CREATE INDEX IF NOT EXISTS ix_finance_configs_effective ON finance_configs (effective_from, effective_until)';
            END IF;
        END $$;
    """)

    op.execute("ALTER TABLE finance_configs ENABLE ROW LEVEL SECURITY;")
    op.execute("DROP POLICY IF EXISTS finance_configs_tenant_isolation ON finance_configs;")
    op.execute("""
