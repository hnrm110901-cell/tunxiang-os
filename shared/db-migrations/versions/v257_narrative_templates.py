...
def upgrade() -> None:
    ...
    if 'narrative_templates' not in existing:
        op.create_table(
            ...
            sa.Column("brand_focus", sa.VARCHAR(200), nullable=True,
                      comment="品牌关注焦点，如：营业额/毛利"),
            sa.Column("prompt_prefix", sa.TEXT, nullable=True,
                      comment="AI 提示词前缀"),
            sa.Column("metrics_weights", postgresql.JSONB, nullable=False,
                      server_default="{}",
                      comment="指标权重配置 {metric_key: weight}"),
            sa.Column("tone", sa.VARCHAR(40), nullable=False,
                      server_default="professional",
                      comment="语气风格: professional/executive/casual"),
            sa.Column("is_default", sa.BOOLEAN, nullable=False, server_default="false",
                      comment="是否为默认模板"),
            sa.Column("is_system", sa.BOOLEAN, nullable=False, server_default="false",
                      comment="true=内置模板不可删除"),
        ...
        op.create_index("ix_nt_tenant_default", "narrative_templates",
