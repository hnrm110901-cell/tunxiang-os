...
def upgrade() -> None:
    ...
    if "store_platform_bindings" not in existing:
        op.create_table(
            ...
            sa.Column("store_id", UUID(as_uuid=True), nullable=False,
                      comment="关联 stores.id"),
            sa.Column("platform", sa.String(50), nullable=False,
                      comment="meituan_booking / dianping_booking / wechat_booking"),
            sa.Column("platform_shop_id", sa.Text, nullable=False,
                      comment="平台侧门店 ID（美团 shop_id / 点评 poi_id 等）"),
    ...
    op.execute(
        "DROP POLICY IF EXISTS store_platform_bindings_tenant ON store_platform_bindings;"
    )
    op.execute("""
