...
def upgrade() -> None:
    ...
    if "franchise_fees" not in _existing:
        op.create_table(
            ...
            sa.Column("fee_type", sa.String(50), nullable=False, comment="royalty/management/brand/training"),
            sa.Column("amount_fen", sa.BigInteger, nullable=False, server_default="0"),
            sa.Column("due_date", sa.Date),
            sa.Column("paid_date", sa.Date),
            sa.Column(
                "status", sa.String(20), nullable=False, server_default="pending", comment="pending/paid/overdue"
            ),
    ...
    op.execute("CREATE INDEX IF NOT EXISTS ix_franchise_fees_tenant ON franchise_fees (tenant_id)")
    op.execute("CREATE INDEX IF NOT EXISTS ix_franchise_fees_franchisee ON franchise_fees (tenant_id, franchisee_id)")
    op.execute("CREATE INDEX IF NOT EXISTS ix_franchise_fees_status ON franchise_fees (tenant_id, status)")
    op.execute("CREATE INDEX IF NOT EXISTS ix_franchise_fees_due_date ON franchise_fees (tenant_id, due_date)")
    ...
    op.execute(f"CREATE POLICY franchise_fees_rls_select ON franchise_fees FOR SELECT USING ({_SAFE_CONDITION})")
    op.execute(f"CREATE POLICY franchise_fees_rls_insert ON franchise_fees FOR INSERT WITH CHECK ({_SAFE_CONDITION})")
    op.execute(
        f"CREATE POLICY franchise_fees_rls_update ON franchise_fees "
        f"FOR UPDATE USING ({_SAFE_CONDITION}) WITH CHECK ({_SAFE_CONDITION})"
    )
    op.execute(f"CREATE POLICY franchise_fees_rls_delete ON franchise_fees FOR DELETE USING ({_SAFE_CONDITION})")
