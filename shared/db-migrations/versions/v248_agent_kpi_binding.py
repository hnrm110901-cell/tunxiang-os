...
def upgrade() -> None:
    ...
    if "agent_kpi_configs" not in existing_tables:
        op.create_table(
            ...
            sa.Column("alert_threshold", sa.Float(), nullable=True, comment="预警阈值（低于此值触发告警）"),
    ...
    if "agent_kpi_snapshots" not in existing_tables:
        op.create_table(
            ...
            sa.Column("agent_id", sa.String(64), nullable=False, comment="Agent标识符"),
            sa.Column("kpi_type", sa.String(64), nullable=False, comment="KPI类型标识"),
            sa.Column("measured_value", sa.Float(), nullable=False, comment="实测值"),
            sa.Column("target_value", sa.Float(), nullable=False, comment="目标值（快照时的目标）"),
            sa.Column("achievement_rate", sa.Float(), nullable=False, comment="达成率（0-1）"),
