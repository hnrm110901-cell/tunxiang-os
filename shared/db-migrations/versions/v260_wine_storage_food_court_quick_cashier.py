...
def upgrade() -> None:
    ...
    if "wine_storage_records" not in existing:
        op.create_table(
            ...
            sa.Column("member_id", UUID(as_uuid=True), nullable=True,
                      comment="会员 ID（可选）"),
            sa.Column("bottle_code", sa.String(64), nullable=False,
                      comment="酒水编号/条码"),
            sa.Column("wine_name", sa.String(128), nullable=False,
                      comment="酒水名称"),
            sa.Column("wine_brand", sa.String(128), nullable=True,
                      comment="品牌"),
            sa.Column("wine_spec", sa.String(64), nullable=True,
                      comment="规格，如 500ml/750ml"),
        ...
        op.create_index("ix_wine_storage_records_tenant_store",
                        "wine_storage_records", ["tenant_id", "store_id"])
        op.create_index("ix_wine_storage_records_bottle_code",
                        "wine_storage_records", ["bottle_code"])
        op.create_index("ix_wine_storage_records_member_id",
                        "wine_storage_records", ["member_id"])
        op.create_index("ix_wine_storage_records_status",
                        "wine_storage_records", ["tenant_id", "status"])
        op.create_index("ix_wine_storage_records_table_id",
                        "wine_storage_records", ["table_id"])

    op.execute("ALTER TABLE wine_storage_records ENABLE ROW LEVEL SECURITY;")
    op.execute(
        "DROP POLICY IF EXISTS wine_storage_records_tenant ON wine_storage_records;"
    )
    op.execute("""
    ...
    if "wine_storage_transactions" not in existing:
        op.create_table(
            ...
            sa.Column("record_id", UUID(as_uuid=True), nullable=False,
                      comment="关联 wine_storage_records.id"),
            sa.Column("store_id", sa.String(64), nullable=False),
            sa.Column("trans_type", sa.String(32), nullable=False,
            ...
            sa.Column("order_id", sa.String(64), nullable=True,
                      comment="关联订单 ID（取酒核销时）"),
            sa.Column("operated_by", sa.String(64), nullable=True,
                      comment="操作员 ID"),
            sa.Column("operated_at", sa.TIMESTAMP(timezone=True), nullable=True,
                      comment="操作时间"),
            sa.Column("approved_by", sa.String(64), nullable=True,
                      comment="审批人 ID（核销/调整时）"),
            sa.Column("notes", sa.Text, nullable=True),
            sa.Column("created_at", sa.TIMESTAMP(timezone=True),
                      server_default=sa.text("now()"), nullable=False),
            sa.Column("updated_at", sa.TIMESTAMP(timezone=True),
            ...
            sa.ForeignKeyConstraint(
                ["record_id"], ["wine_storage_records.id"], ondelete="RESTRICT"
            ),
        )
        op.create_index("ix_wine_storage_transactions_record_id",
                        "wine_storage_transactions", ["record_id"])
        op.create_index("ix_wine_storage_transactions_store_type",
                        "wine_storage_transactions", ["store_id", "trans_type"])
        op.create_index("ix_wine_storage_transactions_tenant",
                        "wine_storage_transactions", ["tenant_id"])

    op.execute("ALTER TABLE wine_storage_transactions ENABLE ROW LEVEL SECURITY;")
    op.execute(
        "DROP POLICY IF EXISTS wine_storage_transactions_tenant ON wine_storage_transactions;"
    )
    op.execute("""
    ...
    if "food_courts" not in existing:
        op.create_table(
            ...
            sa.Column("name", sa.String(100), nullable=False,
                      comment="商街名称，如：XX美食广场"),
            sa.Column("description", sa.String(500), nullable=True,
                      comment="描述/简介"),
            sa.Column("status", sa.String(20), nullable=False,
                      server_default="'active'",
                      comment="active=营业中 / inactive=停用"),
            sa.Column("unified_cashier", sa.Boolean, nullable=False,
                      server_default="true",
                      comment="是否启用统一收银台"),
            sa.Column("config", JSONB, nullable=True,
                      server_default="'{}'",
                      comment="扩展配置 JSON（营业时间、收银台数量等）"),
            sa.Column("created_at", sa.TIMESTAMP(timezone=True),
                      server_default=sa.text("now()"), nullable=False),
            sa.Column("updated_at", sa.TIMESTAMP(timezone=True),
                      server_default=sa.text("now()"), nullable=False),
        ...
        op.create_index("ix_food_courts_tenant_store",
                        "food_courts", ["tenant_id", "store_id"])
    ...
    if "food_court_vendors" not in existing:
        op.create_table(
            ...
            sa.Column("id", UUID(as_uuid=True), primary_key=True,
                      server_default=sa.text("gen_random_uuid()")),
            sa.Column("tenant_id", UUID(as_uuid=True), nullable=False),
            sa.Column("food_court_id", UUID(as_uuid=True), nullable=False,
                      comment="所属商街 ID"),
            ...
            sa.Column("category", sa.String(50), nullable=True,
                      comment="档口分类：烧腊/粉面/饮料/小吃/快餐等"),
            sa.Column("owner_name", sa.String(50), nullable=True,
                      comment="档主姓名"),
            sa.Column("contact_phone", sa.String(20), nullable=True,
                      comment="联系电话"),
            sa.Column("commission_rate", sa.Numeric(5, 4), nullable=True,
                      comment="抽成比例（如 0.08 = 8%）"),
            sa.Column("kds_station_id", sa.String(50), nullable=True,
                      comment="对应 KDS 档口站点 ID"),
            sa.Column("status", sa.String(20), nullable=False,
                      server_default="'active'",
                      comment="active=正常营业 / inactive=暂停 / suspended=被暂停"),
            sa.Column("settlement_account", JSONB, nullable=True,
                      server_default="'{}'",
                      comment="结算账户信息 JSON"),
            ...
            sa.Column("is_deleted", sa.Boolean, nullable=False,
                      server_default="false"),
            sa.ForeignKeyConstraint(
                ["food_court_id"], ["food_courts.id"], ondelete="RESTRICT"
            ),
        )
        op.create_index("ix_food_court_vendors_court_id",
                        "food_court_vendors", ["food_court_id"])
        op.create_index("ix_food_court_vendors_tenant",
                        "food_court_vendors", ["tenant_id"])
    ...
    if "food_court_orders" not in existing:
        ...
        op.create_index("ix_food_court_orders_court_id_status",
                        "food_court_orders", ["food_court_id", "status"])
        op.create_index("ix_food_court_orders_tenant",
                        "food_court_orders", ["tenant_id"])
        op.create_index("ix_food_court_orders_order_no",
                        "food_court_orders", ["order_no"])
    ...
    if "food_court_order_items" not in existing:
        op.create_table(
            ...
            sa.Column("is_deleted", sa.Boolean, nullable=False,
                      server_default="false"),
            sa.ForeignKeyConstraint(
                ["order_id"], ["food_court_orders.id"], ondelete="CASCADE"
            ),
            sa.ForeignKeyConstraint(
                ["vendor_id"], ["food_court_vendors.id"], ondelete="RESTRICT"
            ),
        )
        op.create_index("ix_food_court_order_items_order_id",
                        "food_court_order_items", ["order_id"])
        op.create_index("ix_food_court_order_items_vendor_id",
                        "food_court_order_items", ["vendor_id"])

    op.execute("ALTER TABLE food_court_order_items ENABLE ROW LEVEL SECURITY;")
    op.execute(
        "DROP POLICY IF EXISTS food_court_order_items_tenant ON food_court_order_items;"
    )
    op.execute("""
    ...
    if "food_court_vendor_settlements" not in existing:
        op.create_table(
            ...
            sa.Column("order_count", sa.Integer, nullable=False,
                      server_default="0", comment="当日订单笔数"),
            sa.Column("item_count", sa.Integer, nullable=False,
                      server_default="0", comment="当日菜品份数"),
            sa.Column("gross_amount_fen", sa.Integer, nullable=False,
                      server_default="0", comment="当日营业总额（分）"),
            sa.Column("commission_fen", sa.Integer, nullable=False,
                      server_default="0", comment="平台抽成（分）"),
            sa.Column("net_amount_fen", sa.Integer, nullable=False,
                      server_default="0",
        ...
        op.create_index("ix_food_court_vendor_settlements_vendor_date",
                        "food_court_vendor_settlements",
                        ["vendor_id", "settlement_date"])
        op.create_index("ix_food_court_vendor_settlements_court_date",
                        "food_court_vendor_settlements",
                        ["food_court_id", "settlement_date"])
        op.create_index("ix_food_court_vendor_settlements_tenant",
                        "food_court_vendor_settlements", ["tenant_id"])

    op.execute(
        "ALTER TABLE food_court_vendor_settlements ENABLE ROW LEVEL SECURITY;"
    )
    op.execute(
        "DROP POLICY IF EXISTS food_court_vendor_settlements_tenant "
        "ON food_court_vendor_settlements;"
    )
    op.execute("""
    ...
    if "quick_cashier_counters" not in existing:
        op.create_table(
            ...
            sa.Column("store_id", UUID(as_uuid=True), nullable=False,
                      comment="所属门店 ID"),
            sa.Column("counter_code", sa.String(20), nullable=False,
                      comment="档口编号"),
            sa.Column("counter_name", sa.String(100), nullable=False,
                      comment="档口名称"),
            sa.Column("status", sa.String(20), nullable=False,
        ...
        op.create_index("ix_quick_cashier_counters_tenant_store",
                        "quick_cashier_counters", ["tenant_id", "store_id"])
        op.create_index("ix_quick_cashier_counters_status",
                        "quick_cashier_counters", ["store_id", "status"])

    op.execute("ALTER TABLE quick_cashier_counters ENABLE ROW LEVEL SECURITY;")
    op.execute(
        "DROP POLICY IF EXISTS quick_cashier_counters_tenant ON quick_cashier_counters;"
    )
    op.execute("""
    ...
    if "quick_cashier_queue" not in existing:
        op.create_table(
            ...
            sa.Column("store_id", UUID(as_uuid=True), nullable=False,
                      comment="门店 ID"),
            sa.Column("counter_id", UUID(as_uuid=True), nullable=True,
                      comment="关联档口 ID"),
            sa.Column("queue_number", sa.String(20), nullable=True,
                      comment="叫号号码，如 A001"),
            sa.Column("status", sa.String(20), nullable=False,
                      server_default="'waiting'",
                      comment="waiting=等待中 / processing=服务中 / done=已完成 / left=已离开"),
            sa.Column("customer_info", JSONB, nullable=False,
                      server_default="'{}'",
            ...
            sa.Column("processed_at", sa.TIMESTAMP(timezone=True), nullable=True,
                      comment="开始服务时间"),
            sa.ForeignKeyConstraint(
                ["counter_id"], ["quick_cashier_counters.id"], ondelete="SET NULL"
            ),
        )
        op.create_index("ix_quick_cashier_queue_counter_status",
                        "quick_cashier_queue", ["counter_id", "status"])
        op.create_index("ix_quick_cashier_queue_tenant_store",
                        "quick_cashier_queue", ["tenant_id", "store_id"])

    op.execute("ALTER TABLE quick_cashier_queue ENABLE ROW LEVEL SECURITY;")
    op.execute(
        "DROP POLICY IF EXISTS quick_cashier_queue_tenant ON quick_cashier_queue;"
    )
    op.execute("""
    ...
    if "orders" in existing:
        ...
        if "demo_session_id" not in col_names:
            op.add_column(
                "orders",
                sa.Column("demo_session_id", sa.String(100), nullable=True,
                          comment="演示会话 ID"),
            )
