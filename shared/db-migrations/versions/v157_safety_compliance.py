...
revision: str = "v157"
down_revision: Union[str, None] = "v156"
...
def upgrade() -> None:
    ...
    if "biz_food_safety_inspections" not in _existing:
        op.create_table(
            ...
            sa.Column("inspector_id", UUID(as_uuid=True), nullable=False,
                      comment="巡检员（sys_staff）"),
            sa.Column("inspection_type", sa.Text, nullable=False,
                      comment="daily_open/daily_close/weekly/surprise/government"),
            ...
            sa.Column("pass_threshold", sa.Numeric(5, 2), nullable=False,
                      server_default="80.0"),
            sa.Column("is_passed", sa.Boolean, nullable=True,
                      comment="NULL=未完成，true=合格，false=不合格"),
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_fsi_tenant_store "
        "ON biz_food_safety_inspections (tenant_id, store_id)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_fsi_tenant_date "
        "ON biz_food_safety_inspections (tenant_id, inspection_date)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_fsi_tenant_status "
        "ON biz_food_safety_inspections (tenant_id, status)"
    )
    ...
    if "biz_food_safety_items" not in _existing:
        op.create_table(
            ...
            sa.Column("inspection_id", UUID(as_uuid=True), nullable=False,
                      comment="ref: biz_food_safety_inspections.id"),
            sa.Column("item_code", sa.Text, nullable=False,
                      comment="巡检项目编号，如 FS001-FS030"),
            sa.Column("item_name", sa.Text, nullable=False,
                      comment="巡检项目名称"),
            sa.Column("category", sa.Text, nullable=False,
                      comment="食材管理/烹饪操作/设备卫生/人员卫生/储存管理"),
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_fsitems_tenant_inspection "
        "ON biz_food_safety_items (tenant_id, inspection_id)"
    )
    op.execute(
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_fst_tenant_brand "
        "ON biz_food_safety_templates (tenant_id, brand_id)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_fst_tenant_type "
        "ON biz_food_safety_templates (tenant_id, inspection_type)"
    )
