...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("id", UUID(as_uuid=True), primary_key=True,
                  server_default=sa.text("gen_random_uuid()"), comment="区域ID"),
        ...
        sa.Column(
            "zone_type", sa.String(20), nullable=False, server_default="hall",
        ...
        sa.Column(
            "min_consume_multiplier", sa.Numeric(4, 2), server_default="1.0",
            comment="最低消费倍率，1.0=不额外要求，2.0=包间费2倍",
        ),
        sa.Column(
            "service_config", JSONB, server_default="{}",
    ...
    op.create_table(
        ...
        sa.Column("employee_id", UUID(as_uuid=True), sa.ForeignKey("employees.id"),
                  nullable=False, index=True, comment="服务员员工ID"),
        sa.Column("zone_id", UUID(as_uuid=True), sa.ForeignKey("table_zones.id"),
                  nullable=False, index=True, comment="负责区域ID"),
        sa.Column("shift_date", sa.Date, nullable=False, comment="班次日期"),
        sa.Column("shift_type", sa.String(20), server_default="all_day",
    ...
    # 简化：复用 tenant_id 列
    op.execute("ALTER TABLE waiter_zone_assignments ADD COLUMN tenant_id UUID NOT NULL DEFAULT '00000000-0000-0000-0000-000000000000'")
    ...
    op.create_table(
        "dining_sessions",
        sa.Column("id", UUID(as_uuid=True), primary_key=True,
                  server_default=sa.text("gen_random_uuid()"), comment="会话ID"),
        sa.Column("tenant_id", UUID(as_uuid=True), nullable=False, index=True),
        sa.Column("store_id", UUID(as_uuid=True), sa.ForeignKey("stores.id"),
                  nullable=False, index=True),
        sa.Column("table_id", UUID(as_uuid=True), sa.ForeignKey("tables.id"),
                  nullable=False, index=True, comment="物理桌台ID（正式FK，替代 orders.table_number 字符串）"),
        ...
        sa.Column("guest_count", sa.Integer, nullable=False, server_default="1",
                  comment="就餐人数"),
        sa.Column("vip_customer_id", UUID(as_uuid=True), sa.ForeignKey("customers.id"),
                  comment="主要VIP顾客ID（开台时识别/扫码绑定）"),
        sa.Column("booking_id", UUID(as_uuid=True),
                  comment="来源预订单ID（从预订进来时记录）"),
        ...
        sa.Column(
            "status", sa.String(20), nullable=False, server_default="seated",
            index=True, comment="会话状态：reserved/seated/ordering/dining/add_ordering/billing/paid/clearing",
        ),

        # ── 服务归属 ─────────────────────────────────────────────────
        sa.Column("lead_waiter_id", UUID(as_uuid=True), sa.ForeignKey("employees.id"),
        ...
        # ── 会话类型 ─────────────────────────────────────────────────
        sa.Column(
            "session_type", sa.String(20), nullable=False, server_default="dine_in",
            comment="类型：dine_in(堂食)/banquet(宴席)/vip_room(VIP包间)/self_order(扫码)/hotpot(拼台)",
        ),

        # ── 时间轴关键节点（每个状态变更时记录）───────────────────────
        sa.Column("opened_at", sa.DateTime(timezone=True), nullable=False,
                  server_default=sa.func.now(), comment="开台时间"),
        sa.Column("first_order_at", sa.DateTime(timezone=True),
                  comment="首次点菜时间"),
        sa.Column("first_dish_served_at", sa.DateTime(timezone=True),
                  comment="首道菜上桌时间"),
        sa.Column("last_dish_served_at", sa.DateTime(timezone=True),
                  comment="最后一道菜上桌时间"),
        sa.Column("bill_requested_at", sa.DateTime(timezone=True),
                  comment="请求买单时间"),
        ...
        sa.Column("discount_amount_fen", sa.Integer, nullable=False, server_default="0",
                  comment="折扣总额（分）"),
        sa.Column("final_amount_fen", sa.Integer, nullable=False, server_default="0",
                  comment="实付金额（分）"),
        sa.Column("per_capita_fen", sa.Integer, nullable=False, server_default="0",
                  comment="人均消费（分），= final_amount_fen / guest_count"),
        sa.Column("service_call_count", sa.Integer, nullable=False, server_default="0",
                  comment="服务呼叫次数（含催菜，用于服务质量分析）"),

        # ── 包间/宴席扩展 ────────────────────────────────────────────
        sa.Column(
            "room_config", JSONB, server_default="{}",
            comment="包间配置：{room_fee_fen, min_spend_fen, decoration_requests, pre_order_items}",
        ),

        # ── 关联原始 tables.table_no（兼容旧系统/打印场景）────────────
        sa.Column("table_no_snapshot", sa.String(20),
                  comment="开台时记录的桌号快照（避免桌号变更影响历史记录）"),
    ...
    op.create_index("idx_dining_sessions_store_status",
                    "dining_sessions", ["store_id", "status"])
    op.create_index("idx_dining_sessions_opened",
                    "dining_sessions", ["store_id", "opened_at"])
    op.create_index("idx_dining_sessions_waiter",
                    "dining_sessions", ["lead_waiter_id", "store_id"])
    op.create_index("idx_dining_sessions_vip",
                    "dining_sessions", ["vip_customer_id"],
    ...
    op.create_table(
        ...
        sa.Column("tenant_id", UUID(as_uuid=True), nullable=False, index=True),
        sa.Column("store_id", UUID(as_uuid=True), nullable=False, index=True),
        sa.Column("table_session_id", UUID(as_uuid=True),
                  sa.ForeignKey("dining_sessions.id"), nullable=False, index=True,
                  comment="关联桌台会话"),
        sa.Column(
            "call_type", sa.String(30), nullable=False,
            comment="呼叫类型：call_waiter/urge_dish/need_item/complaint/checkout_request/other",
        ),
        sa.Column("content", sa.Text,
                  comment="呼叫内容，如催菜时填菜名，需要物品时填物品名"),
        sa.Column("target_dish_id", UUID(as_uuid=True),
                  comment="催菜时关联的菜品ID（可选）"),
        sa.Column("target_order_item_id", UUID(as_uuid=True),
                  comment="催菜时关联的订单条目ID（可选，精确定位）"),
        sa.Column(
            "status", sa.String(20), nullable=False, server_default="pending",
            index=True, comment="状态：pending(待处理)/handling(处理中)/handled(已处理)/cancelled(已取消)",
        ),
        sa.Column(
            "called_by", sa.String(20), server_default="pos",
            comment="呼叫来源：pos/self_order(扫码自助)/crew_app(服务员App)/kds",
        ),
        sa.Column("caller_name", sa.String(50), comment="呼叫人姓名或顾客标识"),
        sa.Column("handled_by", UUID(as_uuid=True), sa.ForeignKey("employees.id"),
                  comment="处理员工ID"),
        sa.Column("called_at", sa.DateTime(timezone=True), nullable=False,
                  server_default=sa.func.now(), comment="呼叫时间"),
        sa.Column("handled_at", sa.DateTime(timezone=True), comment="处理完成时间"),
        sa.Column("response_seconds", sa.Integer,
                  comment="响应时长（秒），= handled_at - called_at，用于服务SLA分析"),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now()),
        sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.func.now()),
    )
    op.create_index("idx_service_calls_session",
                    "service_calls", ["table_session_id", "status"])
    op.create_index("idx_service_calls_store_date",
                    "service_calls", ["store_id", "called_at"])
    op.create_index("idx_service_calls_pending",
                    "service_calls", ["store_id"],
    ...
    op.create_table(
        ...
        sa.Column("operator_type", sa.String(20), server_default="employee",
                  comment="操作者类型：employee/customer/system/agent"),
        sa.Column("occurred_at", sa.DateTime(timezone=True), nullable=False,
                  server_default=sa.func.now(), index=True, comment="事件发生时间"),
        # 因果链追踪（与 shared/events 一致的设计）
        sa.Column("causation_id", UUID(as_uuid=True),
                  comment="触发此事件的上游事件ID"),
    )
    # 按会话+时间查询（服务员看单张桌的历史）
    op.create_index("idx_tse_session_time",
                    "dining_session_events", ["table_session_id", "occurred_at"])
    # 按门店+时间查询（管理员看全店事件流）
    op.create_index("idx_tse_store_time",
                    "dining_session_events", ["store_id", "occurred_at"])
    # 按事件类型快速筛选（Agent订阅特定类型）
    op.create_index("idx_tse_event_type",
                    "dining_session_events", ["store_id", "event_type", "occurred_at"])
    ...
    op.add_column(
        "tables",
        sa.Column("zone_id", UUID(as_uuid=True), sa.ForeignKey("table_zones.id"),
                  comment="所属区域ID（v149新增）"),
    )
    op.add_column(
        "tables",
        sa.Column("qr_code_url", sa.String(500),
                  comment="桌台二维码URL（扫码点餐/呼叫服务，v149新增）"),
    )
    op.add_column(
        "tables",
        sa.Column("table_type", sa.String(20), server_default="standard",
