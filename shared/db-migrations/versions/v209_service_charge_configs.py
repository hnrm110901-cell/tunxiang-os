...
def upgrade() -> None:
    ...
    if "service_charge_templates" not in existing:
        op.create_table(
            ...
            sa.Column("status", sa.VARCHAR(20), nullable=False, server_default="active", comment="active/archived"),
    ...
    if "service_charge_configs" not in existing:
        op.create_table(
            ...
            sa.Column(
                ...
                sa.VARCHAR(20),
                nullable=False,
                server_default="by_person",
                comment="by_person/by_table/by_time/by_amount",
            ),
            sa.Column("config", postgresql.JSONB, nullable=False, server_default="{}", comment="完整配置JSON"),
        ...
        op.create_index("ix_scc_tenant_store", "service_charge_configs", ["tenant_id", "store_id"], unique=True)
