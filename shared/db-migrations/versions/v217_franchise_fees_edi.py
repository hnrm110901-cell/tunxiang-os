...
def upgrade() -> None:
    ...
    if "franchise_contracts" not in existing:
        op.create_table(
            ...
            sa.Column("contract_type", sa.VARCHAR(20), nullable=False, comment="initial/renewal/amendment"),
            ...
            sa.Column("status", sa.VARCHAR(20), server_default="active", comment="active/expired/terminated"),
            sa.Column("alert_days_before", sa.INTEGER, server_default="30", comment="到期提醒提前天数"),
        ...
        op.create_index("ix_franchise_contracts_tenant_status", "franchise_contracts", ["tenant_id", "status"])
        op.create_index("ix_franchise_contracts_franchisee", "franchise_contracts", ["tenant_id", "franchisee_id"])
        op.create_index("ix_franchise_contracts_end_date", "franchise_contracts", ["tenant_id", "end_date"])
    ...
    if "franchise_fee_records" not in existing:
        op.create_table(
            ...
            sa.Column("tenant_id", postgresql.UUID(as_uuid=True), nullable=False),
            sa.Column("contract_id", postgresql.UUID(as_uuid=True), nullable=True, comment="关联合同ID"),
            ...
            sa.Column("status", sa.VARCHAR(20), server_default="unpaid", comment="unpaid/partial/paid/overdue/waived"),
        ...
        op.create_index("ix_franchise_fee_records_tenant_status", "franchise_fee_records", ["tenant_id", "status"])
        op.create_index("ix_franchise_fee_records_contract", "franchise_fee_records", ["tenant_id", "contract_id"])
        op.create_index("ix_franchise_fee_records_franchisee", "franchise_fee_records", ["tenant_id", "franchisee_id"])
        op.create_index("ix_franchise_fee_records_due_date", "franchise_fee_records", ["tenant_id", "due_date"])
        op.execute("ALTER TABLE franchise_fee_records ENABLE ROW LEVEL SECURITY")
        op.execute("""
            DO $$ BEGIN
                IF NOT EXISTS (
                    SELECT 1 FROM pg_policies
                    WHERE tablename = 'franchise_fee_records'
                    AND policyname = 'ffr_tenant_isolation'
                ) THEN
                    EXECUTE 'CREATE POLICY ffr_tenant_isolation ON franchise_fee_records
                        USING (tenant_id = current_setting(''app.tenant_id'')::uuid)';
                END IF;
            END$$;
        """)
    ...
    if "edi_orders" not in existing:
        op.create_table(
            ...
            sa.Column("po_id", postgresql.UUID(as_uuid=True), nullable=True, comment="关联采购订单ID"),
        ...
        op.create_index("ix_edi_orders_tenant_status", "edi_orders", ["tenant_id", "status"])
        op.create_index("ix_edi_orders_supplier", "edi_orders", ["tenant_id", "supplier_id"])
        op.create_index("ix_edi_orders_store", "edi_orders", ["tenant_id", "store_id"])
        op.create_index("ix_edi_orders_po", "edi_orders", ["tenant_id", "po_id"])
        op.execute("ALTER TABLE edi_orders ENABLE ROW LEVEL SECURITY")
        op.execute("""
            DO $$ BEGIN
                IF NOT EXISTS (
                    SELECT 1 FROM pg_policies
                    WHERE tablename = 'edi_orders'
                    AND policyname = 'edi_tenant_isolation'
                ) THEN
                    EXECUTE 'CREATE POLICY edi_tenant_isolation ON edi_orders
                        USING (tenant_id = current_setting(''app.tenant_id'')::uuid)';
                END IF;
            END$$;
        """)
