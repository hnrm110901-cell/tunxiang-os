...
def upgrade() -> None:
    ...
    if "banquet_orders" not in existing_tables:
        op.create_table(
            ...
            sa.Column("balance_fen", sa.BigInteger(), nullable=False, server_default="0", comment="尾款=总额-定金"),
            sa.Column("deposit_status", sa.VARCHAR(20), nullable=False, server_default="unpaid", comment="unpaid/paid"),
            sa.Column("balance_status", sa.VARCHAR(20), nullable=False, server_default="unpaid", comment="unpaid/paid"),
            sa.Column(
                "payment_status",
                sa.VARCHAR(20),
                nullable=False,
        ...
        _add_col_if_missing(
            "deposit_rate", sa.Column("deposit_rate", sa.NUMERIC(4, 2), nullable=False, server_default="0.30")
        )
        _add_col_if_missing(
            "deposit_fen", sa.Column("deposit_fen", sa.BigInteger(), nullable=False, server_default="0")
        )
        _add_col_if_missing(
            "balance_fen", sa.Column("balance_fen", sa.BigInteger(), nullable=False, server_default="0")
        )
        _add_col_if_missing(
            "deposit_status", sa.Column("deposit_status", sa.VARCHAR(20), nullable=False, server_default="unpaid")
        )
        _add_col_if_missing(
            "balance_status", sa.Column("balance_status", sa.VARCHAR(20), nullable=False, server_default="unpaid")
        )
        _add_col_if_missing(
            "payment_status", sa.Column("payment_status", sa.VARCHAR(20), nullable=False, server_default="unpaid")
        )
        _add_col_if_missing("banquet_date", sa.Column("banquet_date", sa.DATE(), nullable=True))
        _add_col_if_missing("banquet_time", sa.Column("banquet_time", sa.TIME(), nullable=True))
        _add_col_if_missing("cancel_reason", sa.Column("cancel_reason", sa.TEXT(), nullable=True))
        _add_col_if_missing("cancelled_at", sa.Column("cancelled_at", sa.TIMESTAMP(timezone=True), nullable=True))
    ...
    op.create_table(
        ...
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column("banquet_order_id", postgresql.UUID(as_uuid=True), nullable=False, comment="关联宴席订单"),
        sa.Column("payment_stage", sa.VARCHAR(20), nullable=False, comment="deposit/balance/full：定金/尾款/全额"),
        sa.Column("amount_fen", sa.BigInteger(), nullable=False),
        sa.Column("payment_method", sa.VARCHAR(30), nullable=True, comment="wechat/alipay/cash/card/transfer"),
        sa.Column(
            "payment_status",
            sa.VARCHAR(20),
            nullable=False,
        ...
        sa.Column("transaction_id", sa.VARCHAR(100), nullable=True, comment="第三方支付流水号"),
