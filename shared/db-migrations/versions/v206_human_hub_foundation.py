...
def upgrade() -> None:
    ...
    if 'store_staffing_templates' not in existing:
        op.create_table(
            ...
            sa.Column('store_type', sa.VARCHAR(50), nullable=False,
                      comment='店型: flagship/standard/mini/kiosk'),
            sa.Column('position', sa.VARCHAR(50), nullable=False,
                      comment='岗位: manager/chef/waiter/cashier/cleaner'),
            sa.Column('shift', sa.VARCHAR(30), nullable=False,
                      comment='班次: morning/afternoon/evening/full_day'),
            sa.Column('day_type', sa.VARCHAR(20), nullable=False,
                      server_default='weekday',
                      comment='weekday/weekend/holiday'),
            sa.Column('min_count', sa.Integer(), nullable=False,
                      server_default='1', comment='最低人数'),
            sa.Column('recommended_count', sa.Integer(), nullable=False,
                      server_default='1', comment='建议人数'),
            sa.Column('peak_buffer', sa.Integer(), nullable=False,
                      server_default='0', comment='峰值保护位'),
            sa.Column('min_skill_level', sa.Integer(), nullable=False,
                      server_default='1', comment='最低技能等级 1-5'),
            ...
            sa.Column('updated_at', sa.TIMESTAMP(timezone=True),
                      server_default=sa.text('NOW()'), nullable=False),
            sa.UniqueConstraint('tenant_id', 'store_type', 'position', 'shift', 'day_type',
                                name='uq_staffing_tpl_composite'),
        )
    ...
    if 'staffing_snapshots' not in existing:
        op.create_table(
            ...
            sa.Column('required_count', sa.Integer(), nullable=False,
                      server_default='0', comment='编制人数'),
            sa.Column('actual_count', sa.Integer(), nullable=False,
                      server_default='0', comment='实际在岗'),
            sa.Column('gap', sa.Integer(), nullable=False,
                      server_default='0', comment='缺口(负数=缺编)'),
            sa.Column('skill_gap_detail', postgresql.JSONB(astext_type=sa.Text()),
                      server_default='{}', comment='技能缺口明细'),
            sa.Column('impact_score', sa.Numeric(3, 1), server_default='0',
                      comment='对营业影响评分 0-10'),
            sa.Column('is_deleted', sa.Boolean(), nullable=False, server_default='false'),
            sa.Column('created_at', sa.TIMESTAMP(timezone=True),
                      server_default=sa.text('NOW()'), nullable=False),
            sa.Column('updated_at', sa.TIMESTAMP(timezone=True),
        ...
        op.create_index('idx_staffing_snap_store_date', 'staffing_snapshots',
                        ['tenant_id', 'store_id', sa.text('snapshot_date DESC')])
        op.create_index('idx_staffing_snap_gap', 'staffing_snapshots',
    ...
    if 'mentorship_relations' not in existing:
        op.create_table(
            ...
            sa.Column('end_date', sa.Date(), nullable=True,
                      comment='结束日期(NULL=进行中)'),
            sa.Column('status', sa.VARCHAR(20), nullable=False,
                      server_default='active',
                      comment='active/completed/terminated'),
            sa.Column('mentor_score', sa.Numeric(3, 1), nullable=True,
                      comment='带教评分 0-10'),
            sa.Column('mentee_pass_rate', sa.Numeric(5, 2), nullable=True,
                      comment='学员通关率 0-100'),
            sa.Column('notes', sa.TEXT(), nullable=True),
            sa.Column('is_deleted', sa.Boolean(), nullable=False, server_default='false'),
            sa.Column('created_at', sa.TIMESTAMP(timezone=True),
            ...
            sa.Column('updated_at', sa.TIMESTAMP(timezone=True),
                      server_default=sa.text('NOW()'), nullable=False),
            sa.UniqueConstraint('tenant_id', 'mentor_id', 'mentee_id', 'start_date',
                                name='uq_mentorship_active'),
        )
        op.create_index('idx_mentorship_tenant', 'mentorship_relations', ['tenant_id'])
        op.create_index('idx_mentorship_store_status', 'mentorship_relations',
    ...
    if 'onboarding_paths' not in existing:
        op.create_table(
            ...
            sa.Column('template_name', sa.VARCHAR(100), nullable=False,
                      server_default='标准入职路径'),
            sa.Column('start_date', sa.Date(), nullable=False),
            sa.Column('target_days', sa.Integer(), nullable=False,
                      server_default='30', comment='目标天数: 7/14/30'),
            ...
            sa.Column('progress_pct', sa.Numeric(5, 2), nullable=False,
                      server_default='0', comment='完成进度 0-100'),
            sa.Column('mentor_id', postgresql.UUID(as_uuid=True), nullable=True,
                      comment='带教师傅'),
            sa.Column('readiness_score', sa.Numeric(3, 1), server_default='0',
                      comment='上岗准备度 0-10'),
            sa.Column('status', sa.VARCHAR(20), nullable=False,
        ...
        op.create_index('idx_onboarding_employee', 'onboarding_paths',
                        ['tenant_id', 'employee_id'])
        op.create_index('idx_onboarding_store_status', 'onboarding_paths',
                        ['tenant_id', 'store_id', 'status'])
    ...
    if 'position_certifications' not in existing:
        op.create_table(
            ...
            sa.Column('position', sa.VARCHAR(50), nullable=False,
                      comment='认证岗位'),
            sa.Column('exam_items', postgresql.JSONB(astext_type=sa.Text()), nullable=False,
                      server_default='[]',
            ...
            sa.Column('expires_at', sa.TIMESTAMP(timezone=True), nullable=True,
                      comment='认证有效期'),
            sa.Column('certifier_id', postgresql.UUID(as_uuid=True), nullable=True,
                      comment='认证人ID'),
            sa.Column('retake_count', sa.Integer(), nullable=False,
                      server_default='0', comment='补考次数'),
            sa.Column('notes', sa.TEXT(), nullable=True),
            sa.Column('is_deleted', sa.Boolean(), nullable=False, server_default='false'),
            sa.Column('created_at', sa.TIMESTAMP(timezone=True),
                      server_default=sa.text('NOW()'), nullable=False),
        ...
        op.create_index('idx_pos_cert_employee', 'position_certifications',
                        ['tenant_id', 'employee_id', 'position'])
        op.create_index('idx_pos_cert_store', 'position_certifications',
                        ['tenant_id', 'store_id', 'position', 'passed'])
    ...
    if 'store_readiness_scores' not in existing:
        op.create_table(
            ...
            sa.Column('overall_score', sa.Numeric(5, 2), nullable=False,
                      server_default='0', comment='综合就绪分 0-100'),
            ...
            sa.Column('risk_level', sa.VARCHAR(10), nullable=False,
                      server_default='green', comment='green/yellow/red'),
            sa.Column('risk_positions', postgresql.JSONB(astext_type=sa.Text()),
                      server_default='[]', comment='[{position,gap,reason}]'),
            sa.Column('action_items', postgresql.JSONB(astext_type=sa.Text()),
                      server_default='[]', comment='[{action,priority,assigned_to}]'),
            sa.Column('is_deleted', sa.Boolean(), nullable=False, server_default='false'),
            sa.Column('created_at', sa.TIMESTAMP(timezone=True),
                      server_default=sa.text('NOW()'), nullable=False),
            sa.Column('updated_at', sa.TIMESTAMP(timezone=True),
                      server_default=sa.text('NOW()'), nullable=False),
            sa.UniqueConstraint('tenant_id', 'store_id', 'score_date', 'shift',
                                name='uq_readiness_daily'),
        )
        op.create_index('idx_readiness_tenant', 'store_readiness_scores', ['tenant_id'])
        op.create_index('idx_readiness_date_risk', 'store_readiness_scores',
                        ['tenant_id', sa.text('score_date DESC'), 'risk_level'])
    ...
    if 'peak_guard_records' not in existing:
        op.create_table(
            ...
            sa.Column('peak_type', sa.VARCHAR(20), nullable=False,
                      comment='lunch/dinner/weekend/holiday/event'),
            sa.Column('expected_traffic', sa.Integer(), server_default='0',
                      comment='预计客流'),
            sa.Column('coverage_score', sa.Numeric(5, 2), server_default='0',
                      comment='人力覆盖度 0-100'),
            sa.Column('risk_positions', postgresql.JSONB(astext_type=sa.Text()),
                      server_default='[]',
            ...
            sa.Column('result_score', sa.Numeric(5, 2), nullable=True,
                      comment='实际保障评分'),
            sa.Column('notes', sa.TEXT(), nullable=True),
            sa.Column('is_deleted', sa.Boolean(), nullable=False, server_default='false'),
            sa.Column('created_at', sa.TIMESTAMP(timezone=True),
        ...
        op.create_index('idx_peak_guard_store_date', 'peak_guard_records',
                        ['tenant_id', 'store_id', sa.text('guard_date DESC')])
        op.create_index('idx_peak_guard_coverage', 'peak_guard_records',
    ...
    if 'dri_work_orders' not in existing:
        op.create_table(
            ...
            sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True,
                      server_default=sa.text('gen_random_uuid()')),
            sa.Column('tenant_id', postgresql.UUID(as_uuid=True), nullable=False),
            sa.Column('order_no', sa.VARCHAR(30), nullable=False,
                      comment='工单编号 auto-gen'),
            sa.Column('order_type', sa.VARCHAR(30), nullable=False,
                      comment='recruit/fill_gap/training/retention/reform'),
            sa.Column('store_id', postgresql.UUID(as_uuid=True), nullable=False),
            sa.Column('title', sa.VARCHAR(200), nullable=False),
            sa.Column('description', sa.TEXT(), nullable=True),
            sa.Column('severity', sa.VARCHAR(10), nullable=False,
                      server_default='medium',
                      comment='low/medium/high/critical'),
            sa.Column('status', sa.VARCHAR(20), nullable=False,
                      server_default='draft',
                      comment='draft/assigned/in_progress/completed/closed/cancelled'),
            sa.Column('dri_user_id', postgresql.UUID(as_uuid=True), nullable=True,
                      comment='DRI负责人'),
            sa.Column('collaborators', postgresql.JSONB(astext_type=sa.Text()),
                      server_default='[]', comment='[{user_id,role}]'),
            ...
            sa.Column('resolution', sa.TEXT(), nullable=True,
                      comment='解决结果'),
            sa.Column('source', sa.VARCHAR(30), server_default='manual',
                      comment='manual/ai_alert/system'),
        ...
        op.create_index('idx_dri_wo_store_status', 'dri_work_orders',
                        ['tenant_id', 'store_id', 'status'])
        op.create_index('idx_dri_wo_active', 'dri_work_orders',
                        ['tenant_id', 'status', 'due_date'],
                        postgresql_where=sa.text(
                            "status NOT IN ('completed','closed','cancelled')"))
        op.create_index('idx_dri_wo_dri_user', 'dri_work_orders',
                        ['tenant_id', 'dri_user_id', 'status'])
    ...
    if 'ai_alerts' not in existing:
        op.create_table(
            ...
            sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True,
                      server_default=sa.text('gen_random_uuid()')),
            sa.Column('tenant_id', postgresql.UUID(as_uuid=True), nullable=False),
            sa.Column('alert_type', sa.VARCHAR(30), nullable=False,
            ...
            sa.Column('suggestion', postgresql.JSONB(astext_type=sa.Text()),
                      server_default='{}',
            ...
            sa.CheckConstraint(
                ...
                name='chk_ai_alert_severity'),
        )
        op.create_index('idx_ai_alert_tenant', 'ai_alerts', ['tenant_id'])
        op.create_index('idx_ai_alert_resolved', 'ai_alerts',
                        ['tenant_id', 'resolved', 'severity', sa.text('created_at DESC')])
        op.create_index('idx_ai_alert_store_type', 'ai_alerts',
                        ['tenant_id', 'store_id', 'alert_type', 'resolved'])
        op.create_index('idx_ai_alert_employee', 'ai_alerts',
                        ['tenant_id', 'employee_id'],
    ...
    if 'coach_sessions' not in existing:
        op.create_table(
            ...
            sa.Column('focus_employees', postgresql.JSONB(astext_type=sa.Text()),
                      server_default='[]',
                      comment='[{employee_id,reason,action}]'),
            sa.Column('readiness_before', sa.Numeric(5, 2), nullable=True,
                      comment='教练前就绪分'),
            sa.Column('readiness_after', sa.Numeric(5, 2), nullable=True,
                      comment='教练后就绪分'),
            sa.Column('notes', sa.TEXT(), nullable=True),
            sa.Column('is_deleted', sa.Boolean(), nullable=False, server_default='false'),
            sa.Column('created_at', sa.TIMESTAMP(timezone=True),
        ...
        op.create_index('idx_coach_store_date', 'coach_sessions',
                        ['tenant_id', 'store_id', sa.text('session_date DESC')])
        op.create_index('idx_coach_manager_date', 'coach_sessions',
                        ['tenant_id', 'manager_id', sa.text('session_date DESC')])
