    ...
    if "agent_autonomy_configs" not in existing:
        op.create_table(
            ...
            sa.Column(
                ...
                server_default="1",
                nullable=False,
                comment="自治等级: 1=仅建议, 2=半自动, 3=全自治",
            ),
            sa.Column(
                "auto_rules", postgresql.JSONB, server_default=sa.text("'[]'::jsonb"), comment="自动执行规则列表"
            ),
        ...
        op.create_index(
            "ix_agent_autonomy_configs_tenant_agent", "agent_autonomy_configs", ["tenant_id", "agent_id"], unique=True
        )
    ...
    if "agent_auto_executions" not in existing:
        op.create_table(
            ...
            sa.Column(
                ...
                nullable=False,
                comment="状态: executed/pending/rejected/failed",
            ),
