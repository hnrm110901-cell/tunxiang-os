...
def upgrade() -> None:
    ...
    if 'banquet_menu_plans' not in existing:
        ...
        op.execute("""
            DO $$ BEGIN
                IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='banquet_menu_plans' AND policyname='bmp_tenant') THEN
                    EXECUTE 'CREATE POLICY bmp_tenant ON banquet_menu_plans
    ...
    if 'banquet_sessions' not in existing:
        ...
        op.execute("""
            DO $$ BEGIN
                IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='banquet_sessions' AND policyname='bs_tenant') THEN
    ...
    if 'banquet_split_bills' not in existing:
        ...
        op.execute("""
            DO $$ BEGIN
                IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='banquet_split_bills' AND policyname='bsb_tenant') THEN
                    EXECUTE 'CREATE POLICY bsb_tenant ON banquet_split_bills
                        USING (tenant_id = NULLIF(current_setting(''app.tenant_id'', true), '''')::UUID)
                        WITH CHECK (tenant_id = NULLIF(current_setting(''app.tenant_id'', true), '''')::UUID)';
                END IF;
            END$$;
