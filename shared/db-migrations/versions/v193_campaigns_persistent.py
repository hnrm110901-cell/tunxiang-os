...
def upgrade() -> None:
    ...
    op.add_column(
        "campaigns",
        sa.Column(
            "start_at", sa.TIMESTAMP(timezone=True), nullable=True, comment="活动开始时间（标准命名，兼容 start_time）"
        ),
    )
    op.add_column(
        "campaigns",
        sa.Column(
            "end_at", sa.TIMESTAMP(timezone=True), nullable=True, comment="活动结束时间（标准命名，兼容 end_time）"
        ),
    )
    ...
    op.add_column(
        "campaigns",
        sa.Column("priority", sa.Integer(), server_default="0", nullable=False, comment="优先级，冲突时高优先执行"),
    )

    # 每会员最多参与次数
    op.add_column(
        "campaigns", sa.Column("max_per_member", sa.Integer(), nullable=True, comment="每会员最多参与次数，NULL=不限")
    )

    # 参与人次（汇总计数）
    op.add_column(
        "campaigns",
        sa.Column("total_participants", sa.Integer(), server_default="0", nullable=False, comment="参与人次"),
    )

    # 已使用金额（分）
    op.add_column(
        "campaigns",
        sa.Column("used_fen", sa.BigInteger(), server_default="0", nullable=False, comment="已使用金额（分）"),
    )

    # 创建人
    op.add_column(
        "campaigns", sa.Column("created_by", postgresql.UUID(as_uuid=True), nullable=True, comment="创建人 UUID")
    )
    ...
    op.add_column(
        ...
        sa.Column(
            "member_id", postgresql.UUID(as_uuid=True), nullable=True, comment="会员 UUID（标准命名，兼容 customer_id）"
        ),
    )
    ...
    op.add_column(
        "campaign_participants",
        sa.Column("order_id", postgresql.UUID(as_uuid=True), nullable=True, comment="关联订单 UUID"),
    )

    # 参与类型
    op.add_column(
        "campaign_participants",
        sa.Column("participation_type", sa.VARCHAR(20), nullable=True, comment="joined/used/expired"),
    )
    ...
    op.add_column(
        "campaign_participants",
        sa.Column("points_earned", sa.Integer(), server_default="0", nullable=False, comment="本次获得积分"),
    )

    # 门店
    op.add_column(
        "campaign_participants",
        sa.Column("store_id", postgresql.UUID(as_uuid=True), nullable=True, comment="服务门店 UUID"),
    )

    # ─── 3. 新增索引（高频查询路径）─────────────────────────────────────────────

    # campaigns — tenant 基础索引（v097 已有复合索引，这里补纯 tenant 索引）
    op.create_index("idx_campaigns_tenant", "campaigns", ["tenant_id"], postgresql_where=sa.text("is_deleted = false"))
    ...
    op.create_index("idx_campaign_participants_tenant", "campaign_participants", ["tenant_id"])

    # campaign_participants — campaign
    op.create_index("idx_campaign_participants_campaign", "campaign_participants", ["campaign_id"])

    # campaign_participants — 会员参与历史（member_id + tenant）
    op.create_index(
        "idx_campaign_participants_member",
        "campaign_participants",
        ["tenant_id", "member_id"],
...
def downgrade() -> None:
    ...
    for col in ("store_id", "points_earned", "discount_applied_fen", "participation_type", "order_id", "member_id"):
