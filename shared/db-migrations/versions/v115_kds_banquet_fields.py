...
def upgrade() -> None:
    ...
    op.add_column(
        "kds_tasks",
        sa.Column("banquet_session_id", UUID(as_uuid=True), nullable=True, comment="关联宴席场次ID，NULL=普通堂食任务"),
    )
    op.add_column(
        "kds_tasks",
        sa.Column(
    ...
    op.add_column(
        "kds_tasks",
        sa.Column("weigh_record_id", UUID(as_uuid=True), nullable=True, comment="关联活鲜称重记录ID（仅活鲜菜品有）"),
    )
    op.add_column(
        "kds_tasks",
        sa.Column(
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_dish_dept_mappings_dept ON dish_dept_mappings (dept_id) WHERE dept_id IS NOT NULL"
    )
...
def downgrade() -> None:
    ...
    for col in ["banquet_session_id", "banquet_section_id", "weigh_record_id", "is_live_seafood", "weigh_confirmed"]:
