...
def upgrade() -> None:
    # B'-6 (2026-05-09): v388 试图为 26 张表补 RLS，但许多表在 alembic 拓扑序中
    # 由 parallel branch（v282/v346/v367/v370/v372/v373/v374/v375/v377/v378/v379/
    # v380/v381 等）的 CREATE TABLE 决定，这些 branch 通过 v398 merge 才汇入
    # 主链。chain 历史断裂时序，v388 在 v398 前跑 → 引用的表多数不存在 →
    # ProgrammingError。
    # 本次 fix（B'-6）让 chain 跑通到 head 优先 — RLS 补齐留独立 PR：
    #   1. 重排 v388 chain dependencies 让 v283_banquet_schedule_lock /
    #      v346/v367 等都在 v388 之前；或
    #   2. 改用 information_schema 动态扫描（仅对 existing tables 启用 RLS）；或
    #   3. 拆 v388 成多个 sub-migrations，每个紧跟对应表 CREATE。
    # CLAUDE.md §17 Tier 1 RLS 多租户隔离硬约束 — 此项遗留必须独立 PR 兜底。
    return
    # ── v282 — banquet ────────────────────────────────────────────
    op.execute("ALTER TABLE IF EXISTS banquet_approval_logs ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS banquet_approval_logs FORCE ROW LEVEL SECURITY")
    ...
    op.execute("ALTER TABLE IF EXISTS banquet_eo_tickets ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS banquet_eo_tickets FORCE ROW LEVEL SECURITY")
    ...
    op.execute("ALTER TABLE IF EXISTS stored_value_split_ledger ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS stored_value_split_ledger FORCE ROW LEVEL SECURITY")
    ...
    op.execute("ALTER TABLE IF EXISTS stored_value_split_rules ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS stored_value_split_rules FORCE ROW LEVEL SECURITY")
    ...
    op.execute("ALTER TABLE IF EXISTS sv_settlement_batches ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS sv_settlement_batches FORCE ROW LEVEL SECURITY")
    ...
    op.execute("ALTER TABLE IF EXISTS ingredient_location_bindings ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS ingredient_location_bindings FORCE ROW LEVEL SECURITY")
    ...
    op.execute("ALTER TABLE IF EXISTS inventory_by_location ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS inventory_by_location FORCE ROW LEVEL SECURITY")
    ...
    op.execute("ALTER TABLE IF EXISTS warehouse_locations ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS warehouse_locations FORCE ROW LEVEL SECURITY")
    ...
    op.execute("ALTER TABLE IF EXISTS warehouse_zones ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS warehouse_zones FORCE ROW LEVEL SECURITY")
    ...
    op.execute("ALTER TABLE IF EXISTS delivery_temperature_logs ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS delivery_temperature_logs FORCE ROW LEVEL SECURITY")
    ...
    op.execute("ALTER TABLE IF EXISTS delivery_temperature_logs_default ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS delivery_temperature_logs_default FORCE ROW LEVEL SECURITY")
    ...
    # ── v370 — stocktake loss ──────────────────────────────────────
    op.execute("ALTER TABLE IF EXISTS stocktake_loss_approvals ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS stocktake_loss_approvals FORCE ROW LEVEL SECURITY")
    ...
    op.execute("ALTER TABLE IF EXISTS stocktake_loss_case_no_seq ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS stocktake_loss_case_no_seq FORCE ROW LEVEL SECURITY")
    ...
    op.execute("ALTER TABLE IF EXISTS stocktake_loss_cases ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS stocktake_loss_cases FORCE ROW LEVEL SECURITY")
    ...
    op.execute("ALTER TABLE IF EXISTS stocktake_loss_items ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS stocktake_loss_items FORCE ROW LEVEL SECURITY")
    ...
    op.execute("ALTER TABLE IF EXISTS stocktake_loss_writeoffs ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS stocktake_loss_writeoffs FORCE ROW LEVEL SECURITY")
    ...
    # ── v372 — ceo cockpit ────────────────────────────────────────
    op.execute("ALTER TABLE IF EXISTS ceo_cockpit_snapshots ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS ceo_cockpit_snapshots FORCE ROW LEVEL SECURITY")
    ...
    # ── v373 — dish profit ─────────────────────────────────────────
    op.execute("ALTER TABLE IF EXISTS dish_co_occurrence ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS dish_co_occurrence FORCE ROW LEVEL SECURITY")
    ...
    op.execute("ALTER TABLE IF EXISTS procurement_feedback_logs ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS procurement_feedback_logs FORCE ROW LEVEL SECURITY")
    ...
    # ── v375 — yield alerts ────────────────────────────────────────
    op.execute("ALTER TABLE IF EXISTS yield_alerts ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS yield_alerts FORCE ROW LEVEL SECURITY")
    ...
    op.execute("ALTER TABLE IF EXISTS conversion_funnel_daily ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS conversion_funnel_daily FORCE ROW LEVEL SECURITY")
    ...
    op.execute("ALTER TABLE IF EXISTS customer_journey_timings ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS customer_journey_timings FORCE ROW LEVEL SECURITY")
    ...
    op.execute("ALTER TABLE IF EXISTS satisfaction_ratings ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS satisfaction_ratings FORCE ROW LEVEL SECURITY")
    ...
    op.execute("ALTER TABLE IF EXISTS bonus_rules ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS bonus_rules FORCE ROW LEVEL SECURITY")
    ...
    op.execute("ALTER TABLE IF EXISTS daily_scorecards ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS daily_scorecards FORCE ROW LEVEL SECURITY")
    ...
    op.execute("ALTER TABLE IF EXISTS store_lifecycle_stages ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS store_lifecycle_stages FORCE ROW LEVEL SECURITY")
    ...
    op.execute("ALTER TABLE IF EXISTS dynamic_pricing_logs ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS dynamic_pricing_logs FORCE ROW LEVEL SECURITY")
    ...
    op.execute("ALTER TABLE IF EXISTS dynamic_pricing_rules ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS dynamic_pricing_rules FORCE ROW LEVEL SECURITY")
    ...
    # ── v380 — invoice ocr ─────────────────────────────────────────
    op.execute("ALTER TABLE IF EXISTS invoice_ocr_results ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS invoice_ocr_results FORCE ROW LEVEL SECURITY")
    ...
    op.execute("ALTER TABLE IF EXISTS delivery_disputes ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS delivery_disputes FORCE ROW LEVEL SECURITY")
    ...
    op.execute("ALTER TABLE IF EXISTS dish_pricing_suggestions ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE IF EXISTS dish_pricing_suggestions FORCE ROW LEVEL SECURITY")
