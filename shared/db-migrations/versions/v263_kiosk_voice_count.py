...
def upgrade() -> None:
    ...
    if "kiosk_terminals" not in existing:
        op.create_table(
            ...
            sa.Column("terminal_code", sa.String(50), nullable=False,
                      comment="终端编号，如：KIOSK-01"),
            sa.Column("terminal_name", sa.String(100), nullable=False,
                      comment="终端名称，如：1号自助点单机"),
            sa.Column("terminal_type", sa.String(30), nullable=True,
            ...
            sa.Column("payment_modes", JSONB, nullable=True,
                      server_default="'[]'",
                      comment="支持的支付方式列表，如：[wechat, alipay, unionpay]"),
            sa.Column("welcome_screen_config", JSONB, nullable=True,
                      server_default="'{}'",
            ...
            sa.Column("ad_images", JSONB, nullable=True,
                      server_default="'[]'",
                      comment="广告轮播图URL列表"),
            ...
            sa.Column("updated_at", sa.TIMESTAMP(timezone=True),
                      server_default=sa.text("now()"), nullable=False),
            sa.UniqueConstraint("tenant_id", "terminal_code",
                                name="uq_kiosk_terminals_tenant_code"),
        )
        op.create_index("idx_kiosk_terminals_store",
                        "kiosk_terminals", ["tenant_id", "store_id"])
        op.create_index("idx_kiosk_terminals_status",
                        "kiosk_terminals", ["tenant_id", "status"])

    op.execute("ALTER TABLE kiosk_terminals ENABLE ROW LEVEL SECURITY;")
    op.execute(
        "DROP POLICY IF EXISTS kiosk_terminals_tenant ON kiosk_terminals;"
    )
    op.execute("""
    ...
    if "kiosk_carts" not in existing:
        op.create_table(
            ...
            sa.Column("id", UUID(as_uuid=True), primary_key=True,
                      server_default=sa.text("gen_random_uuid()")),
            sa.Column("tenant_id", UUID(as_uuid=True), nullable=False),
            sa.Column("terminal_id", UUID(as_uuid=True), nullable=False,
                      comment="关联 kiosk_terminals.id"),
            sa.Column("session_token", sa.String(100), nullable=False,
                      comment="会话令牌，唯一标识一次点单会话"),
            ...
            sa.Column("subtotal_fen", sa.BigInteger, nullable=False,
                      server_default="0",
                      comment="小计金额（分）"),
            sa.Column("discounts", JSONB, nullable=True,
                      server_default="'[]'",
                      comment="优惠明细列表"),
            ...
            sa.Column("expires_at", sa.TIMESTAMP(timezone=True), nullable=False,
                      comment="购物车过期时间，过期后应用层清理"),
            sa.Column("created_at", sa.TIMESTAMP(timezone=True),
                      server_default=sa.text("now()"), nullable=False),
            sa.Column("updated_at", sa.TIMESTAMP(timezone=True),
                      server_default=sa.text("now()"), nullable=False),
            sa.ForeignKeyConstraint(
                ["terminal_id"], ["kiosk_terminals.id"], ondelete="CASCADE"
            ),
            sa.UniqueConstraint("terminal_id", "session_token",
                                name="uq_kiosk_carts_terminal_session"),
        )
        op.create_index("idx_kiosk_carts_terminal",
                        "kiosk_carts", ["terminal_id", "session_token"])
        op.create_index("idx_kiosk_carts_expires",
                        "kiosk_carts", ["expires_at"])

    op.execute("ALTER TABLE kiosk_carts ENABLE ROW LEVEL SECURITY;")
    op.execute(
        "DROP POLICY IF EXISTS kiosk_carts_tenant ON kiosk_carts;"
    )
    op.execute("""
    ...
    if "kiosk_orders" not in existing:
        op.create_table(
            ...
            sa.Column("cart_id", UUID(as_uuid=True), nullable=True,
                      comment="关联 kiosk_carts.id，下单后购物车可复用或归档"),
            sa.Column("order_no", sa.String(50), nullable=False, unique=True,
                      comment="速点订单号，系统自动生成"),
            sa.Column("queue_number", sa.String(20), nullable=True,
                      comment="叫号编号，格式：K001"),
            sa.Column("member_id", UUID(as_uuid=True), nullable=True,
                      comment="关联会员 ID"),
            sa.Column("dining_type", sa.String(20), nullable=True,
            ...
            sa.Column("subtotal_fen", sa.BigInteger, nullable=False,
                      comment="小计金额（分）"),
            sa.Column("discount_fen", sa.BigInteger, nullable=False,
                      server_default="0",
                      comment="优惠金额（分）"),
            sa.Column("total_fen", sa.BigInteger, nullable=False,
                      comment="实付金额（分）"),
            ...
            sa.Column("updated_at", sa.TIMESTAMP(timezone=True),
                      server_default=sa.text("now()"), nullable=False),
            sa.ForeignKeyConstraint(
                ["terminal_id"], ["kiosk_terminals.id"]
            ),
            sa.ForeignKeyConstraint(
                ["cart_id"], ["kiosk_carts.id"]
            ),
        )
        op.create_index("idx_kiosk_orders_store",
                        "kiosk_orders", ["tenant_id", "store_id"])
        op.create_index("idx_kiosk_orders_terminal",
                        "kiosk_orders", ["terminal_id", "status"])
        op.create_index("idx_kiosk_orders_created",
                        "kiosk_orders", ["tenant_id", sa.text("created_at DESC")])

    op.execute("ALTER TABLE kiosk_orders ENABLE ROW LEVEL SECURITY;")
    op.execute(
        "DROP POLICY IF EXISTS kiosk_orders_tenant ON kiosk_orders;"
    )
    op.execute("""
    ...
    if "kiosk_payments" not in existing:
        op.create_table(
            ...
            sa.Column("id", UUID(as_uuid=True), primary_key=True,
                      server_default=sa.text("gen_random_uuid()")),
            sa.Column("tenant_id", UUID(as_uuid=True), nullable=False),
            sa.Column("kiosk_order_id", UUID(as_uuid=True), nullable=False,
                      comment="关联 kiosk_orders.id"),
            ...
            sa.Column("transaction_no", sa.String(100), nullable=True,
                      comment="第三方支付流水号（微信/支付宝单号）"),
            sa.Column("scan_code", sa.String(200), nullable=True,
                      comment="主扫时顾客出示的付款码内容"),
            sa.Column("qr_polling_key", sa.String(100), nullable=True,
                      comment="被扫时生成的二维码轮询 key"),
            sa.Column("paid_at", sa.TIMESTAMP(timezone=True), nullable=True,
                      comment="实际支付完成时间"),
            sa.Column("created_at", sa.TIMESTAMP(timezone=True),
                      server_default=sa.text("now()"), nullable=False),
            sa.ForeignKeyConstraint(
                ["kiosk_order_id"], ["kiosk_orders.id"]
            ),
        )
        op.create_index("idx_kiosk_payments_order",
                        "kiosk_payments", ["kiosk_order_id"])
        op.execute("""
    ...
    op.execute(
        "DROP POLICY IF EXISTS kiosk_payments_tenant ON kiosk_payments;"
    )
    op.execute("""
    ...
    if "voice_count_sessions" not in existing:
        op.create_table(
            ...
            sa.Column("warehouse_id", UUID(as_uuid=True), nullable=True,
                      comment="仓库 ID，NULL 表示门店默认仓库"),
            sa.Column("count_type", sa.String(20), nullable=True,
                      server_default="'full'",
                      comment="full=全盘 / partial=部分盘 / spot=抽盘"),
            sa.Column("category_filter", JSONB, nullable=True,
                      server_default="'[]'",
            ...
            sa.Column("operator_id", UUID(as_uuid=True), nullable=True,
                      comment="操作员工 ID"),
            sa.Column("item_list", JSONB, nullable=True,
                      server_default="'[]'",
                      comment="[{item_id, item_name, unit, expected_count}]，盘点目标清单"),
            sa.Column("total_items", sa.Integer, nullable=False,
                      server_default="0",
                      comment="计划盘点品项总数"),
            sa.Column("counted_items", sa.Integer, nullable=False,
                      server_default="0",
                      comment="已录入品项数"),
            ...
            sa.Column("closed_at", sa.TIMESTAMP(timezone=True), nullable=True,
                      comment="关闭时间（关闭或提交后写入）"),
            sa.Column("submitted_at", sa.TIMESTAMP(timezone=True), nullable=True,
                      comment="提交正式盘点单的时间"),
        )
        op.create_index("idx_voice_sessions_store",
                        "voice_count_sessions", ["tenant_id", "store_id", "status"])

    op.execute("ALTER TABLE voice_count_sessions ENABLE ROW LEVEL SECURITY;")
    op.execute(
        "DROP POLICY IF EXISTS voice_count_sessions_tenant ON voice_count_sessions;"
    )
    op.execute("""
    ...
    if "voice_count_entries" not in existing:
        op.create_table(
            ...
            sa.Column("session_id", UUID(as_uuid=True), nullable=False,
                      comment="关联 voice_count_sessions.id"),
            sa.Column("item_id", UUID(as_uuid=True), nullable=False,
                      comment="物料 ID（逻辑引用 ingredients.id）"),
            sa.Column("item_name", sa.String(200), nullable=False,
                      comment="物料名称（快照，防止改名后历史数据失真）"),
            sa.Column("quantity", sa.Numeric(12, 3), nullable=False,
                      comment="盘点数量"),
            sa.Column("unit", sa.String(20), nullable=True,
                      comment="计量单位，如：公斤/个/箱"),
            sa.Column("source", sa.String(20), nullable=True,
                      server_default="'voice'",
                      comment="voice=语音录入 / manual=手动修正"),
            sa.Column("original_text", sa.Text, nullable=True,
                      comment="原始语音识别文字，用于审计和二次解析"),
            sa.Column("asr_confidence", sa.Numeric(4, 3), nullable=True,
                      comment="ASR 识别置信度，0.000~1.000"),
            sa.Column("match_confidence", sa.String(10), nullable=True,
                      comment="物料匹配置信度：high=高 / medium=中 / low=低"),
            sa.Column("created_at", sa.TIMESTAMP(timezone=True),
                      server_default=sa.text("now()"), nullable=False),
            sa.Column("updated_at", sa.TIMESTAMP(timezone=True),
                      server_default=sa.text("now()"), nullable=False),
            sa.ForeignKeyConstraint(
                ["session_id"], ["voice_count_sessions.id"], ondelete="CASCADE"
            ),
            sa.UniqueConstraint("session_id", "item_id",
                                name="uq_voice_count_entries_session_item"),
        )
        op.create_index("idx_voice_entries_session",
                        "voice_count_entries", ["session_id"])

    op.execute("ALTER TABLE voice_count_entries ENABLE ROW LEVEL SECURITY;")
    op.execute(
        "DROP POLICY IF EXISTS voice_count_entries_tenant ON voice_count_entries;"
    )
    op.execute("""
    ...
    if "inventory_count_sheets" not in existing:
        op.create_table(
            ...
            sa.Column("sheet_no", sa.String(50), nullable=False, unique=True,
            ...
            sa.Column("total_items", sa.Integer, nullable=False,
                      server_default="0",
                      comment="盘点品项总数"),
            sa.Column("variance_items", sa.Integer, nullable=False,
                      server_default="0",
                      comment="存在差异的品项数"),
            sa.Column("total_variance_amount_fen", sa.BigInteger, nullable=False,
                      server_default="0",
            ...
            sa.Column("approved_at", sa.TIMESTAMP(timezone=True), nullable=True,
                      comment="审批时间"),
            sa.ForeignKeyConstraint(
                ["session_id"], ["voice_count_sessions.id"]
            ),
        )
        op.create_index("idx_count_sheets_store",
                        "inventory_count_sheets",
                        ["tenant_id", "store_id", sa.text("count_date DESC")])

    op.execute("ALTER TABLE inventory_count_sheets ENABLE ROW LEVEL SECURITY;")
    op.execute(
        "DROP POLICY IF EXISTS inventory_count_sheets_tenant ON inventory_count_sheets;"
    )
    op.execute("""
