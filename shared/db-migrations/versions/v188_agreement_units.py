...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('settlement_cycle', sa.VARCHAR(20), nullable=True,
                  comment='monthly/weekly/custom'),
        sa.Column('settlement_day', sa.Integer(), nullable=True,
                  comment='月结算日，如15=每月15号'),
    ...
    op.create_table(
        ...
        sa.Column('balance_fen', sa.BIGINT(), nullable=False, server_default='0',
                  comment='当前余额（分），可为负=已欠款'),
        sa.Column('credit_used_fen', sa.BIGINT(), nullable=False, server_default='0',
                  comment='已用授信额度'),
        sa.Column('total_consumed_fen', sa.BIGINT(), nullable=False, server_default='0',
                  comment='累计消费'),
        sa.Column('total_repaid_fen', sa.BIGINT(), nullable=False, server_default='0',
                  comment='累计还款'),
    ...
    op.create_table(
        ...
        sa.Column('account_id', postgresql.UUID(as_uuid=True), nullable=True),
        sa.Column('type', sa.VARCHAR(20), nullable=False,
                  comment='charge/repay/manual_charge'),
        sa.Column('amount_fen', sa.BIGINT(), nullable=False,
                  comment='挂账为正，还款为负'),
        sa.Column('order_id', postgresql.UUID(as_uuid=True), nullable=True,
                  comment='挂账时关联订单'),
        sa.Column('operator_id', postgresql.UUID(as_uuid=True), nullable=True),
        sa.Column('repay_method', sa.VARCHAR(30), nullable=True,
                  comment='cash/transfer/wechat'),
        sa.Column('notes', sa.Text(), nullable=True),
        sa.Column('created_at', sa.TIMESTAMP(timezone=True),
                  server_default=sa.text('NOW()'), nullable=False),
    )
    ...
    op.create_index('idx_agreement_transactions_created_at',
                    'agreement_transactions', ['tenant_id', 'created_at'])
    op.create_index('idx_agreement_transactions_order_id',
                    'agreement_transactions', ['order_id'],
                    postgresql_where=sa.text("order_id IS NOT NULL"))

    # ─── RLS 策略（4张表） ────────────────────────────────────────────────────
    for table in ('agreement_units', 'agreement_accounts',
                  'prepaid_records', 'agreement_transactions'):
