...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("customer_id", UUID(as_uuid=True), nullable=False, comment="内部 Golden ID"),
        sa.Column("channel_type", sa.String(20), nullable=False, comment="meituan/eleme/douyin/wechat"),
        sa.Column("channel_openid", sa.String(128), nullable=False),
        sa.Column("phone_hash", sa.String(64), nullable=True, comment="sha256(phone+salt)，隐私保护"),
        sa.Column(
            "binding_status", sa.String(20), nullable=False, server_default="active", comment="active/unbound/conflict"
        ),
    ...
    # 索引
    op.create_index("ix_member_channel_bindings_tenant", "member_channel_bindings", ["tenant_id"])
    op.create_index("ix_member_channel_bindings_customer", "member_channel_bindings", ["customer_id"])
    op.create_index(
    ...
    op.create_table(
        ...
        sa.Column("merge_reason", sa.String(50), nullable=False, comment="phone_match/manual/auto_rule"),
    ...
    # 索引
    op.create_index("ix_golden_id_merge_logs_tenant", "golden_id_merge_logs", ["tenant_id"])
    op.create_index("ix_golden_id_merge_logs_source", "golden_id_merge_logs", ["source_customer_id"])
    op.create_index("ix_golden_id_merge_logs_target", "golden_id_merge_logs", ["target_customer_id"])
