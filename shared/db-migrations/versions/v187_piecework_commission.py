...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "store_id", postgresql.UUID(as_uuid=True), nullable=True, comment="NULL=集团通用，非NULL=仅对该门店生效"
        ),
        sa.Column("name", sa.VARCHAR(50), nullable=False, comment="如：热菜区/凉菜区/传菜组"),
    ...
    op.create_table(
        ...
        sa.Column("calc_type", sa.VARCHAR(20), nullable=False, comment="by_dish=按品项计件 / by_method=按做法计件"),
        sa.Column(
            "applicable_role", sa.VARCHAR(50), nullable=False, comment="chef=厨师 / waiter=服务员 / runner=传菜员"
        ),
        sa.Column("effective_date", sa.Date(), nullable=True, comment="生效日期，NULL=即时生效"),
        sa.Column("is_active", sa.Boolean(), nullable=False, server_default="TRUE"),
        sa.Column("created_at", sa.TIMESTAMP(timezone=True), server_default=sa.text("NOW()")),
        sa.Column("updated_at", sa.TIMESTAMP(timezone=True), server_default=sa.text("NOW()")),
        sa.ForeignKeyConstraint(
            ["zone_id"], ["piecework_zones.id"], name="fk_piecework_schemes_zone", ondelete="SET NULL"
        ),
    )
    op.execute(
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_piecework_schemes_zone ON piecework_schemes (zone_id) WHERE zone_id IS NOT NULL"
    )
    ...
    op.create_table(
        ...
        sa.Column("dish_id", postgresql.UUID(as_uuid=True), nullable=True, comment="calc_type=by_dish 时使用"),
        sa.Column("method_id", postgresql.UUID(as_uuid=True), nullable=True, comment="calc_type=by_method 时使用"),
        sa.Column("dish_name", sa.VARCHAR(100), nullable=True, comment="冗余存储品项/做法名称，避免跨服务join"),
        sa.Column("unit_fee_fen", sa.Integer(), nullable=False, comment="每件提成金额，单位：分"),
        sa.Column("min_qty", sa.Integer(), nullable=False, server_default="1", comment="最低起算件数"),
        sa.Column("created_at", sa.TIMESTAMP(timezone=True), server_default=sa.text("NOW()")),
        sa.ForeignKeyConstraint(
            ["scheme_id"], ["piecework_schemes.id"], name="fk_piecework_scheme_items_scheme", ondelete="CASCADE"
        ),
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_piecework_scheme_items_scheme ON piecework_scheme_items (tenant_id, scheme_id)"
    )
    ...
    op.create_table(
        ...
        sa.Column("unit_fee_fen", sa.Integer(), nullable=False, comment="计件时快照的单价，单位：分"),
        ...
        sa.Column("order_id", postgresql.UUID(as_uuid=True), nullable=True, comment="关联订单ID（来自tx-trade）"),
