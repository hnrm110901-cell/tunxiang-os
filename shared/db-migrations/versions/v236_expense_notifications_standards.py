...
Revision ID: v236
Revises: v235b + v235c (merge — see chain repair note below)
Create Date: 2026-04-12

Chain repair (2026-05-09, B'-3): 原 down_revision = "v235b" 漏依赖 v235c。
  v236 的 expense_notifications 表 FK references expense_applications，但
  expense_applications 由 v235c 创建（v235b 是 LightRAG 知识图谱，与费控无关）。
  原链路让 alembic 走 v235b → v236，v235c 成 leaf head 等 v398 收敛。但 v236 的
  CREATE TABLE 在 v235c 未跑前找不到 expense_applications → ProgrammingError。
  改 merge migration tuple `("v235b", "v235c")`：v236 同时依赖两条 v235 平行
  分支，alembic 拓扑序保证两者都在 v236 之前完成。
"""

from alembic import op

revision = "v236"
down_revision = ("v235b", "v235c")
branch_labels = None
depends_on = None
