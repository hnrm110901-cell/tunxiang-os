...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("total_amount", sa.BigInteger(), nullable=False,
                  comment="总金额，单位：分(fen)，展示时除以100转元"),
        sa.Column("paid_amount", sa.BigInteger(), nullable=False,
                  server_default="0",
                  comment="已付金额，单位：分(fen)"),
    ...
    op.create_table(
        ...
        sa.Column("order_item_id", UUID(as_uuid=True), nullable=True,
                  comment="tx-supply 采购行ID"),
        sa.Column("product_name", sa.String(200), nullable=True,
                  comment="商品名称（冗余存储）"),
        sa.Column("quantity", NUMERIC(12, 3), nullable=True,
                  comment="数量"),
    ...
    op.create_table(
        ...
        sa.Column("payment_amount", sa.BigInteger(), nullable=True,
                  comment="付款单金额，单位：分(fen)"),
        sa.Column("invoice_amount", sa.BigInteger(), nullable=True,
                  comment="发票总金额，单位：分(fen)"),
        sa.Column("discrepancy_amount", sa.BigInteger(), nullable=False,
                  server_default="0",
...
def downgrade() -> None:
    ...
    # procurement_reconciliations
    op.execute(
        "DROP POLICY IF EXISTS procurement_reconciliations_tenant_isolation "
        "ON procurement_reconciliations"
    )
    ...
    # procurement_payment_items
    op.execute(
        "DROP POLICY IF EXISTS procurement_payment_items_tenant_isolation "
        "ON procurement_payment_items"
    )
    ...
    # procurement_payments
    op.execute(
        "DROP POLICY IF EXISTS procurement_payments_tenant_isolation "
        "ON procurement_payments"
    )
