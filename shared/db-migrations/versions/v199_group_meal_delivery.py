...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("company_code", sa.VARCHAR(30), unique=True, nullable=True, comment="企业编码，全局唯一"),
        ...
        sa.Column("credit_limit_fen", sa.BigInteger(), nullable=False, server_default="0", comment="授信额度（分）"),
        sa.Column("used_credit_fen", sa.BigInteger(), nullable=False, server_default="0", comment="已用授信（分）"),
    ...
    # 索引
    op.create_index("idx_corporate_customers_tenant", "corporate_customers", ["tenant_id"])
    op.create_index("idx_corporate_customers_status", "corporate_customers", ["tenant_id", "status"])
    ...
    op.create_table(
        ...
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column("order_id", postgresql.UUID(as_uuid=True), nullable=False, comment="关联交易订单"),
        ...
        sa.Column("estimated_minutes", sa.Integer(), nullable=True, comment="预计配送时长（分钟）"),
        sa.Column("actual_minutes", sa.Integer(), nullable=True),
        sa.Column("rider_id", postgresql.UUID(as_uuid=True), nullable=True, comment="配送员ID"),
        ...
        sa.Column("delivery_fee_fen", sa.Integer(), nullable=False, server_default="0"),
    ...
    # 索引
    op.create_index("idx_delivery_orders_tenant", "self_delivery_orders", ["tenant_id"])
    op.create_index("idx_delivery_orders_order", "self_delivery_orders", ["order_id"])
    op.create_index("idx_delivery_orders_rider", "self_delivery_orders", ["tenant_id", "rider_id"])
    op.create_index("idx_delivery_orders_status", "self_delivery_orders", ["tenant_id", "status"])
