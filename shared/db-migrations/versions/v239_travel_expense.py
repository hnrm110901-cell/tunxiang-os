...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("store_id", UUID(as_uuid=True), nullable=False,
                  comment="申请人所属门店"),
        sa.Column("traveler_id", UUID(as_uuid=True), nullable=False,
                  comment="出行人员工ID"),
        sa.Column("applicant_id", UUID(as_uuid=True), nullable=False,
                  comment="申请人（可能不同于出行人）"),

        # 关联屯象OS巡店任务（核心打通点）
        sa.Column("inspection_task_id", UUID(as_uuid=True), nullable=True,
        ...
        sa.Column("departure_city", sa.String(50), nullable=True,
                  comment="出发城市"),
        sa.Column("destination_cities", JSONB, nullable=True,
                  server_default=sa.text("'[]'::jsonb"),
                  comment="目的地城市列表（多城市行程）"),
        sa.Column("planned_stores", JSONB, nullable=True,
                  server_default=sa.text("'[]'::jsonb"),
                  comment="计划巡店的门店ID列表"),
        ...
        sa.Column("planned_days", sa.Integer(), nullable=False,
                  server_default="1", comment="计划天数"),

        # 适用差标
        sa.Column("staff_level", sa.String(30), nullable=True,
                  comment="出行人职级（提交时固化）"),
        sa.Column("applicable_standards", JSONB, nullable=True,
                  server_default=sa.text("'{}'::jsonb"),
        ...
        sa.Column("estimated_cost_fen", sa.BigInteger(), nullable=True,
                  server_default="0",
        ...
        sa.Column("total_mileage_km", NUMERIC(10, 2), nullable=True,
                  server_default="0", comment="实际里程（公里，GPS计算）"),
        sa.Column("total_cost_fen", sa.BigInteger(), nullable=True,
                  server_default="0",
                  comment="实际总费用，单位：分(fen)，展示时除以100转元"),
        sa.Column("mileage_allowance_fen", sa.BigInteger(), nullable=True,
                  server_default="0",
    ...
    op.create_table(
        ...
        sa.Column("store_name", sa.String(100), nullable=True,
                  comment="冗余存储门店名（避免关联查询）"),

        # 时间记录
        sa.Column("checkin_time", sa.TIMESTAMP(timezone=True), nullable=True,
                  comment="到达签到时间"),
        sa.Column("checkout_time", sa.TIMESTAMP(timezone=True), nullable=True,
                  comment="离开签退时间"),
        sa.Column("duration_minutes", sa.Integer(), nullable=True,
                  server_default="0", comment="停留时长（分钟）"),

        # GPS数据
        sa.Column("checkin_location", JSONB, nullable=True,
                  comment='{"lat": float, "lng": float, "accuracy": float}'),
        sa.Column("checkout_location", JSONB, nullable=True),
        sa.Column("gps_track", JSONB, nullable=True,
                  server_default=sa.text("'[]'::jsonb"),
                  comment="轨迹点列表（精简存储，每5分钟1个点）"),
        sa.Column("distance_from_store_m", sa.Integer(), nullable=True,
                  comment="签到时距门店距离（米）"),

        # 里程
        sa.Column("leg_mileage_km", NUMERIC(10, 2), nullable=True,
                  server_default="0", comment="本段里程（到达本门店的路程）"),
        sa.Column("is_mileage_anomaly", sa.Boolean(), nullable=True,
                  server_default="false",
        ...
        # 状态
        sa.Column("itinerary_status", sa.String(20), nullable=False,
    ...
    op.create_table(
        ...
        # 分摊规则
        sa.Column("allocation_basis", sa.String(20), nullable=False,
                  server_default="duration",
                  comment="值域：duration（按停留时长）/ equal（平均分摊）/ manual（手工指定）"),
        sa.Column("basis_value", NUMERIC(10, 4), nullable=True,
                  comment="分摊基准值（如停留分钟数）"),
        sa.Column("allocation_rate", NUMERIC(7, 6), nullable=False,
                  comment="分摊比例（0.000000-1.000000）"),

        # 分摊金额
        sa.Column("total_travel_cost_fen", sa.BigInteger(), nullable=False,
                  comment="本次差旅总费用，单位：分(fen)，展示时除以100转元"),
        sa.Column("allocated_amount_fen", sa.BigInteger(), nullable=False,
                  comment="分摊到本门店的金额，单位：分(fen)，展示时除以100转元"),

        # 成本归因
        sa.Column("cost_center_id", UUID(as_uuid=True), nullable=True,
                  comment="成本中心ID（关联门店）"),
        sa.Column("is_attributed", sa.Boolean(), nullable=True,
                  server_default="false", comment="是否已归入门店P&L"),
