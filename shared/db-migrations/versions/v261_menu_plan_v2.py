...
def upgrade() -> None:
    ...
    if "menu_plans_v2" not in existing:
        ...
        op.create_index("ix_menu_plans_v2_tenant_brand",
                        "menu_plans_v2", ["tenant_id", "brand_id"])
        op.create_index("ix_menu_plans_v2_status",
                        "menu_plans_v2", ["tenant_id", "status"])
    ...
    if "menu_plan_v2_items" not in existing:
        op.create_table(
            ...
            sa.ForeignKeyConstraint(
                ["plan_id"], ["menu_plans_v2.id"], ondelete="CASCADE"
            ),
        )
        op.create_index("ix_menu_plan_v2_items_plan_id",
                        "menu_plan_v2_items", ["plan_id"])
        op.create_index("ix_menu_plan_v2_items_dish_id",
                        "menu_plan_v2_items", ["dish_id"])
    ...
    op.execute(
        "DROP POLICY IF EXISTS menu_plan_v2_items_open ON menu_plan_v2_items;"
    )
    op.execute("""
    ...
    if "menu_push_tasks" not in existing:
        op.create_table(
            ...
            sa.Column("plan_id", UUID(as_uuid=True), nullable=True,
                      comment="关联 menu_plans_v2.id"),
            sa.Column("push_mode", sa.String(20), nullable=True,
                      comment="immediate=立即推送 / scheduled=定时推送"),
            sa.Column("scheduled_at", sa.TIMESTAMP(timezone=True), nullable=True,
                      comment="定时推送时间（push_mode=scheduled 时有效）"),
            sa.Column("total_stores", sa.Integer, nullable=False,
                      server_default="0", comment="目标门店总数"),
            sa.Column("queued", sa.Integer, nullable=False,
                      server_default="0", comment="已入队数"),
            sa.Column("success", sa.Integer, nullable=False,
                      server_default="0", comment="推送成功数"),
            sa.Column("failed", sa.Integer, nullable=False,
                      server_default="0", comment="推送失败数"),
            sa.Column("created_at", sa.TIMESTAMP(timezone=True),
                      server_default=sa.text("now()"), nullable=False),
            sa.Column("is_deleted", sa.Boolean, nullable=False,
                      server_default="false"),
            sa.ForeignKeyConstraint(
                ["plan_id"], ["menu_plans_v2.id"], ondelete="SET NULL"
            ),
        )
        op.create_index("ix_menu_push_tasks_tenant",
                        "menu_push_tasks", ["tenant_id"])
        op.create_index("ix_menu_push_tasks_plan_id",
                        "menu_push_tasks", ["plan_id"])
    ...
    if "menu_push_logs" not in existing:
        op.create_table(
            ...
            sa.Column("task_id", UUID(as_uuid=True), nullable=True,
                      comment="关联 menu_push_tasks.id"),
            sa.Column("store_id", UUID(as_uuid=True), nullable=False,
                      comment="目标门店 ID"),
            sa.Column("status", sa.String(20), nullable=True,
                      comment="queued=已入队 / success=成功 / failed=失败"),
            sa.Column("error_msg", sa.Text, nullable=True,
                      comment="失败错误信息"),
            sa.Column("pushed_at", sa.TIMESTAMP(timezone=True), nullable=True,
                      comment="实际推送时间"),
            sa.ForeignKeyConstraint(
                ["task_id"], ["menu_push_tasks.id"], ondelete="CASCADE"
            ),
        )
        op.create_index("ix_menu_push_logs_task_id",
                        "menu_push_logs", ["task_id"])
        op.create_index("ix_menu_push_logs_store_id",
                        "menu_push_logs", ["store_id"])
    ...
    if "menu_plan_store_overrides" not in existing:
        op.create_table(
            ...
            sa.Column("dish_id", UUID(as_uuid=True), nullable=False,
                      comment="菜品 ID"),
            sa.Column("override_type", sa.String(30), nullable=True,
                      comment="price=价格覆盖 / availability=上下架 / portion=份量"),
            sa.Column("value", JSONB, nullable=True,
                      comment="Override 值（类型相关，如 {price_fen: 3800}）"),
            sa.Column("reason", sa.Text, nullable=True,
                      comment="Override 原因说明"),
        ...
        op.create_index("ix_menu_plan_store_overrides_tenant_store",
                        "menu_plan_store_overrides", ["tenant_id", "store_id"])
        op.create_index("ix_menu_plan_store_overrides_dish_id",
                        "menu_plan_store_overrides", ["dish_id"])
        op.create_index("ix_menu_plan_store_overrides_store_dish",
                        "menu_plan_store_overrides", ["store_id", "dish_id"])

    op.execute(
        "ALTER TABLE menu_plan_store_overrides ENABLE ROW LEVEL SECURITY;"
    )
    op.execute(
        "DROP POLICY IF EXISTS menu_plan_store_overrides_tenant "
        "ON menu_plan_store_overrides;"
    )
    op.execute("""
