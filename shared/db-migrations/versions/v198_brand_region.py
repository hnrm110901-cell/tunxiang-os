...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('name', sa.VARCHAR(100), nullable=False,
                  comment='品牌名称'),
        sa.Column('brand_code', sa.VARCHAR(20), unique=True, nullable=False,
                  comment='品牌编码：如 XJ/QQ/SG'),
        sa.Column('brand_type', sa.VARCHAR(30), nullable=True,
                  comment='seafood/hotpot/canteen/quick_service/banquet'),
        sa.Column('logo_url', sa.TEXT(), nullable=True,
                  comment='品牌Logo URL'),
        sa.Column('primary_color', sa.VARCHAR(7), nullable=False,
                  server_default='#FF6B35',
                  comment='品牌主色调（Hex）'),
        ...
        sa.Column('hq_store_id', postgresql.UUID(as_uuid=True), nullable=True,
                  comment='总店/旗舰店ID'),
    ...
    op.create_table(
        ...
        sa.Column('parent_id', postgresql.UUID(as_uuid=True), nullable=True,
                  comment='父区域ID（NULL=顶级大区）'),
        sa.Column('name', sa.VARCHAR(50), nullable=False,
                  comment='区域名称'),
        sa.Column('region_code', sa.VARCHAR(20), nullable=True,
                  comment='区域编码'),
        sa.Column('level', sa.Integer(), nullable=False,
                  server_default='1',
                  comment='1=大区 2=省 3=城市'),
        ...
        sa.Column('freight_template', postgresql.JSONB(astext_type=sa.Text()), nullable=False,
                  server_default='{}',
                  comment='运费模板配置'),
