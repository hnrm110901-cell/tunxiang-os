...
def upgrade() -> None:
    ...
    if "shift_handovers" not in _existing:
        op.create_table(
            ...
            sa.Column("shift_type", sa.String(20), nullable=False, comment="morning/afternoon/evening/night"),
            sa.Column("start_time", sa.TIMESTAMP(timezone=True), nullable=True),
            sa.Column("end_time", sa.TIMESTAMP(timezone=True), nullable=True),
            sa.Column("handover_by", UUID(as_uuid=True), nullable=True, comment="交班人员UUID"),
            sa.Column("received_by", UUID(as_uuid=True), nullable=True, comment="接班人员UUID"),
            sa.Column("cash_counted_fen", sa.Integer, nullable=False, server_default="0", comment="清点现金（分）"),
            sa.Column("pos_cash_fen", sa.Integer, nullable=False, server_default="0", comment="POS应收现金（分）"),
            sa.Column("cash_diff_fen", sa.Integer, nullable=False, server_default="0", comment="差额=清点-POS（分）"),
            sa.Column("device_checklist", JSONB, nullable=True, comment="设备检查清单 [{item, status, note}]"),
    ...
    if "daily_summaries" not in _existing:
        op.create_table(
            ...
            sa.Column(
                "total_revenue_fen", sa.Integer, nullable=False, server_default="0", comment="含折扣前总额（分）"
            ),
            sa.Column("actual_revenue_fen", sa.Integer, nullable=False, server_default="0", comment="实收金额（分）"),
            sa.Column("total_discount_fen", sa.Integer, nullable=False, server_default="0", comment="折扣总额（分）"),
            sa.Column("avg_table_value_fen", sa.Integer, nullable=False, server_default="0", comment="人均消费（分）"),
            sa.Column("max_discount_pct", sa.Numeric(5, 2), nullable=True, comment="最高折扣率 0-100"),
            sa.Column(
                "abnormal_discounts",
                sa.Integer,
                nullable=False,
    ...
    if "daily_issues" not in _existing:
        op.create_table(
            ...
            sa.Column(
                "severity", sa.String(10), nullable=False, server_default="'medium'", comment="critical/high/medium/low"
            ),
            sa.Column("title", sa.String(200), nullable=False),
            sa.Column("description", sa.Text, nullable=True),
            sa.Column("evidence_urls", JSONB, nullable=True, comment="照片/视频URL列表"),
            sa.Column("assigned_to", UUID(as_uuid=True), nullable=True, comment="指派处理人UUID"),
    ...
    if "inspection_reports" not in _existing:
        op.create_table(
            ...
            sa.Column("inspector_id", UUID(as_uuid=True), nullable=False, comment="巡店人（总部/区域经理）UUID"),
            sa.Column("overall_score", sa.Numeric(4, 1), nullable=True, comment="综合评分 0-100"),
            sa.Column("dimensions", JSONB, nullable=True, comment="[{name, score, max_score, issues:[]}]"),
            sa.Column("photos", JSONB, nullable=True, comment="[{url, caption, issue_id?}]"),
            sa.Column("action_items", JSONB, nullable=True, comment="[{item, deadline, owner}]"),
            sa.Column(
                "status",
                sa.String(20),
                nullable=False,
    ...
    if "employee_daily_performance" not in _existing:
        op.create_table(
            ...
            sa.Column("role", sa.String(30), nullable=False, comment="cashier/chef/waiter/runner"),
            ...
            sa.Column("dishes_completed", sa.Integer, nullable=False, server_default="0", comment="厨师完成菜品数"),
            sa.Column("tables_served", sa.Integer, nullable=False, server_default="0", comment="服务员服务桌数"),
            sa.Column("avg_service_score", sa.Numeric(3, 1), nullable=True, comment="顾客评分均值"),
            sa.Column("base_commission_fen", sa.Integer, nullable=False, server_default="0", comment="基础提成（分）"),
            sa.Column("created_at", sa.TIMESTAMP(timezone=True), server_default=sa.text("now()")),
            sa.Column("updated_at", sa.TIMESTAMP(timezone=True), server_default=sa.text("now()")),
            sa.UniqueConstraint("tenant_id", "store_id", "perf_date", "employee_id", name="uq_employee_daily_perf"),
        )
    op.execute("""
        DO $$ BEGIN
            IF (SELECT COUNT(*) FROM information_schema.columns 
