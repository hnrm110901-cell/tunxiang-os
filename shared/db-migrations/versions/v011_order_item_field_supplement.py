...
def upgrade() -> None:
    ...
    # =====================================================================
    op.add_column("orders", sa.Column("cashier_id", UUID(as_uuid=True), comment="收银员ID"))
    op.add_column("orders", sa.Column("service_charge_fen", sa.Integer, comment="服务费总额(分)"))
    op.add_column("orders", sa.Column("order_source", sa.String(50), comment="原始订单来源编码"))
    op.add_column("orders", sa.Column("table_transfer_from", sa.String(20), comment="转台前桌号"))

    # =====================================================================
    # order_items — 6 new columns
    # =====================================================================
    op.add_column("order_items", sa.Column("original_price_fen", sa.Integer, comment="原价/折前价(分)"))
    op.add_column("order_items", sa.Column("single_discount_fen", sa.Integer, comment="单品折扣金额(分)"))
    op.add_column("order_items", sa.Column("practice_names", sa.String(500), comment="做法名称(冗余,逗号分隔)"))
    op.add_column("order_items", sa.Column("is_gift", sa.Boolean, server_default="false", comment="是否赠菜"))
    op.add_column("order_items", sa.Column("gift_reason", sa.String(200), comment="赠菜原因"))
    op.add_column("order_items", sa.Column("combo_id", UUID(as_uuid=True), comment="所属套餐ID(NULL=非套餐)"))
