    ...
    if 'delivery_menu_sync_tasks' not in existing:
        op.create_table(
            ...
            sa.Column("platform", sa.String(20), nullable=False,
                      comment="目标平台: meituan/eleme/douyin"),
            sa.Column("sync_mode", sa.String(20), nullable=False, server_default="incremental",
        ...
        # RLS
        op.execute(
            "ALTER TABLE delivery_menu_sync_tasks ENABLE ROW LEVEL SECURITY"
        )
        op.execute("""
    ...
    if 'delivery_soldout_sync_log' not in existing:
        op.create_table(
            ...
            sa.Column("batch_id", sa.String(100), nullable=False,
                      comment="批次ID，同一次多平台推送共享"),
            sa.Column("soldout_items", postgresql.JSONB, nullable=True,
                      comment="估清菜品列表快照"),
            sa.Column("items_count", sa.Integer, nullable=False, server_default="0",
                      comment="估清菜品数"),
            sa.Column("status", sa.String(20), nullable=False, server_default="pending",
        ...
        # RLS
        op.execute(
            "ALTER TABLE delivery_soldout_sync_log ENABLE ROW LEVEL SECURITY"
        )
        op.execute("""
