...
def upgrade() -> None:
    ...
    if "employee_trainings" not in existing:
        ...
        op.create_index("idx_employee_trainings_employee", "employee_trainings", ["tenant_id", "employee_id"])
        op.create_index("idx_employee_trainings_date", "employee_trainings", ["tenant_id", "training_date"])
        op.create_index(
    ...
    if "training_plans" not in existing:
        op.create_table(
            ...
            sa.Column("required_roles", postgresql.JSONB(astext_type=sa.Text()), nullable=False, server_default="[]"),
