...
def upgrade() -> None:
    ...
    op.add_column(
        "leave_requests",
        sa.Column("approval_instance_id", UUID(as_uuid=True), nullable=True, comment="关联审批流实例 ID"),
    )
    # start_half_day / end_half_day: 支持半天请假
    op.add_column(
        "leave_requests",
        sa.Column(
            "start_half_day", sa.Boolean, nullable=False, server_default="false", comment="开始日是否半天（下午）"
        ),
    )
    op.add_column(
        "leave_requests",
        sa.Column("end_half_day", sa.Boolean, nullable=False, server_default="false", comment="结束日是否半天（上午）"),
    )
    # deduction_fen: 事假/病假扣款（分），审批通过后由薪资引擎填写
    op.add_column(
        "leave_requests",
        sa.Column(
            "deduction_fen", sa.BigInteger, nullable=False, server_default="0", comment="薪资扣减金额（分），事假/病假"
        ),
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "shift_config_id", UUID(as_uuid=True), nullable=True, comment="关联 shift_configs.id，NULL 表示当日休息"
        ),
        sa.Column("shift_name", sa.String(50), nullable=True, comment="冗余班次名称（快速查询）"),
        sa.Column("shift_start_time", sa.Time, nullable=True, comment="冗余班次开始时间"),
        sa.Column("shift_end_time", sa.Time, nullable=True, comment="冗余班次结束时间"),
        sa.Column("is_day_off", sa.Boolean, nullable=False, server_default="false", comment="是否排休/当日不上班"),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.text("NOW()")),
        sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.text("NOW()")),
        sa.Column("is_deleted", sa.Boolean, server_default="false"),
        sa.UniqueConstraint("tenant_id", "employee_id", "work_date", name="uq_employee_schedules_emp_date"),
    )
    op.create_index(
    ...
    op.add_column(
        "daily_attendance",
        sa.Column("late_minutes", sa.Integer, nullable=False, server_default="0", comment="迟到分钟数"),
    )
    op.add_column(
        "daily_attendance",
        sa.Column("early_leave_minutes", sa.Integer, nullable=False, server_default="0", comment="早退分钟数"),
    )
