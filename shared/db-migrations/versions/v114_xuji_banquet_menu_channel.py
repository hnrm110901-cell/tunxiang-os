...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("tier", sa.String(20), nullable=False, comment="standard=标准/premium=精品/luxury=豪华/custom=定制"),
        sa.Column("per_person_fen", sa.Integer, nullable=False, comment="人均价格（分）"),
        sa.Column("min_persons", sa.Integer, nullable=False, server_default="20", comment="最少人数"),
        sa.Column("min_tables", sa.Integer, nullable=False, server_default="2", comment="最少桌数"),
    ...
    op.create_table(
        ...
        sa.Column("section_name", sa.String(50), nullable=False, comment="节名：凉菜/热菜/海鲜/汤/主食/甜品/水果"),
        sa.Column("serve_sequence", sa.Integer, nullable=False, comment="出品顺序序号（凉菜=1最先，主食=最后）"),
        sa.Column(
            "serve_delay_minutes",
            sa.Integer,
            nullable=False,
    ...
    op.create_table(
        ...
        sa.Column("quantity_per_table", sa.Integer, nullable=False, server_default="1", comment="每桌份数"),
        sa.Column(
            "is_mandatory",
            sa.Boolean,
            nullable=False,
        ...
        sa.Column("alternative_dish_ids", JSONB, nullable=True, comment="可替换的其他菜品ID列表（换菜选项）"),
        sa.Column(
            "extra_price_fen",
            sa.Integer,
            nullable=False,
    ...
    op.create_table(
        ...
        sa.Column("current_section_id", UUID(as_uuid=True), nullable=True, comment="当前正在出品的节ID"),
        sa.Column("next_section_at", sa.TIMESTAMP(timezone=True), nullable=True, comment="下一节出品触发时间"),
    ...
    op.create_table(
        ...
        sa.Column("sync_status", sa.String(20), nullable=True, comment="idle/syncing/success/failed"),
    ...
    op.create_table(
        ...
        sa.Column("is_featured", sa.Boolean, nullable=False, server_default="false", comment="是否在渠道置顶推荐"),
        sa.Column("sort_order", sa.Integer, nullable=False, server_default="0"),
        sa.Column("created_at", sa.TIMESTAMP(timezone=True), server_default=sa.text("now()")),
        sa.Column("updated_at", sa.TIMESTAMP(timezone=True), server_default=sa.text("now()")),
        # 唯一约束：每个渠道每个菜品只有一条配置
