...
def upgrade() -> None:
    ...
    if "opportunity_relays" not in existing:
        op.create_table(
            ...
            sa.Column("relay_id", sa.String(50), nullable=False, unique=True,
                      comment="业务层ID，格式 relay-{hex8}"),
            sa.Column("opportunity_id", sa.String(100), nullable=False),
            sa.Column("opportunity_type", sa.String(50), nullable=False,
                      comment="新品机会/竞对防御/需求趋势/原料机会"),
            sa.Column("opportunity_title", sa.Text, nullable=False),
            sa.Column("opportunity_score", sa.Numeric(5, 1), nullable=False,
                      comment="机会评分 0-100"),
        ...
        op.create_index("ix_opportunity_relays_tenant_status",
                        "opportunity_relays", ["tenant_id", "status"])
        op.create_index("ix_opportunity_relays_opportunity_id",
                        "opportunity_relays", ["opportunity_id"])
    ...
    if "pilot_feedbacks" not in existing:
        op.create_table(
            ...
            sa.Column("id", UUID(as_uuid=True), primary_key=True,
                      server_default=sa.text("gen_random_uuid()")),
            sa.Column("tenant_id", UUID(as_uuid=True), nullable=False),
            sa.Column("feedback_id", sa.String(50), nullable=False, unique=True,
                      comment="业务层ID，格式 fb-{hex6}"),
            sa.Column("pilot_id", sa.String(100), nullable=False),
            sa.Column("relay_id", sa.String(50), nullable=False,
                      comment="关联 opportunity_relays.relay_id"),
            sa.Column("results", JSONB, nullable=False, server_default="'{}'"),
            sa.Column("metrics", JSONB, nullable=False, server_default="'{}'"),
            sa.Column("intel_model_updated", sa.Boolean, nullable=False,
                      server_default="false"),
            sa.Column("created_at", sa.TIMESTAMP(timezone=True),
                      server_default=sa.text("now()"), nullable=False),
            sa.Column("is_deleted", sa.Boolean, nullable=False,
        ...
        op.create_index("ix_pilot_feedbacks_tenant",
                        "pilot_feedbacks", ["tenant_id"])
        op.create_index("ix_pilot_feedbacks_relay_id",
                        "pilot_feedbacks", ["relay_id"])
