...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("priority", sa.Integer(), nullable=False, server_default="0", comment="优先级，越大越先匹配"),
        # 匹配条件（全部可为NULL，NULL=通配）
        sa.Column("match_dish_id", postgresql.UUID(as_uuid=True), nullable=True, comment="按菜品ID精确匹配"),
        sa.Column("match_dish_category", sa.String(50), nullable=True, comment="按菜品分类匹配"),
        sa.Column(
            "match_brand_id", postgresql.UUID(as_uuid=True), nullable=True, comment="按品牌ID匹配（多品牌共用厨房场景）"
        ),
        sa.Column(
            "match_channel", sa.String(30), nullable=True, comment="按渠道匹配：dine_in/takeaway/delivery/reservation"
        ),
        sa.Column("match_time_start", sa.Time(), nullable=True, comment="时段开始时间（含），如 11:00"),
        sa.Column("match_time_end", sa.Time(), nullable=True, comment="时段结束时间（含），如 14:00"),
        sa.Column("match_day_type", sa.String(20), nullable=True, comment="工作日类型：weekday/weekend/holiday"),
        # 路由目标
        sa.Column("target_dept_id", postgresql.UUID(as_uuid=True), nullable=False, comment="路由目标档口ID"),
        sa.Column("target_printer_id", postgresql.UUID(as_uuid=True), nullable=True, comment="可选覆盖打印机ID"),
        # 作用域
        sa.Column("is_active", sa.Boolean(), nullable=False, server_default="true", comment="规则是否启用"),
        sa.Column("store_id", postgresql.UUID(as_uuid=True), nullable=False, comment="规则所属门店"),
        # 外键约束
        sa.ForeignKeyConstraint(["target_dept_id"], ["production_depts.id"], name="fk_dispatch_rules_target_dept"),
    )
