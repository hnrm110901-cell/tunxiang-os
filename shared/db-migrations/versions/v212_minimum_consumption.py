...
def upgrade() -> None:
    ...
    if "minimum_consumption_configs" not in existing:
        op.create_table(
    ...
    if "minimum_consumption_surcharges" not in existing:
        op.create_table(
            ...
            sa.Column("dining_session_id", postgresql.UUID(as_uuid=True), nullable=False, comment="关联堂食会话"),
            sa.Column(
                "rule_type", sa.VARCHAR(30), nullable=False, comment="触发的规则类型: room/per_person/time_based"
            ),
            sa.Column("min_amount_fen", sa.BigInteger, nullable=False, comment="最低消费要求（分）"),
            sa.Column("actual_amount_fen", sa.BigInteger, nullable=False, comment="实际消费金额（分）"),
            sa.Column("surcharge_fen", sa.BigInteger, nullable=False, comment="补齐金额（分）= min - actual"),
            sa.Column("waived", sa.Boolean, nullable=False, server_default="false", comment="是否被豁免"),
            sa.Column("waive_reason", sa.VARCHAR(200), nullable=True, comment="豁免原因"),
            sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.text("NOW()"), nullable=False),
            sa.Column("is_deleted", sa.Boolean, nullable=False, server_default="false"),
        )
        op.create_index(
...
def downgrade() -> None:
    op.execute(
        "DROP POLICY IF EXISTS minimum_consumption_surcharges_tenant_isolation ON minimum_consumption_surcharges"
    )
