...
def upgrade() -> None:
    ...
    op.add_column(
        "stored_value_cards",
        sa.Column("main_balance_fen", sa.Integer, nullable=False, server_default="0", comment="本金余额(分)"),
    )
    op.add_column(
        "stored_value_cards",
        sa.Column("scope_type", sa.String(20), nullable=False, server_default="brand", comment="store|brand|group"),
    )
    op.add_column(
        "stored_value_cards",
        sa.Column(
            "scope_id", UUID(as_uuid=True), nullable=True, comment="scope_type=store 时存 store_id，brand 时存 brand_id"
        ),
    )
    op.add_column(
        "stored_value_cards",
        sa.Column("expiry_date", sa.Date, nullable=True, comment="过期日期，NULL=永不过期"),
    )
    op.add_column(
        "stored_value_cards",
    ...
    op.add_column(
        "stored_value_transactions",
        sa.Column("main_amount_fen", sa.Integer, nullable=False, server_default="0", comment="本金变动(分)"),
    )
    op.add_column(
        "stored_value_transactions",
        sa.Column("customer_id", UUID(as_uuid=True), nullable=True, comment="冗余存储便于查询"),
    )
    op.add_column(
        "stored_value_transactions",
        sa.Column("recharge_plan_id", sa.String(50), nullable=True, comment="充值套餐ID"),
    )
    ...
    op.create_table(
        ...
        sa.Column("gift_amount_fen", sa.Integer, nullable=False, server_default="0", comment="赠送金额(分)"),
        sa.Column("scope_type", sa.String(20), nullable=False, server_default="brand", comment="store|brand|group"),
...
def downgrade() -> None:
    ...
    for suffix in ("select", "insert", "update", "delete"):
        op.execute(f"DROP POLICY IF EXISTS stored_value_recharge_plans_rls_{suffix} ON stored_value_recharge_plans")
