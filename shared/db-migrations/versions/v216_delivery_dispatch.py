...
def upgrade() -> None:
    ...
    if "delivery_dispatches" not in existing:
        op.create_table(
            "delivery_dispatches",
            sa.Column(
            ...
            sa.Column("provider", sa.VARCHAR(20), nullable=False, comment="配送商: dada / shunfeng / self_rider"),
            sa.Column("provider_order_id", sa.VARCHAR(128), nullable=True, comment="三方配送单号"),
            sa.Column(
                "status",
                sa.VARCHAR(20),
                nullable=False,
            ...
            sa.Column("rider_updated_at", sa.TIMESTAMP(timezone=True), nullable=True, comment="骑手位置最后更新时间"),
            ...
            sa.Column("provider_callback_raw", postgresql.JSONB, nullable=True, comment="三方回调原始数据"),
    ...
    if "delivery_provider_configs" not in existing:
        op.create_table(
            ...
            sa.Column("tenant_id", postgresql.UUID(as_uuid=True), nullable=False),
            sa.Column("store_id", postgresql.UUID(as_uuid=True), nullable=False),
            sa.Column("provider", sa.VARCHAR(20), nullable=False, comment="dada / shunfeng / self_rider"),
            sa.Column("enabled", sa.BOOLEAN, server_default="false", nullable=False),
            sa.Column("priority", sa.INTEGER, server_default="0", comment="优先级, 数字越小越优先"),
            sa.Column("app_key", sa.VARCHAR(200), nullable=True, comment="三方 AppKey（加密存储）"),
            sa.Column("app_secret", sa.VARCHAR(200), nullable=True, comment="三方 AppSecret（加密存储）"),
            sa.Column("merchant_id", sa.VARCHAR(100), nullable=True, comment="三方商户号"),
            sa.Column("shop_no", sa.VARCHAR(100), nullable=True, comment="三方门店编号"),
            sa.Column("callback_url", sa.VARCHAR(500), nullable=True, comment="回调地址"),
            sa.Column(
                "extra_config", postgresql.JSONB, server_default="{}", comment="额外配置（如超时秒数、自动取消阈值等）"
            ),
            sa.Column("created_at", sa.TIMESTAMP(timezone=True), server_default=sa.text("now()")),
            sa.Column("updated_at", sa.TIMESTAMP(timezone=True), server_default=sa.text("now()")),
            sa.Column("is_deleted", sa.BOOLEAN, server_default="false", nullable=False),
        ...
        op.create_index(
            "ix_dpc_store_provider", "delivery_provider_configs", ["tenant_id", "store_id", "provider"], unique=True
        )
