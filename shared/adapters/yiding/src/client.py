...
class YiDingAPIError(Exception):
    ...
    def __init__(self, message: str, error_code: Optional[int] = None, response_data: Optional[Dict[str, Any]] = None):
...
class YiDingClient:
    ...
    async def _get_session(self) -> ClientSession:
        ...
        if self._session is None or self._session.closed:
            ...
            self._session = ClientSession(
                timeout=timeout,
                connector=connector,
                headers={"Content-Type": "application/json", "User-Agent": "TunxiangOS/1.0"},
            )
    ...
    async def get_token(self) -> str:
        ...
        try:
            async with session.get(url, params={"appid": self.appid, "secret": self.secret}) as response:
                data = await response.json()

                if data.get("error_code") != 0:
                    error_msg = data.get("error_msg", "认证失败")
                    self.logger.error("yiding_token_failed", error_code=data.get("error_code"), error_msg=error_msg)
                    raise YiDingAPIError(
                        message=f"易订认证失败: {error_msg}", error_code=data.get("error_code"), response_data=data
                    )
                ...
                self.logger.info("yiding_token_ok", business_name=self._business_name)
    ...
    async def _request(
        ...
        **kwargs,
        ...
        for attempt in range(self.max_retries):
            try:
                self.logger.info("yiding_api_request", method=method, url=url, attempt=attempt + 1)
                ...
                async with session.request(method, url, **request_kwargs) as response:
                    try:
                        ...
                        raise YiDingAPIError(f"易订API返回非JSON: {text[:200]}")

                    self.logger.info("yiding_api_response", status=response.status, error_code=data.get("error_code"))
                    ...
                    if error_code is not None and int(error_code) != 0:
                        ...
                        if int(error_code) in (-2, -3):
                            ...
                            if attempt < self.max_retries - 1:
                                self.logger.warning("yiding_token_expired_retry", attempt=attempt + 1)
                                continue

                        raise YiDingAPIError(
                            message=f"易订API错误: {error_msg}", error_code=int(error_code), response_data=data
                        )
                ...
                last_error = e
                self.logger.warning("yiding_request_failed", attempt=attempt + 1, error=str(e))
                if attempt < self.max_retries - 1:
                    wait_time = 2**attempt
    ...
    async def put(self, path: str, json: Optional[Dict[str, Any]] = None, **kwargs) -> Dict[str, Any]:
        """PUT请求"""
