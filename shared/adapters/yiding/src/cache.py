...
class YiDingCache:
    ...
    async def get_reservation(self, reservation_id: str) -> Optional[UnifiedReservation]:
        """获取预订缓存"""
        async with self._lock:
            key = f"reservation:{reservation_id}"
            return self._get_cache(key)

    async def set_reservation(self, reservation_id: str, reservation: UnifiedReservation, ttl: Optional[int] = None):
        """设置预订缓存"""
        async with self._lock:
            key = f"reservation:{reservation_id}"
    ...
    async def get_reservations(self, store_id: str, date: str) -> Optional[List[UnifiedReservation]]:
        """获取预订列表缓存"""
        async with self._lock:
    ...
    async def set_reservations(
        self, store_id: str, date: str, reservations: List[UnifiedReservation], ttl: Optional[int] = None
    ):
        """设置预订列表缓存"""
        async with self._lock:
            key = f"reservations:{store_id}:{date}"
    ...
    async def set_customer(self, customer_id: str, customer: Any, ttl: Optional[int] = None):
        """设置客户缓存"""
        async with self._lock:
            key = f"customer:{customer_id}"
    ...
    async def get_cache_stats(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            return {"total_keys": total_keys, "active_keys": total_keys - expired_keys, "expired_keys": expired_keys}
