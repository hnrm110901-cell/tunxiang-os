...
class ERPVoucherEntry(BaseModel):
    ...
    def validate_debit_credit(self) -> "ERPVoucherEntry":
        ...
        if self.debit_fen == 0 and self.credit_fen == 0:
            raise ValueError(f"分录 [{self.account_code}] 借贷金额不能同时为 0")
        if self.debit_fen > 0 and self.credit_fen > 0:
            raise ValueError(f"分录 [{self.account_code}] 借贷金额不能同时非零（单边原则）")
        return self
...
class ERPVoucher(BaseModel):
    ...
    entries: list[ERPVoucherEntry] = Field(..., min_length=2, description="凭证分录列表，至少 2 条")
    ...
    def validate_balance(self) -> "ERPVoucher":
        ...
        if total_debit != total_credit:
            raise ValueError(f"凭证借贷不平衡: 借方合计={total_debit}分, 贷方合计={total_credit}分")
        return self
...
class ERPPushResult(BaseModel):
    ...
    erp_voucher_id: str | None = Field(default=None, description="ERP 系统返回的凭证 ID（失败时为 None）")
    status: PushStatus
    erp_type: ERPType
    error_message: str | None = Field(default=None)
    pushed_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
    raw_response: dict[str, Any] = Field(default_factory=dict, description="ERP 原始响应（用于排查问题）")
