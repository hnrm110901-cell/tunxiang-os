...
class LogisticsAdapter(ABC):
    ...
    async def _call_with_retry(self, fn, *args, max_retries: int = 2, **kwargs):
        """指数退避重试包装"""
    ...
    async def query_track(
        ...
        try:
            result = await self._call_with_retry(self._client.query_track, tracking_no, carrier_code)
    ...
    async def auto_detect(self, tracking_no: str) -> dict[str, Any]:
        ...
        try:
            result = await self._call_with_retry(self._client.auto_detect, tracking_no)
        except (OSError, RuntimeError, ValueError) as exc:
    ...
    async def subscribe_push(
        ...
        try:
            result = await self._call_with_retry(self._client.subscribe_push, tracking_no, carrier_code, callback_url)
        except (OSError, RuntimeError, ValueError) as exc:
