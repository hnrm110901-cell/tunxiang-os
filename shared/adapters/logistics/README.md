...
| 字段 | 类型 | 说明 | 快递100响应字段 |
|------|------|------|----------------|
| `status` | str | 查询状态（ok/error） | `status` |
| `state` | str | 物流状态码（0-7, 10-14） | `state` |
| `carrier_code` | str | 快递公司编码 | `com` |
| `tracking_no` | str | 快递单号 | `nu` |
| `traces[].time` | str | 轨迹时间 | `traces[].time` |
| `traces[].context` | str | 轨迹描述 | `traces[].context` |
| `state_label` | str | 中文状态描述（适配器附加） | - |
