...
def _make_app() -> FastAPI:
    ...
    # Webhook 路径示例（pre-cutover hot-patch — 防外部回调被 401 拦截）
...
class TestExternalWebhookExempt:
    """外部平台 webhook 必须无条件放行（pre-cutover hot-patch）—— 美团/饿了么/抖音/微信
    等回调走公网入口，不带 X-Internal-JWT；webhook 路由内部有平台签名校验。

    详见 docs/runbooks/audit-2026-05-execution-plan.md §九。
    """
